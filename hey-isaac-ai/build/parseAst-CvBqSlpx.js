import { __commonJS, __require, __toESM } from "./chunk-BMwS5_Mx.js";
import { basename, dirname, extname, resolve } from "node:path";

//#region node_modules/rollup/dist/native.js
var require_native = __commonJS({ "node_modules/rollup/dist/native.js"(exports, module) {
	const { existsSync } = __require("node:fs");
	const path$1 = __require("node:path");
	const { platform, arch, report } = __require("node:process");
	...
	const requireWithFriendlyError = (id) => {
		try {
			...
			if (platform === "win32" && error$1 instanceof Error && error$1.code === "ERR_DLOPEN_FAILED" && error$1.message.includes("The specified module could not be found")) {
				const msvcDownloadLink = `https://aka.ms/vs/17/release/${msvcLinkFilenameByArch[arch]}`;
				throw new Error(`Failed to load module ${id}. Required DLL was not found. This error usually happens when Microsoft Visual C++ Redistributable is not installed. You can download it from ${msvcDownloadLink}`, { cause: error$1 });
			}
			throw new Error(`Cannot find module ${id}. npm has a bug related to optional dependencies (https://github.com/npm/cli/issues/4828). Please try \`npm i\` again after removing both package-lock.json and node_modules directory.`, { cause: error$1 });
		}
	};
	const { parse, parseAsync, xxhashBase64Url, xxhashBase36, xxhashBase16 } = requireWithFriendlyError(existsSync(path$1.join(__dirname, localName)) ? localName : `@rollup/rollup-${packageBase}`);
	function getPackageBase() {
		const imported = bindingsByPlatformAndArch[platform]?.[arch];
		if (!imported) throwUnsupportedError(false);
		if ("musl" in imported && isMusl()) return imported.musl || throwUnsupportedError(true);
		return imported.base;
	}
	function throwUnsupportedError(isMusl$1) {
		throw new Error(`Your current platform "${platform}${isMusl$1 ? " (musl)" : ""}" and architecture "${arch}" combination is not yet supported by the native Rollup build. Please use the WASM build "@rollup/wasm-node" instead.

The following platform-architecture combinations are supported:
${Object.entries(bindingsByPlatformAndArch).flatMap(([platformName, architectures]) => Object.entries(architectures).flatMap(([architectureName, { musl }]) => {
...
//#endregion
//#region node_modules/rollup/dist/es/shared/parseAst.js
...
function isAbsolute$1(path$2) {
	return ABSOLUTE_PATH_REGEX.test(path$2);
}
function isRelative(path$2) {
	return RELATIVE_PATH_REGEX.test(path$2);
}
const BACKSLASH_REGEX = /\\/g;
function normalize$1(path$2) {
	return path$2.replace(BACKSLASH_REGEX, "/");
}
function printQuotedStringList(list, verbs) {
	...
	if (verbs) output += ` ${isSingleItem ? verbs[0] : verbs[1]}`;
	return output;
}
...
function relativeId(id) {
	if (!isAbsolute$1(id)) return id;
	return relative$1(resolve(), id);
}
function isPathFragment(name) {
	return name[0] === "/" || name[0] === "." && (name[1] === "/" || name[1] === ".") || isAbsolute$1(name);
}
...
function getImportPath(importerId, targetPath, stripJsExtension, ensureFileName) {
	...
	let relativePath = normalize$1(relative$1(dirname(importerId), targetPath));
	...
	if (ensureFileName) {
		...
		if (UPPER_DIR_REGEX.test(relativePath)) return [
			...relativePath.split("/"),
			"..",
			basename(targetPath)
		].join("/");
...
function augmentLogMessage(log) {
	if (!(log.plugin || log.loc) || log[symbolAugmented]) return;
	log[symbolAugmented] = true;
	let prefix = "";
	if (log.plugin) prefix += `[plugin ${log.plugin}] `;
...
function logAmbiguousExternalNamespaces(binding, reexportingModule, usedModule, sources) {
	return {
		...
		ids: sources,
		message: `Ambiguous external namespace resolution: "${relativeId(reexportingModule)}" re-exports "${binding}" from one of the external modules ${printQuotedStringList(sources.map((module$1) => relativeId(module$1)))}, guessing "${relativeId(usedModule)}".`,
...
function tweakStackMessage(error$1, oldMessage) {
	if (!error$1.stack) return error$1;
	error$1.stack = error$1.stack.replace(oldMessage, error$1.message);
	return error$1;
}
...
function logMissingExport(binding, importingModule, exporter, missingButExportExists) {
	...
	if (missingButExportExists) return {
		...baseLog,
...
function logImplicitDependantIsNotIncluded(module$1) {
	const implicitDependencies = [...module$1.implicitlyLoadedBefore].map((dependency) => relativeId(dependency.id)).sort();
	return {
		code: MISSING_IMPLICIT_DEPENDANT,
		message: `Module "${relativeId(module$1.id)}" that should be implicitly loaded before ${printQuotedStringList(implicitDependencies)} is not included in the module graph. Either it was not imported by an included module or only via a tree-shaken dynamic import, or no imported bindings were used and it had otherwise no side-effects.`
	};
}
...
function logModuleParseError(error$1, moduleId) {
	let message = error$1.message.replace(/ \(\d+:\d+\)$/, "");
	if (moduleId.endsWith(".json")) message += " (Note that you need @rollup/plugin-json to import JSON files)";
	else if (!moduleId.endsWith(".js")) message += " (Note that you need plugins to import files that are not JavaScript)";
...
function logInvalidSourcemapForError(error$1, id, column, line, pos) {
	return {
		cause: error$1,
		...
		message: `Error when using sourcemap for reporting an error: ${error$1.message}`,
		pos
	};
}
...
function warnDeprecationWithOptions(deprecation, urlSnippet, activeDeprecation, log, strictDeprecations, plugin) {
	if (activeDeprecation || strictDeprecations) {
		...
		if (strictDeprecations) return error(warning);
...
const convertAnnotations = (position, buffer) => {
	...
	for (let index = 0; index < length; index++) list[index] = convertAnnotation(buffer[position++], buffer);
	return list;
};
...
function convertProgram(buffer) {
	...
	switch (node.type) {
		case PanicError: return error(getRollupError(logParseError(node.message)));
		case ParseError: return error(getRollupError(logParseError(node.message, node.start)));
		default: return node;
	}
}
...
function getAstBuffer(astBuffer) {
	...
	let convertString;
	if (typeof Buffer !== "undefined" && astBuffer instanceof Buffer) convertString = (position) => {
