...
import { createFilter$1$1 as createFilter$1, fseventsImporter, getAugmentedNamespace, getDefaultExportFromCjs$1 as getDefaultExportFromCjs, rollupInternal } from "./node-entry-Dhg1gigt.js";
...
//#region node_modules/rollup/dist/es/shared/watch.js
var chokidar$1 = {};
var utils$2 = {};
var constants$3;
...
function requireUtils$2() {
	...
	(function(exports) {
		...
		const { REGEX_BACKSLASH, REGEX_REMOVE_BACKSLASH, REGEX_SPECIAL_CHARS, REGEX_SPECIAL_CHARS_GLOBAL } = /* @__PURE__ */ requireConstants$3();
		...
		exports.isWindows = (options) => {
			...
			return win32$1 === true || path$2.sep === "\\";
		};
...
function requireScan$1() {
	...
	hasRequiredScan$1 = 1;
	const utils$3 = /* @__PURE__ */ requireUtils$2();
...
function requireParse$2() {
	...
	const constants$5 = /* @__PURE__ */ requireConstants$3();
	const utils$3 = /* @__PURE__ */ requireUtils$2();
	/**
	* Constants
	*/
	const { MAX_LENGTH, POSIX_REGEX_SOURCE, REGEX_NON_SPECIAL_CHARS, REGEX_SPECIAL_CHARS_BACKREF, REPLACEMENTS } = constants$5;
	/**
	* Helpers
	*/
	...
	*/
	...
	const getStarExtglobSequenceOutput = (pattern) => {
		...
		const source = chars.length === 1 ? utils$3.escapeRegex(chars[0]) : `[${chars.map((ch) => utils$3.escapeRegex(ch)).join("")}]`;
		return `${source}*`;
	};
	...
	const analyzeRepeatedExtglob = (body, options) => {
		...
		const max = typeof options.maxExtglobRecursion === "number" ? options.maxExtglobRecursion : constants$5.DEFAULT_MAX_EXTGLOB_RECURSION;
	...
	*/
	const parse = (input, options) => {
		...
		const win32$1 = utils$3.isWindows(options);
		const PLATFORM_CHARS = constants$5.globChars(win32$1);
		...
		*/
		...
		const extglobClose = (token) => {
			...
			if ((token.type === "plus" || token.type === "star") && analysis.risky) {
				...
				open.type = "text";
				open.value = literal;
				open.output = safeOutput || utils$3.escapeRegex(literal);
		...
		while (!eos()) {
			...
			*/
			...
			if (value === "]") {
				...
				prev.value += value;
				append({ value });
				if (opts.literalBrackets === false || utils$3.hasRegexChars(prevValue)) continue;
			...
			*/
			if (value === "?") {
				...
				if (prev && prev.type === "paren") {
					const next = peek();
					let output = value;
					if (next === "<" && !utils$3.supportsLookbehinds()) throw new Error("Node.js v10 or higher is required for regex lookbehinds");
	...
	*/
	parse.fastpaths = (input, options) => {
		...
		input = REPLACEMENTS[input] || input;
		const win32$1 = utils$3.isWindows(options);
		const { DOT_LITERAL, SLASH_LITERAL, ONE_CHAR, DOTS_SLASH, NO_DOT, NO_DOTS, NO_DOTS_SLASH, STAR, START_ANCHOR } = constants$5.globChars(win32$1);
...
function requirePicomatch$3() {
	...
	*/
	const picomatch$2 = (glob, options, returnState = false) => {
		...
		const opts = options || {};
		const posix$2 = utils$3.isWindows(options);
	...
	* @api public
	*/
	picomatch$2.test = (input, regex, options, { glob, posix: posix$2 } = {}) => {
		...
		const opts = options || {};
		const format = opts.format || (posix$2 ? utils$3.toPosixSlashes : null);
		...
		if (match === false || opts.capture === true) if (opts.matchBase === true || opts.basename === true) match = picomatch$2.matchBase(input, regex, options, posix$2);
	...
	* @api public
	*/
	picomatch$2.matchBase = (input, glob, options, posix$2 = utils$3.isWindows(options)) => {
	...
	* @return {Object}
	*/
	picomatch$2.constants = constants$5;
	/**
	* Expose "picomatch"
	*/
	picomatch_1$1 = picomatch$2;
	...
	*/
	...
	const normalizeFilter = (filter) => {
		...
		if (Array.isArray(filter)) {
			...
			for (const item of filter) {
				...
				if (trimmed.charAt(0) === BANG) negative.push(picomatch$2(trimmed.slice(1)));
	...
	class ReaddirpStream extends Readable$1 {
		static get defaultOptions() {
			return {
				root: ".",
				fileFilter: (path$2) => true,
				directoryFilter: (path$2) => true,
				type: FILE_TYPE,
				lstat: false,
				depth: 2147483648,
				alwaysStat: false
		...
		async _read(batch) {
			...
			try {
				while (!this.destroyed && batch > 0) {
					const { path: path$2, depth, files = [] } = this.parent || {};
					if (files.length > 0) {
						const slice = files.splice(0, batch).map((dirent) => this._formatEntry(dirent, path$2));
		...
		async _formatEntry(dirent, path$2) {
			...
			try {
				...
				const fullPath = sysPath.resolve(sysPath.join(path$2, basename$2));
				entry = {
	...
	*/
	...
	const readdirpPromise = (root, options = {}) => {
		return new Promise((resolve$2, reject) => {
			const files = [];
			readdirp(root, options).on("data", (entry) => files.push(entry)).on("end", () => resolve$2(files)).on("error", (error) => reject(error));
		});
	};
...
function requireUtils$1() {
	...
	(function(exports) {
		const path$2 = path$1;
		const win32$1 = process.platform === "win32";
		const { REGEX_BACKSLASH, REGEX_REMOVE_BACKSLASH, REGEX_SPECIAL_CHARS, REGEX_SPECIAL_CHARS_GLOBAL } = /* @__PURE__ */ requireConstants$2();
		exports.isObject = (val) => val !== null && typeof val === "object" && !Array.isArray(val);
		exports.hasRegexChars = (str) => REGEX_SPECIAL_CHARS.test(str);
		exports.isRegexChar = (str) => str.length === 1 && exports.hasRegexChars(str);
		exports.escapeRegex = (str) => str.replace(REGEX_SPECIAL_CHARS_GLOBAL, "\\$1");
...
function requireScan() {
	...
	hasRequiredScan = 1;
	const utils$3 = /* @__PURE__ */ requireUtils$1();
	const { CHAR_ASTERISK, CHAR_AT, CHAR_BACKWARD_SLASH, CHAR_COMMA, CHAR_DOT, CHAR_EXCLAMATION_MARK, CHAR_FORWARD_SLASH, CHAR_LEFT_CURLY_BRACE, CHAR_LEFT_PARENTHESES, CHAR_LEFT_SQUARE_BRACKET, CHAR_PLUS, CHAR_QUESTION_MARK, CHAR_RIGHT_CURLY_BRACE, CHAR_RIGHT_PARENTHESES, CHAR_RIGHT_SQUARE_BRACKET } = /* @__PURE__ */ requireConstants$2();
	const isPathSeparator = (code) => {
		return code === CHAR_FORWARD_SLASH || code === CHAR_BACKWARD_SLASH;
	};
...
function requireParse$1() {
	...
	const constants$5 = /* @__PURE__ */ requireConstants$2();
	const utils$3 = /* @__PURE__ */ requireUtils$1();
	/**
	* Constants
	*/
...
function requireNormalizePath() {
	...
	normalizePath = function(path$2, stripTrailing) {
		if (typeof path$2 !== "string") throw new TypeError("expected path to be a string");
		if (path$2 === "\\" || path$2 === "/") return "/";
		var len = path$2.length;
		if (len <= 1) return path$2;
		var prefix = "";
		if (len > 4 && path$2[3] === "\\") {
			var ch = path$2[2];
			if ((ch === "?" || ch === ".") && path$2.slice(0, 2) === "\\\\") {
				path$2 = path$2.slice(2);
				prefix = "//";
			}
		}
		var segs = path$2.split(/[/\\]+/);
...
function requireAnymatch() {
	...
	const picomatch$2 = /* @__PURE__ */ requirePicomatch();
	const normalizePath$1 = /* @__PURE__ */ requireNormalizePath();
	/**
	...
	*/
	const createPattern = (matcher, options) => {
		...
		if (typeof matcher === "string") {
			const glob = picomatch$2(matcher, options);
	...
	*/
	const matchPatterns = (patterns, negPatterns, args, returnIndex) => {
		...
		const path$2 = normalizePath$1(_path, false);
		for (let index = 0; index < negPatterns.length; index++) {
			const nglob = negPatterns[index];
			if (nglob(path$2)) return returnIndex ? -1 : false;
		}
		const applied = isList && [path$2].concat(args.slice(1));
		for (let index = 0; index < patterns.length; index++) {
			const pattern = patterns[index];
			if (isList ? pattern(...applied) : pattern(path$2)) return returnIndex ? index : true;
		}
	...
	*/
	const anymatch$1 = (matchers, testString, options = DEFAULT_OPTIONS) => {
		...
		const negatedGlobs = mtchers.filter((item) => typeof item === "string" && item.charAt(0) === BANG).map((item) => item.slice(1)).map((item) => picomatch$2(item, opts));
...
function requireIsExtglob() {
	...
	isExtglob = function isExtglob$1(str) {
...
function requireIsGlob() {
	...
	hasRequiredIsGlob = 1;
	var isExtglob$1 = /* @__PURE__ */ requireIsExtglob();
	var chars = {
		"{": "}",
		"(": ")",
		"[": "]"
	...
	isGlob = function isGlob$1(str, options) {
		if (typeof str !== "string" || str === "") return false;
...
function requireGlobParent() {
	...
	var isGlob$1 = /* @__PURE__ */ requireIsGlob();
	...
	* @returns {string}
	*/
	globParent = function globParent$1(str, opts) {
...
function requireStringify() {
	...
	const utils$3 = /* @__PURE__ */ requireUtils();
	stringify = (ast, options = {}) => {
		const stringify$1 = (node, parent = {}) => {
			const invalidBlock = options.escapeInvalid && utils$3.isInvalidBrace(parent);
			const invalidNode = node.invalid === true && options.escapeInvalid === true;
			let output = "";
			if (node.value) {
				if ((invalidBlock || invalidNode) && utils$3.isOpenOrClose(node)) return "\\" + node.value;
				return node.value;
			}
...
function requireToRegexRange() {
	...
	const isNumber$1 = /* @__PURE__ */ requireIsNumber();
...
function requireExpand() {
	...
	hasRequiredExpand = 1;
	const fill = /* @__PURE__ */ requireFillRange();
	const stringify$1 = /* @__PURE__ */ requireStringify();
	const utils$3 = /* @__PURE__ */ requireUtils();
	const append = (queue = "", stash = "", enclose = false) => {
		...
		if (!queue.length) return enclose ? utils$3.flatten(stash).map((ele) => `{${ele}}`) : stash;
	...
	const expand = (ast, options = {}) => {
		...
		const walk = (node, parent = {}) => {
			...
			if (node.nodes && node.ranges > 0) {
				const args = utils$3.reduce(node.nodes);
				if (utils$3.exceedsLimit(...args, options.step, rangeLimit)) throw new RangeError("expanded array length exceeds range limit. Use options.rangeLimit to increase or disable the limit.");
				let range = fill(...args, options);
				if (range.length === 0) range = stringify$1(node, options);
...
function requireBraces() {
	...
	*/
	braces.stringify = (input, options = {}) => {
		if (typeof input === "string") return stringify$1(braces.parse(input, options), options);
...
function requireIsBinaryPath() {
	...
	const path$2 = path$1;
	const binaryExtensions$1 = /* @__PURE__ */ requireBinaryExtensions();
	const extensions = new Set(binaryExtensions$1);
	isBinaryPath = (filePath) => extensions.has(path$2.extname(filePath).slice(1).toLowerCase());
	return isBinaryPath;
}
var constants$4 = {};
...
function requireNodefsHandler() {
	...
	const sysPath = path$1;
	const { promisify } = require$$1;
	const isBinaryPath$1 = /* @__PURE__ */ requireIsBinaryPath();
	const { isWindows, isLinux, EMPTY_FN, EMPTY_STR, KEY_LISTENERS, KEY_ERR, KEY_RAW, HANDLER_KEYS, EV_CHANGE, EV_ADD, EV_ADD_DIR, EV_ERROR, STR_DATA, STR_END, BRACE_START, STAR } = /* @__PURE__ */ requireConstants();
	const THROTTLE_MODE_WATCH = "watch";
	const open = promisify(fs$1.open);
	const stat$1 = promisify(fs$1.stat);
	const lstat = promisify(fs$1.lstat);
	const close = promisify(fs$1.close);
	const fsrealpath = promisify(fs$1.realpath);
	const statMethods = {
		lstat,
		stat: stat$1
	};
	...
	*/
	function createFsWatchInstance(path$2, options, listener, errHandler, emitRaw) {
		...
		try {
			return fs$1.watch(path$2, options, handleEvent);
		} catch (error) {
			errHandler(error);
		}
	}
	...
	class NodeFsHandler {
		...
		*/
		_watchWithNodeFs(path$2, listener) {
			...
			parent.add(basename$2);
			const absolutePath = sysPath.resolve(path$2);
			const options = { persistent: opts.persistent };
			if (!listener) listener = EMPTY_FN;
			let closer;
			if (opts.usePolling) {
				options.interval = opts.enableBinaryInterval && isBinaryPath$1(basename$2) ? opts.binaryInterval : opts.interval;
			...
			const listener = async (path$2, newStats) => {
				...
				if (!newStats || newStats.mtimeMs === 0) try {
					...
					if (isLinux && prevStats.ino !== newStats$1.ino) {
						...
						this.fsw._addPathCloser(path$2, this._watchWithNodeFs(file, listener));
		...
		*/
		async _handleSymlink(entry, directory, path$2, item) {
			...
			if (!this.fsw.options.followSymlinks) {
				...
				let linkPath;
				try {
					linkPath = await fsrealpath(path$2);
				} catch (e) {
					this.fsw._emitReady();
					return true;
				}
				...
				if (dir.has(item)) {
					if (this.fsw._symlinkPaths.get(full) !== linkPath) {
						...
						this.fsw._emit(EV_CHANGE, path$2, entry.stats);
					}
				} else {
					dir.add(item);
					this.fsw._symlinkPaths.set(full, linkPath);
					this.fsw._emit(EV_ADD, path$2, entry.stats);
				}
				this.fsw._emitReady();
				return true;
			}
		...
		_handleRead(directory, initialAdd, wh, target, dir, depth, throttler) {
			...
			return new Promise((resolve$2) => stream.once(STR_END, () => {
				if (this.fsw.closed) {
					stream = void 0;
					return;
				}
				...
				previous.getChildren().filter((item) => {
					return item !== directory && !current.has(item) && (!wh.hasGlob || wh.filterPath({ fullPath: sysPath.resolve(directory, item) }));
				}).forEach((item) => {
		...
		*/
		async _addToNodeFs(path$2, initialAdd, priorWh, depth, target) {
			...
			const wh = this.fsw._getWatchHelpers(path$2, depth);
			...
			try {
				...
				const follow = this.fsw.options.followSymlinks && !path$2.includes(STAR) && !path$2.includes(BRACE_START);
				let closer;
				if (stats.isDirectory()) {
					const absPath = sysPath.resolve(path$2);
					const targetPath = follow ? await fsrealpath(path$2) : path$2;
					if (this.fsw.closed) return;
				...
				ready();
				this.fsw._addPathCloser(path$2, closer);
				return false;
			} catch (error) {
...
function requireFseventsHandler() {
	...
	if (fsevents) {
		...
		if (mtch && mtch[1] && mtch[2]) {
			...
			if (maj === 8 && min < 16) fsevents = void 0;
		}
	}
	const { EV_ADD, EV_CHANGE, EV_ADD_DIR, EV_UNLINK, EV_ERROR, STR_DATA, STR_END, FSEVENT_CREATED, FSEVENT_MODIFIED, FSEVENT_DELETED, FSEVENT_MOVED, FSEVENT_UNKNOWN, FSEVENT_FLAG_MUST_SCAN_SUBDIRS, FSEVENT_TYPE_FILE, FSEVENT_TYPE_DIRECTORY, FSEVENT_TYPE_SYMLINK, ROOT_GLOBSTAR, DIR_SUFFIX, DOT_SLASH, FUNCTION_TYPE, EMPTY_FN, IDENTITY_FN } = /* @__PURE__ */ requireConstants();
	...
	*/
	const createFSEventsInstance = (path$2, callback) => {
		const stop = fsevents.watch(path$2, callback);
		return { stop };
	};
	/**
	...
	* @returns {Function} closer
	*/
	function setFSEventsListener(path$2, realPath, listener, rawEmitter) {
		let watchPath = sysPath.extname(realPath) ? sysPath.dirname(realPath) : realPath;
		const parentPath = sysPath.dirname(watchPath);
		let cont = FSEventsWatchers.get(watchPath);
		if (couldConsolidate(parentPath)) watchPath = parentPath;
		const resolvedPath = sysPath.resolve(path$2);
		...
		for (const watchedPath of FSEventsWatchers.keys()) if (realPath.indexOf(sysPath.resolve(watchedPath) + sysPath.sep) === 0) {
	...
	const couldConsolidate = (path$2) => {
		let count = 0;
		for (const watchPath of FSEventsWatchers.keys()) if (watchPath.indexOf(path$2) === 0) {
			count++;
	...
	const calcDepth = (path$2, root) => {
		let i = 0;
		while (!path$2.indexOf(root) && (path$2 = sysPath.dirname(path$2)) !== root) i++;
		return i;
	};
	...
	*/
	class FsEventsHandler {
		...
		*/
		...
		checkIgnored(path$2, stats) {
			const ipaths = this.fsw._ignoredPaths;
			if (this.fsw._isIgnored(path$2, stats)) {
				ipaths.add(path$2);
				if (stats && stats.isDirectory()) ipaths.add(path$2 + ROOT_GLOBSTAR);
				return true;
			}
			ipaths.delete(path$2);
			ipaths.delete(path$2 + ROOT_GLOBSTAR);
		}
		addOrChange(path$2, fullPath, realPath, parent, watchedDir, item, info, opts) {
			const event = watchedDir.has(item) ? EV_CHANGE : EV_ADD;
			this.handleEvent(event, path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
		}
		async checkExists(path$2, fullPath, realPath, parent, watchedDir, item, info, opts) {
			try {
				const stats = await stat$1(path$2);
				if (this.fsw.closed) return;
				if (sameTypes(info, stats)) this.addOrChange(path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
				else this.handleEvent(EV_UNLINK, path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
			} catch (error) {
				if (error.code === "EACCES") this.addOrChange(path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
				else this.handleEvent(EV_UNLINK, path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
			}
		}
		handleEvent(event, path$2, fullPath, realPath, parent, watchedDir, item, info, opts) {
			if (this.fsw.closed || this.checkIgnored(path$2)) return;
			if (event === EV_UNLINK) {
				const isDirectory = info.type === FSEVENT_TYPE_DIRECTORY;
				if (isDirectory || watchedDir.has(item)) this.fsw._remove(parent, item, isDirectory);
			} else {
				if (event === EV_ADD) {
					if (info.type === FSEVENT_TYPE_DIRECTORY) this.fsw._getWatchedDir(path$2);
					if (info.type === FSEVENT_TYPE_SYMLINK && opts.followSymlinks) {
						const curDepth = opts.depth === void 0 ? void 0 : calcDepth(fullPath, realPath) + 1;
						return this._addToFsEvents(path$2, false, true, curDepth);
					}
				...
				*/
				...
				this.fsw._emit(eventName, path$2);
				if (eventName === EV_ADD_DIR) this._addToFsEvents(path$2, false, true);
			}
		}
		/**
		...
		*/
		_watchWithFsEvents(watchPath, realPath, transform, globFilter) {
			...
			const watchCallback = async (fullPath, flags, info) => {
				...
				const path$2 = transform(sysPath.join(watchPath, sysPath.relative(watchPath, fullPath)));
				if (globFilter && !globFilter(path$2)) return;
				const parent = sysPath.dirname(path$2);
				const item = sysPath.basename(path$2);
				const watchedDir = this.fsw._getWatchedDir(info.type === FSEVENT_TYPE_DIRECTORY ? path$2 : parent);
				if (wrongEventFlags.has(flags) || info.event === FSEVENT_UNKNOWN) if (typeof opts.ignored === FUNCTION_TYPE) {
					let stats;
					try {
						stats = await stat$1(path$2);
					} catch (error) {}
					if (this.fsw.closed) return;
					if (this.checkIgnored(path$2, stats)) return;
					if (sameTypes(info, stats)) this.addOrChange(path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
					else this.handleEvent(EV_UNLINK, path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
				} else this.checkExists(path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
				else switch (info.event) {
					case FSEVENT_CREATED:
					case FSEVENT_MODIFIED: return this.addOrChange(path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
					case FSEVENT_DELETED:
					case FSEVENT_MOVED: return this.checkExists(path$2, fullPath, realPath, parent, watchedDir, item, info, opts);
				}
			};
		...
		*/
		async _handleFsEventsSymlink(linkPath, fullPath, transform, curDepth) {
			...
			try {
				...
				this.fsw._incrReadyCount();
				this._addToFsEvents(linkTarget || linkPath, (path$2) => {
					let aliasedPath = linkPath;
					if (linkTarget && linkTarget !== DOT_SLASH) aliasedPath = path$2.replace(linkTarget, linkPath);
					else if (path$2 !== DOT_SLASH) aliasedPath = sysPath.join(linkPath, path$2);
		...
		*/
		...
		initWatch(realPath, path$2, wh, processPath) {
			if (this.fsw.closed) return;
		...
		* @returns {Promise<void>}
		*/
		async _addToFsEvents(path$2, transform, forceAdd, priorDepth) {
			if (this.fsw.closed) return;
			const opts = this.fsw.options;
			...
			try {
				...
				if (stats.isDirectory()) {
					if (!wh.globFilter) this.emitAdd(processPath(path$2), stats, processPath, opts, forceAdd);
			...
			if (opts.persistent && forceAdd !== true) if (typeof transform === FUNCTION_TYPE) this.initWatch(void 0, path$2, wh, processPath);
			else {
				let realPath;
				try {
					realPath = await realpath$1(wh.watchPath);
				} catch (e) {}
				this.initWatch(realPath, path$2, wh, processPath);
			}
		}
	}
...
function requireChokidar() {
	...
	const anymatch$1 = requireAnymatch().default;
	const globParent$1 = /* @__PURE__ */ requireGlobParent();
	const isGlob$1 = /* @__PURE__ */ requireIsGlob();
	...
	*/
	...
	const normalizeIgnored = (cwd = EMPTY_STR) => (path$2) => {
		if (typeof path$2 !== STRING_TYPE) return path$2;
		return normalizePathToUnix(sysPath.isAbsolute(path$2) ? path$2 : sysPath.join(cwd, path$2));
	};
	const getAbsolutePath = (path$2, cwd) => {
		if (sysPath.isAbsolute(path$2)) return path$2;
		if (path$2.startsWith(BANG)) return BANG + sysPath.join(cwd, path$2.slice(1));
	...
	*/
	...
	class WatchHelper {
		constructor(path$2, watchPath, follow, fsw) {
			this.fsw = fsw;
			this.path = path$2 = path$2.replace(REPLACER_RE, EMPTY_STR);
			this.watchPath = watchPath;
			this.fullWatchPath = sysPath.resolve(watchPath);
			this.hasGlob = watchPath !== path$2;
			/** @type {object|boolean} */
			if (path$2 === EMPTY_STR) this.hasGlob = false;
			this.globSymlink = this.hasGlob && follow ? void 0 : false;
			this.globFilter = this.hasGlob ? anymatch$1(path$2, void 0, ANYMATCH_OPTS) : false;
			this.dirParts = this.getDirParts(path$2);
		...
		getDirParts(path$2) {
			...
			const parts = [];
			const expandedPath = path$2.includes(BRACE_START) ? braces.expand(path$2) : [path$2];
			expandedPath.forEach((path$3) => {
				parts.push(sysPath.relative(this.watchPath, path$3).split(SLASH_OR_BACK_SLASH_RE));
			});
			return parts;
		}
		filterDir(entry) {
	...
	class FSWatcher extends EventEmitter {
		...
		*/
		add(paths_, _origAdd, _internal) {
			...
			if (cwd) paths = paths.map((path$2) => {
				const absPath = getAbsolutePath(path$2, cwd);
				if (disableGlobbing || !isGlob$1(path$2)) return absPath;
				return normalizePath$1(absPath);
			});
			paths = paths.filter((path$2) => {
				if (path$2.startsWith(BANG)) {
					this._ignoredPaths.add(path$2.slice(1));
					return false;
				}
				this._ignoredPaths.delete(path$2);
				this._ignoredPaths.delete(path$2 + SLASH_GLOBSTAR);
			...
			if (this.options.useFsEvents && this._fsEventsHandler) {
				...
				paths.forEach((path$2) => this._fsEventsHandler._addToFsEvents(path$2));
			} else {
				if (!this._readyCount) this._readyCount = 0;
				this._readyCount += paths.length;
				Promise.all(paths.map(async (path$2) => {
					const res = await this._nodeFsHandler._addToNodeFs(path$2, !_internal, 0, 0, _origAdd);
		...
		*/
		unwatch(paths_) {
			...
			paths.forEach((path$2) => {
				...
				this._ignoredPaths.add(path$2);
				if (this._watched.has(path$2)) this._ignoredPaths.add(path$2 + SLASH_GLOBSTAR);
				this._userIgnored = void 0;
			});
			return this;
		}
		...
		*/
		async _emit(event, path$2, val1, val2, val3) {
			...
			if (opts.atomic) {
				if (event === EV_UNLINK) {
					this._pendingUnlinks.set(path$2, args);
					setTimeout(() => {
				...
				if (event === EV_ADD && this._pendingUnlinks.has(path$2)) {
			...
			if (event === EV_CHANGE) {
				const isThrottled = !this._throttle(EV_CHANGE, path$2, 50);
		...
		_awaitWriteFinish(path$2, threshold, event, awfEmit) {
			...
			const awaitWriteFinish = (prevStat) => {
				fs$1.stat(fullPath, (err, curStat) => {
					if (err || !this._pendingWrites.has(path$2)) {
						if (err && err.code !== "ENOENT") awfEmit(err);
						return;
					}
					const now$1 = Number(new Date());
					if (prevStat && curStat.size !== prevStat.size) this._pendingWrites.get(path$2).lastChange = now$1;
					const pw = this._pendingWrites.get(path$2);
					const df = now$1 - pw.lastChange;
					if (df >= threshold) {
						this._pendingWrites.delete(path$2);
						awfEmit(void 0, curStat);
					} else timeoutHandler = setTimeout(awaitWriteFinish, this.options.awaitWriteFinish.pollInterval, curStat);
				});
			};
			if (!this._pendingWrites.has(path$2)) {
				this._pendingWrites.set(path$2, {
					lastChange: now,
					cancelWait: () => {
						this._pendingWrites.delete(path$2);
		...
		*/
		_isIgnored(path$2, stats) {
			...
			if (!this._userIgnored) {
				...
				const paths = arrify(ignored).filter((path$3) => typeof path$3 === STRING_TYPE && !isGlob$1(path$3)).map((path$3) => path$3 + SLASH_GLOBSTAR);
				const list = this._getGlobIgnored().map(normalizeIgnored(cwd)).concat(ignored, paths);
				this._userIgnored = anymatch$1(list, void 0, ANYMATCH_OPTS);
			}
			return this._userIgnored([path$2, stats]);
		}
		_isntIgnored(path$2, stat$2) {
			return !this._isIgnored(path$2, stat$2);
		}
		/**
		...
		*/
		_getWatchHelpers(path$2, depth) {
			const watchPath = depth || this.options.disableGlobbing || !isGlob$1(path$2) ? path$2 : globParent$1(path$2);
			const follow = this.options.followSymlinks;
			return new WatchHelper(path$2, watchPath, follow, this);
		}
		/**
		...
		*/
		_remove(directory, item, isDirectory) {
			...
			const wp = this._getWatchedDir(path$2);
			const nestedDirectoryChildren = wp.getChildren();
			nestedDirectoryChildren.forEach((nested) => this._remove(path$2, nested));
			...
			if (!this.options.useFsEvents) this._closePath(path$2);
		}
		/**
...
var FileWatcher = class {
	...
	close() {
		this.watcher.close();
		for (const watcher of this.transformWatchers.values()) watcher.close();
	}
	unwatch(id) {
...
var Watcher = class {
	constructor(optionsList, emitter) {
		...
		for (const { watch } of optionsList) if (watch && typeof watch.buildDelay === "number") this.buildDelay = Math.max(this.buildDelay, watch.buildDelay);
	...
	invalidate(file) {
		if (file) {
			...
			if (event === "buggy") this.invalidatedIds.set(file.id, file.event);
			else if (event === null) this.invalidatedIds.delete(file.id);
			else this.invalidatedIds.set(file.id, event);
		}
		if (this.running) {
			this.rerun = true;
			return;
		...
		this.buildTimeout = setTimeout(async () => {
			...
			try {
				...
				await this.emitter.emit("event", { code: "END" });
			}
		}, this.buildDelay);
	}
	async run() {
		this.running = true;
		await this.emitter.emit("event", { code: "START" });
...
var Task = class {
	...
	invalidate(id, details) {
		this.invalidated = true;
		if (details.isTransformDependency) for (const module of this.cache.modules) {
	...
	async run() {
		...
		try {
			...
			if (!this.closed) {
				if (Array.isArray(error.watchFiles)) for (const id of error.watchFiles) this.watchFile(id);
				if (error.id) this.cache.modules = this.cache.modules.filter((module) => module.id !== error.id);
			}
			await this.watcher.emitter.emit("event", {
				code: "ERROR",
				error,
	...
	updateWatchedFiles(result) {
		...
		for (const id of this.watchFiles) this.watchFile(id);
		for (const module of this.cache.modules) for (const depId of module.transformDependencies) this.watchFile(depId, true);
		for (const id of previouslyWatched) if (!this.watched.has(id)) this.fileWatcher.unwatch(id);
	}
	watchFile(id, isTransformDependency = false) {
		if (!this.filter(id)) return;
		this.watched.add(id);
		if (this.outputFiles.includes(id)) throw new Error("Cannot import the generated bundle");
