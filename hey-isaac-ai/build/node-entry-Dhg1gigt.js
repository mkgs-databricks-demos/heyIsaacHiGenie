...
let Chunk$1 = class Chunk$2 {
	...
	clone() {
		const chunk = new Chunk$2(this.start, this.end, this.original);
	...
	split(index) {
		...
		const newChunk = new Chunk$2(index, this.end, originalAfter);
...
function getLocator(source) {
	...
	return function locate$1(index) {
		let i = 0;
...
var MagicString = class MagicString {
	...
	generateDecodedMap(options) {
		...
		const locate$1 = getLocator(this.original);
		if (this.intro) mappings.advance(this.intro);
		this.firstChunk.eachNext((chunk) => {
			const loc = locate$1(chunk.start);
...
let Bundle$1 = class Bundle$2 {
	...
	generateDecodedMap(options = {}) {
		...
		this.sources.forEach((source, i) => {
			...
			const locate$1 = getLocator(magicString.original);
...
function findFirstOccurrenceOutsideComment(code, searchString, start = 0) {
	...
	while (true) {
		...
		if (start > searchPos) searchPos = code.indexOf(searchString, start);
	}
}
...
function findLastWhiteSpaceReverse(code, start, end) {
	while (true) {
		...
		if (WHITESPACE.test(code[end - 1])) end--;
		else return end;
	}
}
function findFirstLineBreakOutsideComment(code) {
	...
	while (true) {
		...
		if (start > lineBreakPos) lineBreakPos = code.indexOf("\n", start);
	}
}
function renderStatementList(statements, code, start, end, options) {
	...
	if (nextNodeNeedsBoundaries) nextNodeStart = start + findFirstLineBreakOutsideComment(code.original.slice(start, nextNode.start))[1];
	for (let nextIndex = 1; nextIndex <= statements.length; nextIndex++) {
		...
		if (currentNodeNeedsBoundaries || nextNodeNeedsBoundaries) {
			...
			if (currentNode.included) if (currentNodeNeedsBoundaries) currentNode.render(code, options, {
				end: nextNodeStart,
				start: currentNodeStart
			});
			else currentNode.render(code, options);
...
function getCommaSeparatedNodesWithBoundaries(nodes, code, start, end) {
	...
	for (const nextNode of nodes) {
		if (node !== void 0) separator = node.end + findFirstOccurrenceOutsideComment(code.original.slice(node.end, nextNode.start), ",");
		nextNodeStart = contentEnd = separator + 1 + findFirstLineBreakOutsideComment(code.original.slice(separator + 1, nextNode.start))[1];
		while (char = code.original.charCodeAt(nextNodeStart), char === 32 || char === 9 || char === 10 || char === 13) nextNodeStart++;
		if (node !== void 0) splitUpNodes.push({
			contentEnd,
			end: nextNodeStart,
			node,
			separator,
...
function getSystemExportStatement(exportedVariables, { exportNamesByVariable, snippets: { _, getObject, getPropertyAccess } }, modifier = "") {
	if (exportedVariables.length === 1 && exportNamesByVariable.get(exportedVariables[0]).length === 1) {
		...
		const fields = [];
		for (const variable of exportedVariables) for (const exportName of exportNamesByVariable.get(variable)) fields.push([exportName, variable.getName(getPropertyAccess) + modifier]);
...
const TREE_SHAKEABLE_SYMBOLS_LIST = [
	SymbolHasInstance,
	SymbolDispose,
	SymbolAsyncDispose
];
...
var EntityPathTracker = class {
	...
	trackEntityAtPathAndGetIfTracked(path$2, entity) {
		const trackedEntities = this.getEntities(path$2);
		if (trackedEntities.has(entity)) return true;
		trackedEntities.add(entity);
		return false;
	}
	withTrackedEntityAtPath(path$2, entity, onUntracked, returnIfTracked) {
		const trackedEntities = this.getEntities(path$2);
	...
	getEntities(path$2) {
		...
		for (const pathSegment of path$2) currentPaths = currentPaths[pathSegment] ||= Object.create(null, { [EntitiesKey]: { value: new Set() } });
...
var DiscriminatedPathTracker = class {
	constructor() {
		this.entityPaths = Object.create(null, { [EntitiesKey]: { value: new Map() } });
	}
	trackEntityAtPathAndGetIfTracked(path$2, discriminator, entity) {
		let currentPaths = this.entityPaths;
		for (const pathSegment of path$2) currentPaths = currentPaths[pathSegment] ||= Object.create(null, { [EntitiesKey]: { value: new Map() } });
...
var IncludedFullPathTracker = class {
	...
	includePathAndGetIfIncluded(path$2) {
		let included = true;
		let parent = this;
		let parentSegment = "includedPaths";
		let currentPaths = this.includedPaths ||= (included = false, Object.create(null));
		for (const pathSegment of path$2) {
			if (currentPaths[UnknownKey]) return true;
...
const UNKNOWN_INCLUDED_TOP_LEVEL_PATH = Object.freeze({ [UnknownKey]: true });
var IncludedTopLevelPathTracker = class {
	...
	includePathAndGetIfIncluded(path$2) {
		let included = true;
		const includedPaths = this.includedPaths ||= (included = false, Object.create(null));
		if (includedPaths[UnknownKey]) return true;
		const [firstPathSegment, secondPathSegment] = path$2;
		if (!firstPathSegment) return included;
		if (!isConcreteKey(firstPathSegment)) {
			this.includedPaths = UNKNOWN_INCLUDED_TOP_LEVEL_PATH;
			return false;
		}
		if (secondPathSegment) {
			if (includedPaths[firstPathSegment] === UnknownKey) return true;
			includedPaths[firstPathSegment] = UnknownKey;
			return false;
		}
		if (includedPaths[firstPathSegment]) return true;
		includedPaths[firstPathSegment] = true;
		return false;
	}
	includeAllPaths(entity, context, basePath) {
		const { includedPaths } = this;
		if (includedPaths) if (includedPaths[UnknownKey]) entity.includePath([...basePath, UnknownKey], context);
		else {
			const inclusionEntries = Object.entries(includedPaths);
			if (inclusionEntries.length === 0) entity.includePath(basePath, context);
			else for (const [key, value] of inclusionEntries) entity.includePath(value === UnknownKey ? [
				...basePath,
				key,
				UnknownKey
			] : [...basePath, key], context);
		}
	}
};
...
const getPureFunctions = ({ treeshake }) => {
	...
	for (const functionName of treeshake ? treeshake.manualPureFunctions : []) {
		...
		for (const pathSegment of functionName.split(".")) currentFunctions = currentFunctions[pathSegment] ||= Object.create(null);
...
var Variable = class extends ExpressionEntity {
	...
	*/
	addUsedPlace(usedPlace) {
		...
		if (!isFunctionCall && usedPlace.parent.type !== ExportDefaultDeclaration) this.onlyFunctionCallUsed = false;
	}
	/**
	...
	*/
	...
	getName(getPropertyAccess, useOriginalName) {
		...
		if (useOriginalName?.(this)) return this.name;
		if (this.renderedLikeHoisted) return this.renderedLikeHoisted.getName(getPropertyAccess, useOriginalName);
		const name = this.renderName || this.name;
		return this.renderBaseName ? `${this.renderBaseName}${getPropertyAccess(name)}` : name;
	}
	hasEffectsOnInteractionAtPath(path$2, { type }, _context) {
		return type !== INTERACTION_ACCESSED || path$2.length > 0;
	}
	/**
	...
	*/
	includePath(path$2, context) {
		this.included = true;
		this.renderedLikeHoisted?.includePath(path$2, context);
	}
	/**
...
var ExternalVariable = class extends Variable {
	...
	addReference(identifier) {
		...
		if (this.name === "default" || this.name === "*") this.module.suggestName(identifier.name);
	}
	hasEffectsOnInteractionAtPath(path$2, { type }) {
		return type !== INTERACTION_ACCESSED || path$2.length > (this.isNamespace ? 1 : 0);
	}
	includePath(path$2, context) {
		super.includePath(path$2, context);
...
function isLegal(value) {
	if (needsEscape(value)) return false;
...
function stringifyObjectKeyIfNeeded(key) {
	if (VALID_IDENTIFIER_REGEXP.test(key)) return key === "__proto__" ? "[\"__proto__\"]" : key;
	if (NUMBER_REGEXP.test(key) && +key <= Number.MAX_SAFE_INTEGER) return key;
	return JSON.stringify(key);
}
function stringifyIdentifierIfNeeded(key) {
	if (VALID_IDENTIFIER_REGEXP.test(key)) return key;
	return JSON.stringify(key);
}
var ExternalModule = class {
	...
	getVariableForExportName(name, { importChain }) {
		...
		for (const module of importChain) getOrCreate(this.importersByExportedName, name, getNewSet).add(module);
...
function markModuleAndImpureDependenciesAsExecuted(baseModule) {
	...
	for (const module of modules) for (const dependency of [...module.dependencies, ...module.implicitlyLoadedBefore]) if (!(dependency instanceof ExternalModule) && !dependency.isExecuted && (dependency.info.moduleSideEffects || module.implicitlyLoadedBefore.has(dependency)) && !visitedModules.has(dependency.id)) {
...
var NodeBase = class extends ExpressionEntity {
	...
	*/
	bind() {
		for (const key of childNodeKeys[this.type]) {
			...
			if (Array.isArray(value)) for (const child of value) child?.bind();
	...
	*/
	...
	hasEffects(context) {
		...
		for (const key of childNodeKeys[this.type]) {
			...
			if (Array.isArray(value)) {
				for (const child of value) if (child?.hasEffects(context)) return true;
	...
	include(context, includeChildrenRecursively, _options) {
		...
		for (const key of childNodeKeys[this.type]) {
			const value = this[key];
			if (value === null) continue;
			if (Array.isArray(value)) for (const child of value) child?.include(context, includeChildrenRecursively);
			else value.include(context, includeChildrenRecursively);
		}
	}
	includeNode(context) {
		this.included = true;
		...
		for (const key of childNodeKeys[this.type]) {
			const value = this[key];
			if (value === null) continue;
			if (Array.isArray(value)) for (const child of value) child?.includePath(UNKNOWN_PATH, context);
	...
	*/
	...
	parseNode(esTreeNode) {
		for (const [key, value] of Object.entries(esTreeNode)) {
			...
			if (key.charCodeAt(0) === 95) {
				if (key === ANNOTATION_KEY) this.annotations = value;
				else if (key === INVALID_ANNOTATION_KEY) this.invalidAnnotations = value;
			} else if (typeof value !== "object" || value === null) this[key] = value;
			else if (Array.isArray(value)) {
				this[key] = new Array(value.length);
				let index = 0;
				for (const child of value) this[key][index++] = child === null ? null : new (this.scope.context.getNodeConstructor(child.type))(this, this.scope).parseNode(child);
			} else this[key] = new (this.scope.context.getNodeConstructor(value.type))(this, this.scope).parseNode(value);
		}
	...
	removeAnnotations(code) {
		if (this.annotations) for (const annotation of this.annotations) code.remove(annotation.start, annotation.end);
	}
	render(code, options) {
		for (const key of childNodeKeys[this.type]) {
			const value = this[key];
			if (value === null) continue;
			if (Array.isArray(value)) for (const child of value) child?.render(code, options);
			else value.render(code, options);
		}
	}
	...
	*/
	applyDeoptimizations() {
		...
		for (const key of childNodeKeys[this.type]) {
			const value = this[key];
			if (value === null) continue;
			if (Array.isArray(value)) for (const child of value) child?.deoptimizePath(UNKNOWN_PATH);
			else value.deoptimizePath(UNKNOWN_PATH);
		}
...
const UNDEFINED_EXPRESSION = new class UndefinedExpression extends ExpressionEntity {
	getLiteralValueAtPath(path$2) {
		return path$2.length > 0 ? UnknownValue : void 0;
	}
}();
const returnsUnknown = { value: {
	hasEffectsWhenCalled: null,
	returns: UNKNOWN_EXPRESSION
} };
const UNKNOWN_LITERAL_BOOLEAN = new class UnknownBoolean extends ExpressionEntity {
	getReturnExpressionWhenCalledAtPath(path$2) {
		if (path$2.length === 1) return getMemberReturnExpressionWhenCalled(literalBooleanMembers, path$2[0]);
		return UNKNOWN_RETURN_EXPRESSION;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (interaction.type === INTERACTION_ACCESSED) return path$2.length > 1;
		if (interaction.type === INTERACTION_CALLED && path$2.length === 1) return hasMemberEffectWhenCalled(literalBooleanMembers, path$2[0], interaction, context);
		return true;
	}
}();
const returnsBoolean = { value: {
	hasEffectsWhenCalled: null,
	returns: UNKNOWN_LITERAL_BOOLEAN
} };
const UNKNOWN_LITERAL_NUMBER = new class UnknownNumber extends ExpressionEntity {
	getReturnExpressionWhenCalledAtPath(path$2) {
		if (path$2.length === 1) return getMemberReturnExpressionWhenCalled(literalNumberMembers, path$2[0]);
		return UNKNOWN_RETURN_EXPRESSION;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (interaction.type === INTERACTION_ACCESSED) return path$2.length > 1;
		if (interaction.type === INTERACTION_CALLED && path$2.length === 1) return hasMemberEffectWhenCalled(literalNumberMembers, path$2[0], interaction, context);
		return true;
	}
}();
const returnsNumber = { value: {
	hasEffectsWhenCalled: null,
	returns: UNKNOWN_LITERAL_NUMBER
} };
const UNKNOWN_LITERAL_STRING = new class UnknownString extends ExpressionEntity {
	getReturnExpressionWhenCalledAtPath(path$2) {
		if (path$2.length === 1) return getMemberReturnExpressionWhenCalled(literalStringMembers, path$2[0]);
		return UNKNOWN_RETURN_EXPRESSION;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (interaction.type === INTERACTION_ACCESSED) return path$2.length > 1;
		if (interaction.type === INTERACTION_CALLED && path$2.length === 1) return hasMemberEffectWhenCalled(literalStringMembers, path$2[0], interaction, context);
		return true;
	}
}();
const returnsString = { value: {
	hasEffectsWhenCalled: null,
...
const stringReplace = { value: {
	hasEffectsWhenCalled({ args }, context) {
		...
		return args.length < 3 || typeof argument1.getLiteralValueAtPath(EMPTY_PATH, SHARED_RECURSION_TRACKER, { deoptimizeCache() {} }) === "symbol" && argument1.hasEffectsOnInteractionAtPath(EMPTY_PATH, NODE_INTERACTION_UNKNOWN_CALL, context);
	},
	returns: UNKNOWN_LITERAL_STRING
} };
...
const literalBooleanMembers = assembleMemberDescriptions({ valueOf: returnsBoolean }, objectMembers);
...
function getLiteralMembersForValue(value) {
	if (value instanceof RegExp) return literalRegExpMembers;
	switch (typeof value) {
		case "boolean": return literalBooleanMembers;
		case "number": return literalNumberMembers;
		case "string": return literalStringMembers;
	}
...
var Method = class extends ExpressionEntity {
	...
	deoptimizeArgumentsOnInteractionAtPath({ args, type }, path$2) {
		if (type === INTERACTION_CALLED && path$2.length === 0) {
			if (this.description.mutatesSelfAsArray) args[0]?.deoptimizePath(UNKNOWN_INTEGER_PATH);
			if (this.description.mutatesArgs) for (let index = 1; index < args.length; index++) args[index].deoptimizePath(UNKNOWN_PATH);
		}
	}
	getReturnExpressionWhenCalledAtPath(path$2, { args }) {
		if (path$2.length > 0) return UNKNOWN_RETURN_EXPRESSION;
		return [this.description.returnsPrimitive || (this.description.returns === "self" ? args[0] || UNKNOWN_EXPRESSION : this.description.returns()), false];
	}
	hasEffectsOnInteractionAtPath(path$2, { args, type }, context) {
		if (path$2.length > (type === INTERACTION_ACCESSED ? 1 : 0)) return true;
		if (type === INTERACTION_CALLED) {
			if (this.description.mutatesSelfAsArray === true && args[0]?.hasEffectsOnInteractionAtPath(UNKNOWN_INTEGER_PATH, NODE_INTERACTION_UNKNOWN_ASSIGNMENT, context)) return true;
			if (this.description.callsArgs) {
				for (const argumentIndex of this.description.callsArgs) if (args[argumentIndex + 1]?.hasEffectsOnInteractionAtPath(EMPTY_PATH, NODE_INTERACTION_UNKNOWN_CALL, context)) return true;
			}
		}
		return false;
	}
...
var ObjectEntity = class extends ExpressionEntity {
	...
	constructor(properties, prototypeExpression, immutable = false) {
		...
		if (Array.isArray(properties)) this.buildPropertyMaps(properties);
		else {
			this.propertiesAndGettersByKey = this.propertiesAndSettersByKey = properties;
			for (const propertiesForKey of properties.values()) this.allProperties.push(...propertiesForKey);
		}
	}
	deoptimizeAllProperties(noAccessors) {
		const isDeoptimized = this.hasLostTrack || this.hasUnknownDeoptimizedProperty;
		if (noAccessors) this.hasUnknownDeoptimizedProperty = true;
		else this.hasLostTrack = true;
		if (isDeoptimized) return;
		for (const properties of [...this.propertiesAndGettersByKey.values(), ...this.settersByKey.values()]) for (const property of properties) property.deoptimizePath(UNKNOWN_PATH);
		this.prototypeExpression?.deoptimizePath([UnknownKey, UnknownKey]);
		this.deoptimizeCachedEntities();
	}
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		const [key, ...subPath] = path$2;
		const { args, type } = interaction;
		if (this.hasLostTrack || (type === INTERACTION_CALLED || path$2.length > 1) && (this.hasUnknownDeoptimizedProperty || isConcreteKey(key) && this.deoptimizedPaths.get(key))) {
			deoptimizeInteraction(interaction);
			return;
		}
		const [propertiesForExactMatchByKey, relevantPropertiesByKey, relevantUnmatchableProperties] = type === INTERACTION_CALLED || path$2.length > 1 ? [
		...
		if (isConcreteKey(key)) {
			if (propertiesForExactMatchByKey.get(key)) {
				...
				if (properties) for (const property of properties) property.deoptimizeArgumentsOnInteractionAtPath(interaction, subPath, recursionTracker);
				if (!this.immutable) {
					for (const argument of args) if (argument) this.additionalExpressionsToBeDeoptimized.add(argument);
				}
				return;
			}
			for (const property of relevantUnmatchableProperties) property.deoptimizeArgumentsOnInteractionAtPath(interaction, subPath, recursionTracker);
			if (typeof key === "string" && INTEGER_REG_EXP.test(key)) for (const property of this.unknownIntegerProps) property.deoptimizeArgumentsOnInteractionAtPath(interaction, subPath, recursionTracker);
		} else {
			for (const properties of [...relevantPropertiesByKey.values(), relevantUnmatchableProperties]) for (const property of properties) property.deoptimizeArgumentsOnInteractionAtPath(interaction, subPath, recursionTracker);
			for (const property of this.unknownIntegerProps) property.deoptimizeArgumentsOnInteractionAtPath(interaction, subPath, recursionTracker);
		}
		if (!this.immutable) {
			for (const argument of args) if (argument) this.additionalExpressionsToBeDeoptimized.add(argument);
		}
		this.prototypeExpression?.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizeIntegerProperties() {
		if (this.hasLostTrack || this.hasUnknownDeoptimizedProperty || this.hasUnknownDeoptimizedInteger) return;
		this.hasUnknownDeoptimizedInteger = true;
		for (const [key, propertiesAndGetters] of this.propertiesAndGettersByKey.entries()) if (typeof key === "string" && INTEGER_REG_EXP.test(key)) for (const property of propertiesAndGetters) property.deoptimizePath(UNKNOWN_PATH);
		this.deoptimizeCachedIntegerEntities();
	}
	deoptimizePath(path$2) {
		if (this.hasLostTrack || this.immutable) return;
		const key = path$2[0];
		if (path$2.length === 1) {
			if (key === UnknownInteger) return this.deoptimizeIntegerProperties();
			else if (!isConcreteKey(key)) return this.deoptimizeAllProperties(key === UnknownNonAccessorKey);
			if (!this.deoptimizedPaths.get(key)) {
				...
				if (expressionsToBeDeoptimized) for (const expression of expressionsToBeDeoptimized) expression.deoptimizeCache();
			}
		}
		const subPath = path$2.length === 1 ? UNKNOWN_PATH : path$2.slice(1);
		for (const property of isConcreteKey(key) ? [...this.propertiesAndGettersByKey.get(key) || this.unmatchablePropertiesAndGetters, ...this.settersByKey.get(key) || this.unmatchableSetters] : this.allProperties) property.deoptimizePath(subPath);
		this.prototypeExpression?.deoptimizePath(path$2.length === 1 ? [path$2[0], UnknownKey] : path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		if (path$2.length === 0) return UnknownValue;
		const key = path$2[0];
		const expressionAtPath = this.getMemberExpressionAndTrackDeopt(key, origin);
		if (expressionAtPath) return expressionAtPath.getLiteralValueAtPath(path$2.slice(1), recursionTracker, origin);
		if (this.prototypeExpression) return this.prototypeExpression.getLiteralValueAtPath(path$2, recursionTracker, origin);
		if (path$2.length === 1) return void 0;
		return UnknownValue;
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		if (path$2.length === 0) return UNKNOWN_RETURN_EXPRESSION;
		const [key, ...subPath] = path$2;
		const expressionAtPath = this.getMemberExpressionAndTrackDeopt(key, origin);
		if (expressionAtPath) return expressionAtPath.getReturnExpressionWhenCalledAtPath(subPath, interaction, recursionTracker, origin);
		if (this.prototypeExpression) return this.prototypeExpression.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
		return UNKNOWN_RETURN_EXPRESSION;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		const [key, ...subPath] = path$2;
		if (subPath.length > 0 || interaction.type === INTERACTION_CALLED) {
			...
			if (expressionAtPath) return expressionAtPath.hasEffectsOnInteractionAtPath(subPath, interaction, context);
			if (this.prototypeExpression) return this.prototypeExpression.hasEffectsOnInteractionAtPath(path$2, interaction, context);
			return true;
		}
		...
		if (isConcreteKey(key)) {
			if (propertiesAndAccessorsByKey.get(key)) {
				...
				if (accessors) {
					for (const accessor of accessors) if (accessor.hasEffectsOnInteractionAtPath(subPath, interaction, context)) return true;
				}
				return false;
			}
			for (const accessor of unmatchableAccessors) if (accessor.hasEffectsOnInteractionAtPath(subPath, interaction, context)) return true;
		} else for (const accessors of [...accessorsByKey.values(), unmatchableAccessors]) for (const accessor of accessors) if (accessor.hasEffectsOnInteractionAtPath(subPath, interaction, context)) return true;
		if (this.prototypeExpression) return this.prototypeExpression.hasEffectsOnInteractionAtPath(path$2, interaction, context);
		return false;
	}
	include(context, includeChildrenRecursively) {
		this.included = true;
		for (const property of this.allProperties) if (includeChildrenRecursively || property.shouldBeIncluded(context) || this.alwaysIncludedProperties.has(property)) property.include(context, includeChildrenRecursively);
		this.prototypeExpression?.include(context, includeChildrenRecursively);
	}
	includePath(path$2, context) {
		this.included = true;
		for (const property of this.alwaysIncludedProperties) property.includePath(UNKNOWN_PATH, context);
		if (path$2.length === 0) return;
		const [key, ...subPath] = path$2;
		const [includedMembers, includedPath] = isConcreteKey(key) ? [new Set([...this.propertiesAndGettersByKey.get(key) || this.unmatchablePropertiesAndGetters, ...this.propertiesAndSettersByKey.get(key) || this.unmatchablePropertiesAndSetters]), subPath] : [this.allProperties, UNKNOWN_PATH];
		for (const property of includedMembers) property.includePath(includedPath, context);
		this.prototypeExpression?.includePath(path$2, context);
	}
	buildPropertyMaps(properties) {
		...
		for (let index = properties.length - 1; index >= 0; index--) {
			...
			if (isConcreteKey(key)) if (kind === "set") {
				...
				if (!propertiesAndSettersByKey.has(key)) propertiesAndSettersByKey.set(key, [property, ...unmatchablePropertiesAndSetters]);
				if (!propertiesAndGettersByKey.has(key)) propertiesAndGettersByKey.set(key, [property, ...unmatchablePropertiesAndGetters]);
			}
			else {
	...
	deoptimizeCachedEntities() {
		for (const expressionsToBeDeoptimized of this.expressionsToBeDeoptimizedByKey.values()) for (const expression of expressionsToBeDeoptimized) expression.deoptimizeCache();
		for (const expression of this.additionalExpressionsToBeDeoptimized) expression.deoptimizePath(UNKNOWN_PATH);
	}
	deoptimizeCachedIntegerEntities() {
		for (const [key, expressionsToBeDeoptimized] of this.expressionsToBeDeoptimizedByKey.entries()) if (typeof key === "string" && INTEGER_REG_EXP.test(key)) for (const expression of expressionsToBeDeoptimized) expression.deoptimizeCache();
		for (const expression of this.additionalExpressionsToBeDeoptimized) expression.deoptimizePath(UNKNOWN_INTEGER_PATH);
	}
	getMemberExpression(key) {
		if (this.hasLostTrack || this.hasUnknownDeoptimizedProperty || !isConcreteKey(key) || this.hasUnknownDeoptimizedInteger && typeof key === "string" && INTEGER_REG_EXP.test(key) || this.deoptimizedPaths.get(key)) return UNKNOWN_EXPRESSION;
		const properties = this.propertiesAndGettersByKey.get(key);
		if (properties?.length === 1) return properties[0];
		if (properties || this.unmatchablePropertiesAndGetters.length > 0 || this.unknownIntegerProps.length > 0 && typeof key === "string" && INTEGER_REG_EXP.test(key)) return UNKNOWN_EXPRESSION;
		return null;
	}
	getMemberExpressionAndTrackDeopt(key, origin) {
		if (!isConcreteKey(key)) return UNKNOWN_EXPRESSION;
		const expression = this.getMemberExpression(key);
		if (!(expression === UNKNOWN_EXPRESSION || this.immutable)) {
			let expressionsToBeDeoptimized = this.expressionsToBeDeoptimizedByKey.get(key);
			if (!expressionsToBeDeoptimized) this.expressionsToBeDeoptimizedByKey.set(key, expressionsToBeDeoptimized = []);
...
const OBJECT_PROTOTYPE_FALLBACK = new class ObjectPrototypeFallbackExpression extends ExpressionEntity {
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2) {
		if (interaction.type === INTERACTION_CALLED && path$2.length === 1 && !isInteger(path$2[0])) deoptimizeInteraction(interaction);
	}
	getLiteralValueAtPath(path$2) {
		return path$2.length === 1 && isInteger(path$2[0]) ? void 0 : UnknownValue;
	}
	hasEffectsOnInteractionAtPath(path$2, { type }) {
		return path$2.length > 1 || type === INTERACTION_CALLED;
	}
}();
...
const METHOD_MUTATES_SELF_AND_ARGS_RETURNS_NUMBER = [new Method({
	callsArgs: null,
	mutatesArgs: true,
	mutatesSelfAsArray: true,
	returns: null,
...
var SpreadElement = class extends NodeBase {
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		if (path$2.length > 0) this.argument.deoptimizeArgumentsOnInteractionAtPath(interaction, UNKNOWN_PATH, recursionTracker);
	}
	hasEffects(context) {
		if (!this.deoptimized) this.applyDeoptimizations();
...
var ArrayExpression = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.getObjectEntity().deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizePath(path$2) {
		this.getObjectEntity().deoptimizePath(path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		return this.getObjectEntity().getLiteralValueAtPath(path$2, recursionTracker, origin);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		return this.getObjectEntity().getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return this.getObjectEntity().hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	includeNode(context) {
		this.included = true;
		if (!this.deoptimized) this.applyDeoptimizations();
		for (const element of this.elements) if (element) element?.includePath(UNKNOWN_PATH, context);
	}
	applyDeoptimizations() {
		this.deoptimized = true;
		let hasSpread = false;
	...
	getObjectEntity() {
		...
		for (let index = 0; index < this.elements.length; index++) {
			...
			} else if (element) properties.push({
				key: String(index),
				kind: "init",
				property: element
			});
		...
		[ValueProperties]: {
			deoptimizeArgumentsOnCall: ({ args: [, target, parameter] }) => {
				if (isObjectExpressionNode(parameter)) {
					...
					if (!hasSpreadElement) {
						for (const property of parameter.properties) property.deoptimizeArgumentsOnInteractionAtPath({
							args: [null, target],
							type: INTERACTION_CALLED,
							withNew: false
...
function getGlobalAtPath(path$2) {
	let currentGlobal = knownGlobals;
	for (const pathSegment of path$2) {
		if (typeof pathSegment !== "string") return null;
		currentGlobal = currentGlobal[pathSegment];
		if (!currentGlobal) return path$2[0] === "Symbol" && path$2.length === 2 ? UNKNOWN_WELL_KNOWN : null;
	}
...
var GlobalVariable = class extends Variable {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		switch (interaction.type) {
			...
			case INTERACTION_ASSIGNED: {
				if (!getGlobalAtPath([this.name, ...path$2].slice(0, -1))) super.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
				return;
			}
			case INTERACTION_CALLED: {
				const globalAtPath = getGlobalAtPath([this.name, ...path$2]);
				if (globalAtPath) globalAtPath.deoptimizeArgumentsOnCall(interaction);
				else super.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
				return;
			}
		}
	}
	getLiteralValueAtPath(path$2, _recursionTracker, _origin) {
		const globalAtPath = getGlobalAtPath([this.name, ...path$2]);
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		switch (interaction.type) {
			case INTERACTION_ACCESSED: {
				if (path$2.length === 0) return this.name !== "undefined" && !getGlobalAtPath([this.name]);
				return !getGlobalAtPath([this.name, ...path$2].slice(0, -1));
			}
			case INTERACTION_ASSIGNED: return true;
			case INTERACTION_CALLED: {
				const globalAtPath = getGlobalAtPath([this.name, ...path$2]);
...
const limitConcatenatedPathDepth = (path1, path2) => {
	...
	return length1 === 0 ? path2 : length2 === 0 ? path1 : length1 + length2 > MAX_PATH_DEPTH ? [
		...path1,
		...path2.slice(0, MAX_PATH_DEPTH - 1 - path1.length),
		"UnknownKey"
	] : [...path1, ...path2];
};
var LocalVariable = class extends Variable {
	...
	consolidateInitializers() {
		if (this.additionalInitializers) for (const initializer of this.additionalInitializers) initializer.deoptimizePath(UNKNOWN_PATH);
	}
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		if (this.isReassigned || path$2.length + this.initPath.length > MAX_PATH_DEPTH) {
			deoptimizeInteraction(interaction);
			return;
		}
		recursionTracker.withTrackedEntityAtPath(path$2, this.init, () => {
			this.init.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.initPath, ...path$2], recursionTracker);
		}, void 0);
	}
	deoptimizePath(path$2) {
		if (this.isReassigned || this.deoptimizationTracker.trackEntityAtPathAndGetIfTracked(path$2, this)) return;
		if (path$2.length === 0) {
			this.markReassigned();
			const expressionsToBeDeoptimized = this.expressionsToBeDeoptimized;
			this.expressionsToBeDeoptimized = EMPTY_ARRAY;
			for (const expression of expressionsToBeDeoptimized) expression.deoptimizeCache();
			this.init.deoptimizePath([...this.initPath, UnknownKey]);
		} else this.init.deoptimizePath(limitConcatenatedPathDepth(this.initPath, path$2));
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		if (this.isReassigned || path$2.length + this.initPath.length > MAX_PATH_DEPTH) return UnknownValue;
		return recursionTracker.withTrackedEntityAtPath(path$2, this.init, () => {
			this.expressionsToBeDeoptimized.push(origin);
			return this.init.getLiteralValueAtPath([...this.initPath, ...path$2], recursionTracker, origin);
		}, UnknownValue);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		if (this.isReassigned || path$2.length + this.initPath.length > MAX_PATH_DEPTH) return UNKNOWN_RETURN_EXPRESSION;
		return recursionTracker.withTrackedEntityAtPath(path$2, this.init, () => {
			this.expressionsToBeDeoptimized.push(origin);
			return this.init.getReturnExpressionWhenCalledAtPath([...this.initPath, ...path$2], interaction, recursionTracker, origin);
		}, UNKNOWN_RETURN_EXPRESSION);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (path$2.length + this.initPath.length > MAX_PATH_DEPTH) return true;
		switch (interaction.type) {
			case INTERACTION_ACCESSED: {
				if (this.isReassigned) return true;
				return !context.accessed.trackEntityAtPathAndGetIfTracked(path$2, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path$2], interaction, context);
			}
			case INTERACTION_ASSIGNED: {
				if (this.included) return true;
				if (path$2.length === 0) return false;
				if (this.isReassigned) return true;
				return !context.assigned.trackEntityAtPathAndGetIfTracked(path$2, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path$2], interaction, context);
			}
			case INTERACTION_CALLED: {
				if (this.isReassigned) return true;
				return !(interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path$2, interaction.args, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path$2], interaction, context);
			}
		}
	}
	includePath(path$2, context) {
		if (!this.includedPathTracker.includePathAndGetIfIncluded(path$2)) {
			this.module.scope.context.requestTreeshakingPass();
			if (!this.included) this.module.scope.context.newlyIncludedVariableInits.add(this.init);
			super.includePath(path$2, context);
			...
			if (path$2.length > 0) {
				this.init.includePath(limitConcatenatedPathDepth(this.initPath, path$2), context);
				this.additionalInitializers?.forEach((initializer) => initializer.includePath(UNKNOWN_PATH, context));
			}
		}
	}
	includeCallArguments(interaction, context) {
		if (this.isReassigned || context.includedCallArguments.has(this.init) || this.initPath.length > 0) includeInteraction(interaction, context);
		else {
...
const tdzVariableKinds = new Set([
	"class",
	"const",
	"let",
	"var",
...
var IdentifierBase = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.variable.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizePath(path$2) {
		if (path$2.length === 0 && !this.scope.contains(this.name)) this.disallowImportReassignment();
		this.variable?.deoptimizePath(path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		return this.getVariableRespectingTDZ().getLiteralValueAtPath(path$2, recursionTracker, origin);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		const [expression, isPure] = this.getVariableRespectingTDZ().getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
		return [expression, isPure || this.isPureFunction(path$2)];
	}
	hasEffects(context) {
		if (!this.deoptimized) this.applyDeoptimizations();
		if (this.isPossibleTDZ() && this.variable.kind !== "var") return true;
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		switch (interaction.type) {
			case INTERACTION_ACCESSED: return this.variable !== null && !this.isPureFunction(path$2) && this.getVariableRespectingTDZ().hasEffectsOnInteractionAtPath(path$2, interaction, context);
			case INTERACTION_ASSIGNED: return (path$2.length > 0 ? this.getVariableRespectingTDZ() : this.variable).hasEffectsOnInteractionAtPath(path$2, interaction, context);
			case INTERACTION_CALLED: return !this.isPureFunction(path$2) && this.getVariableRespectingTDZ().hasEffectsOnInteractionAtPath(path$2, interaction, context);
		}
	}
	include(context, includeChildrenRecursively) {
		if (!this.included) this.includeNode(context);
		if (includeChildrenRecursively) this.variable?.includePath(UNKNOWN_PATH, context);
	}
	includeNode(context) {
		this.included = true;
		if (!this.deoptimized) this.applyDeoptimizations();
		if (this.variable !== null) this.scope.context.includeVariableInModule(this.variable, EMPTY_PATH, context);
	}
	includePath(path$2, context) {
		if (!this.included) {
			this.included = true;
			if (!this.deoptimized) this.applyDeoptimizations();
			if (this.variable !== null) this.scope.context.includeVariableInModule(this.variable, path$2, context);
		} else if (path$2.length > 0) this.variable?.includePath(path$2, context);
	}
	includeCallArguments(interaction, context) {
	...
	isPossibleTDZ() {
		...
		if (!(this.variable instanceof LocalVariable && this.variable.kind && tdzVariableKinds.has(this.variable.kind) && this.variable.module.hasTreeShakingPassStarted)) return this.isTDZAccess = false;
		let decl_id;
		if (this.variable.declarations && this.variable.declarations.length === 1 && (decl_id = this.variable.declarations[0]) && this.start < decl_id.start && closestParentFunctionOrProgram(this) === closestParentFunctionOrProgram(decl_id)) return this.isTDZAccess = true;
		if (!this.variable.initReached) return this.isTDZAccess = true;
		return this.isTDZAccess = false;
	}
	applyDeoptimizations() {
		this.deoptimized = true;
		if (this.variable instanceof LocalVariable) {
			if (!this.variable.module.isExecuted) markModuleAndImpureDependenciesAsExecuted(this.variable.module);
	...
	getVariableRespectingTDZ() {
		if (this.isPossibleTDZ()) return UNKNOWN_EXPRESSION;
		return this.variable;
	}
	isPureFunction(path$2) {
		let currentPureFunction = this.scope.context.manualPureFunctions[this.name];
		for (const segment of path$2) if (currentPureFunction) {
			if (currentPureFunction[PureFunctionKey]) return true;
...
function closestParentFunctionOrProgram(node) {
	while (node && !/^Program|Function/.test(node.type)) node = node.parent;
	return node;
}
var ObjectMember = class extends ExpressionEntity {
	constructor(object, path$2) {
		super();
		this.object = object;
		this.path = path$2;
	}
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.object.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.path, ...path$2], recursionTracker);
	}
	deoptimizePath(path$2) {
		this.object.deoptimizePath([...this.path, ...path$2]);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		return this.object.getLiteralValueAtPath([...this.path, ...path$2], recursionTracker, origin);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		return this.object.getReturnExpressionWhenCalledAtPath([...this.path, ...path$2], interaction, recursionTracker, origin);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return this.object.hasEffectsOnInteractionAtPath([...this.path, ...path$2], interaction, context);
	}
};
var Identifier$1 = class extends IdentifierBase {
	constructor() {
		super(...arguments);
		this.variable = null;
	}
	...
	addExportedVariables(variables, exportNamesByVariable) {
		if (exportNamesByVariable.has(this.variable)) variables.push(this.variable);
	}
	bind() {
	...
	declare(kind, destructuredInitPath, init) {
		...
		if (kind === "parameter") variable = this.scope.addParameterDeclaration(this, destructuredInitPath);
		else {
			variable = this.scope.addDeclaration(this, this.scope.context, init, destructuredInitPath, kind);
			if (kind === "var" && treeshake && treeshake.correctVarValueBeforeDeclaration) variable.markInitializersForDeoptimization();
		}
	...
	includeDestructuredIfNecessary(context, destructuredInitPath, init) {
		...
		if (included ||= destructuredInitPath.length > 0 && !context.brokenFlow && propertyReadSideEffects && (propertyReadSideEffects === "always" || init.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, createHasEffectsContext()))) {
			if (this.variable && !this.variable.included) this.scope.context.includeVariableInModule(this.variable, EMPTY_PATH, context);
			init.includePath(destructuredInitPath, context);
		}
		if (!this.included && included) this.includeNode(context);
	...
	render(code, { snippets: { getPropertyAccess }, useOriginalName }, { renderedParentType, isCalleeOfRenderedParent, isShorthandProperty } = BLANK) {
		if (this.variable) {
			...
			if (name !== this.name) {
				...
				if (isShorthandProperty) code.prependRight(this.start, `${this.name}: `);
			}
			if (name === "eval" && renderedParentType === CallExpression && isCalleeOfRenderedParent) code.appendRight(this.start, "0, ");
		}
	}
};
...
function getSafeName(baseName, usedNames, forbiddenNames) {
	...
	let count = 1;
	while (usedNames.has(safeName) || RESERVED_NAMES.has(safeName) || forbiddenNames?.has(safeName)) safeName = `${baseName}$${toBase64(count++)}`;
...
var ChildScope = class ChildScope extends Scope {
	...
	addAccessedDynamicImport(importExpression) {
		...
		if (this.parent instanceof ChildScope) this.parent.addAccessedDynamicImport(importExpression);
	}
	addAccessedGlobals(globals, accessedGlobalsByScope) {
		const accessedGlobals = accessedGlobalsByScope.get(this) || new Set();
		for (const name of globals) accessedGlobals.add(name);
		accessedGlobalsByScope.set(this, accessedGlobals);
		if (this.parent instanceof ChildScope) this.parent.addAccessedGlobals(globals, accessedGlobalsByScope);
	}
	...
	addReturnExpression(expression) {
		if (this.parent instanceof ChildScope) this.parent.addReturnExpression(expression);
	}
	addUsedOutsideNames(usedNames, format, exportNamesByVariable, accessedGlobalsByScope) {
		for (const variable of this.accessedOutsideVariables.values()) if (variable.included) {
			usedNames.add(variable.getBaseVariableName());
			if (format === "system" && exportNamesByVariable.has(variable)) usedNames.add("exports");
		}
		const accessedGlobals = accessedGlobalsByScope.get(this);
		if (accessedGlobals) for (const name of accessedGlobals) usedNames.add(name);
	}
	contains(name) {
	...
	deconflict(format, exportNamesByVariable, accessedGlobalsByScope) {
		...
		if (this.accessedDynamicImports) {
			for (const importExpression of this.accessedDynamicImports) if (importExpression.inlineNamespace) usedNames.add(importExpression.inlineNamespace.getBaseVariableName());
		}
		for (const [name, variable] of this.variables) if (variable.included || variable.alwaysRendered) variable.setRenderNames(null, getSafeName(name, usedNames, variable.forbiddenNames));
...
function checkEffectForNodes(nodes, context) {
	for (const node of nodes) if (node.hasEffects(context)) return true;
	return false;
}
var MethodBase = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		if (interaction.type === INTERACTION_ACCESSED && this.kind === "get" && path$2.length === 0) return this.value.deoptimizeArgumentsOnInteractionAtPath({
			args: interaction.args,
			type: INTERACTION_CALLED,
			withNew: false
		}, EMPTY_PATH, recursionTracker);
		if (interaction.type === INTERACTION_ASSIGNED && this.kind === "set" && path$2.length === 0) return this.value.deoptimizeArgumentsOnInteractionAtPath({
			args: interaction.args,
			type: INTERACTION_CALLED,
			withNew: false
		}, EMPTY_PATH, recursionTracker);
		this.getAccessedValue()[0].deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizeCache() {}
	deoptimizePath(path$2) {
		this.getAccessedValue()[0].deoptimizePath(path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		return this.getAccessedValue()[0].getLiteralValueAtPath(path$2, recursionTracker, origin);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		return this.getAccessedValue()[0].getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
	}
	hasEffects(context) {
		return this.key.hasEffects(context);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (this.kind === "get" && interaction.type === INTERACTION_ACCESSED && path$2.length === 0) return this.value.hasEffectsOnInteractionAtPath(EMPTY_PATH, {
			args: interaction.args,
			type: INTERACTION_CALLED,
			withNew: false
		}, context);
		if (this.kind === "set" && interaction.type === INTERACTION_ASSIGNED) return this.value.hasEffectsOnInteractionAtPath(EMPTY_PATH, {
			args: interaction.args,
			type: INTERACTION_CALLED,
			withNew: false
		}, context);
		return this.getAccessedValue()[0].hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	getAccessedValue() {
		if (this.accessedValue === null) if (this.kind === "get") {
		...
		} else return this.accessedValue = [this.value, false];
...
var StaticBlock$1 = class extends NodeBase {
	createScope(parentScope) {
		this.scope = new BlockScope(parentScope);
	}
	hasEffects(context) {
		for (const node of this.body) if (node.hasEffects(context)) return true;
		return false;
	}
	include(context, includeChildrenRecursively) {
		this.included = true;
		for (const node of this.body) if (includeChildrenRecursively || node.shouldBeIncluded(context)) node.include(context, includeChildrenRecursively);
	}
	render(code, options) {
...
StaticBlock$1.prototype.includeNode = onlyIncludeSelfNoDeoptimize;
StaticBlock$1.prototype.applyDeoptimizations = doNotDeoptimize;
...
var ClassNode = class extends NodeBase {
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return interaction.type === INTERACTION_CALLED && path$2.length === 0 ? !interaction.withNew || (this.classConstructor === null ? this.superClass?.hasEffectsOnInteractionAtPath(path$2, interaction, context) : this.classConstructor.hasEffectsOnInteractionAtPath(path$2, interaction, context)) || false : this.getObjectEntity().hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	include(context, includeChildrenRecursively) {
		if (!this.included) this.includeNode(context);
	...
	initialise() {
		...
		for (const method of this.body.body) if (method instanceof MethodDefinition && method.kind === "constructor") {
	...
	applyDeoptimizations() {
		this.deoptimized = true;
		for (const definition of this.body.body) if (!isStaticBlock(definition) && !(definition.static || definition instanceof MethodDefinition && definition.kind === "constructor")) definition.deoptimizePath(UNKNOWN_PATH);
		this.scope.context.requestTreeshakingPass();
	}
	getObjectEntity() {
		if (this.objectEntity !== null) return this.objectEntity;
		...
		for (const definition of this.body.body) {
			...
			if (definition.computed) {
				...
				if (typeof keyValue === "symbol") {
					...
					continue;
				} else key = String(keyValue);
			} else key = definition.key instanceof Identifier$1 ? definition.key.name : String(definition.key.value);
			properties.push({
				key,
				kind,
				property: definition
...
var ClassDeclaration = class extends ClassNode {
	initialise() {
		super.initialise();
		if (this.id !== null) this.id.variable.isId = true;
	}
	parseNode(esTreeNode) {
		if (esTreeNode.id !== null) this.id = new Identifier$1(this, this.scope.parent).parseNode(esTreeNode.id);
	...
	render(code, options) {
		...
		if (this.id) {
			...
			if (format === "system" && exportNamesByVariable.has(variable)) code.appendLeft(this.end, `${_}${getSystemExportStatement([variable], options)};`);
	...
	applyDeoptimizations() {
		...
		if (id) {
			...
			for (const accessedVariable of scope.accessedOutsideVariables.values()) if (accessedVariable !== variable) accessedVariable.forbidName(name);
		}
	}
};
var ArgumentsVariable = class extends LocalVariable {
	...
	hasEffectsOnInteractionAtPath(path$2, { type }) {
		return type !== INTERACTION_ACCESSED || path$2.length > 1;
	}
	includePath(path$2, context) {
		super.includePath(path$2, context);
		for (const argument of this.deoptimizedArguments) argument.deoptimizePath(UNKNOWN_PATH);
		this.deoptimizedArguments.length = 0;
	}
};
function addArgumentToBeDeoptimized(argument) {
	if (this.included) argument.deoptimizePath(UNKNOWN_PATH);
	else this.deoptimizedArguments?.push(argument);
}
...
var ParameterVariable = class extends LocalVariable {
	...
	addArgumentForDeoptimization(entity) {
		...
		if (entity === UNKNOWN_EXPRESSION) {
			if (!this.argumentsToBeDeoptimized.has(UNKNOWN_EXPRESSION)) {
				...
				for (const { interaction } of this.deoptimizationInteractions) deoptimizeInteraction(interaction);
				this.deoptimizationInteractions = NO_INTERACTIONS;
			}
		} else if (this.deoptimizedFields.has(UnknownKey)) entity.deoptimizePath([...this.initPath, UnknownKey]);
		else if (!this.argumentsToBeDeoptimized.has(entity)) {
			this.argumentsToBeDeoptimized.add(entity);
			for (const field of this.deoptimizedFields) entity.deoptimizePath([...this.initPath, field]);
			for (const { interaction, path: path$2 } of this.deoptimizationInteractions) entity.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.initPath, ...path$2], SHARED_RECURSION_TRACKER);
		}
	}
	...
	*  to be deoptimized. */
	markReassigned() {
		...
		for (const expression of this.expressionsDependingOnKnownValue) expression.deoptimizeCache();
	...
	*/
	updateKnownValue(argument) {
		...
		if (this.knownValue === argument || this.knownValue instanceof Identifier$1 && argument instanceof Identifier$1 && this.knownValue.variable === argument.variable) return;
		const { knownValueLiteral } = this;
		if (typeof knownValueLiteral === "symbol" || argument.getLiteralValueAtPath(this.initPath, SHARED_RECURSION_TRACKER, this) !== knownValueLiteral) this.markReassigned();
	}
	/**
	...
	*/
	...
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		...
		return recursionTracker.withTrackedEntityAtPath(path$2, knownValue, () => knownValue.getLiteralValueAtPath([...this.initPath, ...path$2], recursionTracker, origin), UnknownValue);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		const { type } = interaction;
		if (this.isReassigned || type === INTERACTION_ASSIGNED || path$2.length + this.initPath.length > MAX_PATH_DEPTH) return super.hasEffectsOnInteractionAtPath(path$2, interaction, context);
		return !(type === INTERACTION_CALLED ? (interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path$2, interaction.args, this) : context.accessed.trackEntityAtPathAndGetIfTracked(path$2, this)) && this.getKnownValue().hasEffectsOnInteractionAtPath([...this.initPath, ...path$2], interaction, context);
	}
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2) {
		if (path$2.length >= 2 || this.argumentsToBeDeoptimized.has(UNKNOWN_EXPRESSION) || this.deoptimizationInteractions.length >= MAX_TRACKED_INTERACTIONS || path$2.length === 1 && (this.deoptimizedFields.has(UnknownKey) || interaction.type === INTERACTION_CALLED && this.deoptimizedFields.has(path$2[0])) || this.initPath.length + path$2.length > MAX_PATH_DEPTH) {
			deoptimizeInteraction(interaction);
			return;
		}
		if (!this.deoptimizations.trackEntityAtPathAndGetIfTracked(path$2, interaction.args)) {
			for (const entity of this.argumentsToBeDeoptimized) entity.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.initPath, ...path$2], SHARED_RECURSION_TRACKER);
			if (!this.argumentsToBeDeoptimized.has(UNKNOWN_EXPRESSION)) this.deoptimizationInteractions.push({
				interaction,
				path: path$2
			});
		}
	...
	deoptimizePath(path$2) {
		...
		if (this.deoptimizedFields.has(UnknownKey)) return;
		const key = path$2[0];
		if (this.deoptimizedFields.has(key)) return;
		this.deoptimizedFields.add(key);
		for (const entity of this.argumentsToBeDeoptimized) entity.deoptimizePath([...this.initPath, key]);
		if (key === UnknownKey) {
			this.deoptimizationInteractions = NO_INTERACTIONS;
	...
	getReturnExpressionWhenCalledAtPath(path$2) {
		if (path$2.length === 0) this.deoptimizePath(UNKNOWN_PATH);
		else if (!this.deoptimizedFields.has(path$2[0])) this.deoptimizePath([path$2[0]]);
		return UNKNOWN_RETURN_EXPRESSION;
	}
...
var ThisVariable = class extends ParameterVariable {
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return (context.replacedVariableInits.get(this) || UNKNOWN_EXPRESSION).hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
};
	...
	addDeclaration(identifier, context, init, destructuredInitPath, kind) {
		if (kind === "var") {
			...
			if (existingVariable) {
				...
				if (existingKind === "parameter" && existingVariable.declarations[0].parent.type === CatchClause) {
					const declaredVariable$1 = this.parent.parent.addDeclaration(identifier, context, UNDEFINED_EXPRESSION, destructuredInitPath, kind);
					existingVariable.renderLikeHoisted(declaredVariable$1);
					this.addHoistedVariable(name, declaredVariable$1);
					return declaredVariable$1;
				}
				if (existingKind === "var") {
					existingVariable.addDeclaration(identifier, init);
					return existingVariable;
...
var ParameterScope = class extends ChildScope {
	...
	*/
	addParameterDeclaration(identifier, argumentPath) {
		...
		if (existingParameter) return this.context.error(logDuplicateArgumentNameError(name), start);
	...
	addParameterVariables(parameters, hasRest) {
		...
		for (const parameterList of parameters) for (const parameter of parameterList) parameter.alwaysRendered = true;
	...
	includeCallArguments({ args }, context) {
		...
		for (let index = lastExplicitlyIncludedIndex; index >= 1; index--) {
			...
			if (parameterVariables) {
				...
				if (parameterVariables.length === 0) argumentIncluded = true;
				else for (const parameterVariable of parameterVariables) {
					if (parameterVariable.calledFromTryStatement) calledFromTryStatement = true;
					if (parameterVariable.included) {
						argumentIncluded = true;
						if (calledFromTryStatement) argument.include(context, true);
						else {
...
var ReturnValueScope = class extends ParameterScope {
		...
		for (; position < args.length - 1; position++) {
			...
			if (argument instanceof SpreadElement) {
				for (; position < parameters.length; position++) {
					...
					for (const variable of parameters[position]) variable.markReassigned();
				}
				break;
			}
			if (this.hasRest && position >= parameters.length - 1) argument.deoptimizePath(UNKNOWN_PATH);
			else {
				const variables = parameters[position];
				if (variables) for (const variable of variables) variable.addArgumentForDeoptimization(argument);
				this.addArgumentToBeDeoptimized(argument);
			}
		}
		const nonRestParameterLength = this.hasRest ? parameters.length - 1 : parameters.length;
		for (; position < nonRestParameterLength; position++) for (const variable of parameters[position]) variable.addArgumentForDeoptimization(UNDEFINED_EXPRESSION);
	}
	getReturnExpression() {
	...
	deoptimizeAllParameters() {
		for (const parameter of this.parameters) for (const variable of parameter) {
			variable.deoptimizePath(UNKNOWN_PATH);
			variable.markReassigned();
		}
	}
	reassignAllParameters() {
		for (const parameter of this.parameters) for (const variable of parameter) variable.markReassigned();
	}
	addArgumentToBeDeoptimized(_argument) {}
	updateReturnExpression() {
		if (this.returnExpressions.length === 1) this.returnExpression = this.returnExpressions[0];
			...
			for (const expression of this.returnExpressions) expression.deoptimizePath(UNKNOWN_PATH);
		}
	}
};
...
var ExpressionStatement$1 = class extends NodeBase {
	initialise() {
		super.initialise();
		if (this.directive && this.directive !== "use strict" && this.parent.type === Program) this.scope.context.log(
			LOGLEVEL_WARN,
...
ExpressionStatement$1.prototype.includeNode = onlyIncludeSelfNoDeoptimize;
ExpressionStatement$1.prototype.applyDeoptimizations = doNotDeoptimize;
var BlockStatement$1 = class extends NodeBase {
	get deoptimizeBody() {
		return isFlagSet(
			this.flags,
			32768
	...
	addImplicitReturnExpressionToScope() {
		...
		if (!lastStatement || lastStatement.type !== ReturnStatement) this.scope.addReturnExpression(UNKNOWN_EXPRESSION);
	}
	createScope(parentScope) {
	...
	render(code, options) {
		if (this.body.length > 0) renderStatementList(this.body, code, this.start + 1, this.end - 1, options);
		else super.render(code, options);
	}
};
BlockStatement$1.prototype.includeNode = onlyIncludeSelfNoDeoptimize;
BlockStatement$1.prototype.applyDeoptimizations = doNotDeoptimize;
var RestElement$1 = class extends NodeBase {
	constructor() {
		super(...arguments);
	...
	deoptimizePath(path$2) {
		if (path$2.length === 0) this.argument.deoptimizePath(EMPTY_PATH);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return path$2.length > 0 || this.argument.hasEffectsOnInteractionAtPath(EMPTY_PATH, interaction, context);
	}
	hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
...
RestElement$1.prototype.includeNode = onlyIncludeSelf;
const getIncludedPatternPath$1 = (destructuredInitPath) => destructuredInitPath.at(-1) === UnknownKey ? destructuredInitPath : [...destructuredInitPath, UnknownKey];
var FunctionBase = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		if (interaction.type === INTERACTION_CALLED && path$2.length === 0) this.scope.deoptimizeArgumentsOnCall(interaction);
		else this.getObjectEntity().deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizePath(path$2) {
		this.getObjectEntity().deoptimizePath(path$2);
		if (path$2.length === 1 && path$2[0] === UnknownKey) {
	...
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		if (path$2.length > 0) return this.getObjectEntity().getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
		if (this.async) {
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (path$2.length > 0 || interaction.type !== INTERACTION_CALLED) return this.getObjectEntity().hasEffectsOnInteractionAtPath(path$2, interaction, context);
		if (this.hasCachedEffects) return true;
		if (this.async) {
			const { propertyReadSideEffects: propertyReadSideEffects$1 } = this.scope.context.options.treeshake;
			const returnExpression = this.scope.getReturnExpression();
			if (returnExpression.hasEffectsOnInteractionAtPath(["then"], NODE_INTERACTION_UNKNOWN_CALL, context) || propertyReadSideEffects$1 && (propertyReadSideEffects$1 === "always" || returnExpression.hasEffectsOnInteractionAtPath(["then"], NODE_INTERACTION_UNKNOWN_ACCESS, context))) {
	...
	*/
	onlyFunctionCallUsed() {
		let variable = null;
		if (this.parent.type === VariableDeclarator) variable = this.parent.id.variable ?? null;
		if (this.parent.type === ExportDefaultDeclaration) variable = this.parent.variable;
	...
	initialise() {
		super.initialise();
		if (this.body instanceof BlockStatement$1) this.body.addImplicitReturnExpressionToScope();
		else this.scope.addReturnExpression(this.body);
		if (this.annotations && this.scope.context.options.treeshake.annotations) this.annotationNoSideEffects = this.annotations.some((comment) => comment.type === "noSideEffects");
	}
	parseNode(esTreeNode) {
		...
		scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement$1);
...
var FunctionNode = class extends FunctionBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		super.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
		if (interaction.type === INTERACTION_CALLED && path$2.length === 0 && interaction.args[0]) this.scope.thisVariable.addArgumentForDeoptimization(interaction.args[0]);
	}
	hasEffects(context) {
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (this.annotationNoSideEffects && path$2.length === 0 && interaction.type === INTERACTION_CALLED) return false;
		if (super.hasEffectsOnInteractionAtPath(path$2, interaction, context)) return true;
		if (path$2.length === 0 && interaction.type === INTERACTION_CALLED) {
			...
			context.brokenFlow = brokenFlow;
			if (thisInit) replacedVariableInits.set(this.scope.thisVariable, thisInit);
			else replacedVariableInits.delete(this.scope.thisVariable);
	...
	include(context, includeChildrenRecursively) {
		...
		for (const parameter of this.params) if (!(parameter instanceof Identifier$1) || hasArguments) parameter.include(context, includeChildrenRecursively);
	}
	includeNode(context) {
		this.included = true;
		const hasArguments = this.scope.argumentsVariable.included;
		for (const parameter of this.params) if (!(parameter instanceof Identifier$1) || hasArguments) parameter.includePath(UNKNOWN_PATH, context);
	}
	initialise() {
		super.initialise();
...
function getFunctionIdInsertPosition(code, start) {
	const declarationEnd = findFirstOccurrenceOutsideComment(code, "function", start) + 8;
	code = code.slice(declarationEnd, findFirstOccurrenceOutsideComment(code, "(", declarationEnd));
	const generatorStarPos = findFirstOccurrenceOutsideComment(code, "*");
	if (generatorStarPos === -1) return declarationEnd;
	return declarationEnd + generatorStarPos + 1;
}
var ExportDefaultDeclaration$1 = class extends NodeBase {
	bind() {
		super.bind();
	...
	include(context, includeChildrenRecursively) {
		...
		if (includeChildrenRecursively) this.scope.context.includeVariableInModule(this.variable, UNKNOWN_PATH, context);
	}
	includePath(path$2, context) {
		this.included = true;
		this.declaration.includePath(path$2, context);
	}
	initialise() {
		super.initialise();
	...
	render(code, options, nodeRenderOptions) {
		...
		if (this.declaration instanceof FunctionDeclaration) this.renderNamedDeclaration(code, declarationStart, this.declaration.id === null ? getFunctionIdInsertPosition(code.original, declarationStart) : null, options);
		else if (this.declaration instanceof ClassDeclaration) this.renderNamedDeclaration(code, declarationStart, this.declaration.id === null ? findFirstOccurrenceOutsideComment(code.original, "class", start) + 5 : null, options);
		else if (this.variable.getOriginalVariable() !== this.variable) {
			treeshakeNode(this, code, start, end);
			return;
		} else if (this.variable.included) this.renderVariableDeclaration(code, declarationStart, options);
		else {
			code.remove(this.start, declarationStart);
			this.declaration.render(code, options, { renderedSurroundingElement: ExpressionStatement });
			if (code.original[this.end - 1] !== ";") code.appendLeft(this.end, ";");
			return;
		}
	...
	renderNamedDeclaration(code, declarationStart, idInsertPosition, options) {
		...
		code.remove(this.start, declarationStart);
		if (idInsertPosition !== null) code.appendLeft(idInsertPosition, ` ${name}`);
		if (format === "system" && this.declaration instanceof ClassDeclaration && exportNamesByVariable.has(this.variable)) code.appendLeft(this.end, ` ${getSystemExportStatement([this.variable], options)};`);
	}
	renderVariableDeclaration(code, declarationStart, { format, exportNamesByVariable, snippets: { cnst, getPropertyAccess } }) {
		...
		if (systemExportNames) {
			...
			if (!hasTrailingSemicolon) code.appendLeft(this.end, ";");
		}
	}
};
ExportDefaultDeclaration$1.prototype.needsBoundaries = true;
ExportDefaultDeclaration$1.prototype.includeNode = onlyIncludeSelfNoDeoptimize;
ExportDefaultDeclaration$1.prototype.applyDeoptimizations = doNotDeoptimize;
...
const getHelpersBlock = (additionalHelpers, accessedGlobals, indent, snippets, liveBindings, freeze, symbols) => {
	...
	for (const variable of HELPER_NAMES) if (accessedGlobals.has(variable)) usedHelpers.add(variable);
...
const HELPER_GENERATORS = {
	[DOCUMENT_CURRENT_SCRIPT](_t, { _, n: n$1 }) {
		return `var ${DOCUMENT_CURRENT_SCRIPT}${_}=${_}typeof document${_}!==${_}'undefined'${_}?${_}document.currentScript${_}:${_}null;${n$1}`;
	},
	[INTEROP_DEFAULT_COMPAT_VARIABLE](_t, snippets, liveBindings) {
		const { _, getDirectReturnFunction, n: n$1 } = snippets;
		const [left, right] = getDirectReturnFunction(["e"], {
			functionReturn: true,
			lineBreakIndent: null,
			name: INTEROP_DEFAULT_COMPAT_VARIABLE
		});
		return `${left}${getIsCompatNamespace(snippets)}${_}?${_}${liveBindings ? getDefaultLiveBinding(snippets) : getDefaultStatic(snippets)}${right}${n$1}${n$1}`;
	},
	[INTEROP_DEFAULT_VARIABLE](_t, snippets, liveBindings) {
		...
		return `${left}e${_}&&${_}e.__esModule${_}?${_}${liveBindings ? getDefaultLiveBinding(snippets) : getDefaultStatic(snippets)}${right}${n$1}${n$1}`;
	},
	[INTEROP_NAMESPACE_COMPAT_VARIABLE](t, snippets, liveBindings, freeze, symbols, usedHelpers) {
		...
		if (usedHelpers.has(INTEROP_NAMESPACE_DEFAULT_VARIABLE)) {
			...
			return `${left}${getIsCompatNamespace(snippets)}${_}?${_}e${_}:${_}${INTEROP_NAMESPACE_DEFAULT_VARIABLE}(e)${right}${n$1}${n$1}`;
		}
		return `function ${INTEROP_NAMESPACE_COMPAT_VARIABLE}(e)${_}{${n$1}${t}if${_}(${getIsCompatNamespace(snippets)})${_}return e;${n$1}` + createNamespaceObject(t, t, snippets, liveBindings, freeze, symbols) + `}${n$1}${n$1}`;
	},
	[INTEROP_NAMESPACE_DEFAULT_ONLY_VARIABLE](_t, snippets, _liveBindings, freeze, symbols) {
		const { getDirectReturnFunction, getObject, n: n$1, _ } = snippets;
		const [left, right] = getDirectReturnFunction(["e"], {
			functionReturn: true,
			lineBreakIndent: null,
			name: INTEROP_NAMESPACE_DEFAULT_ONLY_VARIABLE
		});
		return `${left}${getFrozen(freeze, getWithToStringTag(symbols, getObject([[null, `__proto__:${_}null`], ["default", "e"]], { lineBreakIndent: null }), snippets))}${right}${n$1}${n$1}`;
	},
	[INTEROP_NAMESPACE_DEFAULT_VARIABLE](t, snippets, liveBindings, freeze, symbols) {
		const { _, n: n$1 } = snippets;
		return `function ${INTEROP_NAMESPACE_DEFAULT_VARIABLE}(e)${_}{${n$1}` + createNamespaceObject(t, t, snippets, liveBindings, freeze, symbols) + `}${n$1}${n$1}`;
	},
	[INTEROP_NAMESPACE_VARIABLE](t, snippets, liveBindings, freeze, symbols, usedHelpers) {
		...
		if (usedHelpers.has(INTEROP_NAMESPACE_DEFAULT_VARIABLE)) {
			...
			return `${left}e${_}&&${_}e.__esModule${_}?${_}e${_}:${_}${INTEROP_NAMESPACE_DEFAULT_VARIABLE}(e)${right}${n$1}${n$1}`;
		}
		return `function ${INTEROP_NAMESPACE_VARIABLE}(e)${_}{${n$1}${t}if${_}(e${_}&&${_}e.__esModule)${_}return e;${n$1}` + createNamespaceObject(t, t, snippets, liveBindings, freeze, symbols) + `}${n$1}${n$1}`;
	},
	[MERGE_NAMESPACES_VARIABLE](t, snippets, liveBindings, freeze, symbols) {
		const { _, cnst, n: n$1 } = snippets;
		const useForEach = cnst === "var" && liveBindings;
		return `function ${MERGE_NAMESPACES_VARIABLE}(n, m)${_}{${n$1}${t}${loopOverNamespaces(`{${n$1}${t}${t}${t}if${_}(k${_}!==${_}'default'${_}&&${_}!(k in n))${_}{${n$1}` + (liveBindings ? useForEach ? copyOwnPropertyLiveBinding : copyPropertyLiveBinding : copyPropertyStatic)(t, t + t + t + t, snippets) + `${t}${t}${t}}${n$1}${t}${t}}`, useForEach, t, snippets)}${n$1}${t}return ${getFrozen(freeze, getWithToStringTag(symbols, "n", snippets))};${n$1}}${n$1}${n$1}`;
	}
};
...
const createNamespaceObject = (t, index, snippets, liveBindings, freeze, symbols) => {
	const { _, cnst, getObject, getPropertyAccess, n: n$1, s } = snippets;
	const copyProperty = `{${n$1}` + (liveBindings ? copyNonDefaultOwnPropertyLiveBinding : copyPropertyStatic)(t, index + t + t, snippets) + `${index}${t}}`;
	return `${index}${cnst} n${_}=${_}Object.create(null${symbols ? `,${_}{${_}[Symbol.toStringTag]:${_}${getToStringTagValue(getObject)}${_}}` : ""});${n$1}${index}if${_}(e)${_}{${n$1}${index}${t}${loopOverKeys(copyProperty, !liveBindings, snippets)}${n$1}${index}}${n$1}${index}n${getPropertyAccess("default")}${_}=${_}e;${n$1}${index}return ${getFrozen(freeze, "n")}${s}${n$1}`;
};
const loopOverKeys = (body, allowVariableLoopVariable, { _, cnst, getFunctionIntro, s }) => cnst !== "var" || allowVariableLoopVariable ? `for${_}(${cnst} k in e)${_}${body}` : `Object.keys(e).forEach(${getFunctionIntro(["k"], {
	isAsync: false,
	name: null
})}${body})${s}`;
const loopOverNamespaces = (body, useForEach, t, { _, cnst, getDirectReturnFunction, getFunctionIntro, n: n$1 }) => {
	if (useForEach) {
		const [left, right] = getDirectReturnFunction(["e"], {
	...
	return `for${_}(var i${_}=${_}0;${_}i${_}<${_}m.length;${_}i++)${_}{${n$1}${t}${t}${cnst} e${_}=${_}m[i];${n$1}${t}${t}if${_}(typeof e${_}!==${_}'string'${_}&&${_}!Array.isArray(e))${_}{${_}for${_}(${cnst} k in e)${_}${body}${_}}${n$1}${t}}`;
};
const copyNonDefaultOwnPropertyLiveBinding = (t, index, snippets) => {
	const { _, n: n$1 } = snippets;
	return `${index}if${_}(k${_}!==${_}'default')${_}{${n$1}` + copyOwnPropertyLiveBinding(t, index + t, snippets) + `${index}}${n$1}`;
};
const copyOwnPropertyLiveBinding = (t, index, { _, cnst, getDirectReturnFunction, n: n$1 }) => {
	const [left, right] = getDirectReturnFunction([], {
		functionReturn: true,
		lineBreakIndent: null,
		name: null
	});
	return `${index}${cnst} d${_}=${_}Object.getOwnPropertyDescriptor(e,${_}k);${n$1}${index}Object.defineProperty(n,${_}k,${_}d.get${_}?${_}d${_}:${_}{${n$1}${index}${t}enumerable:${_}true,${n$1}${index}${t}get:${_}${left}e[k]${right}${n$1}${index}});${n$1}`;
};
const copyPropertyLiveBinding = (t, index, { _, cnst, getDirectReturnFunction, n: n$1 }) => {
	const [left, right] = getDirectReturnFunction([], {
		functionReturn: true,
		lineBreakIndent: null,
		name: null
	});
	return `${index}${cnst} d${_}=${_}Object.getOwnPropertyDescriptor(e,${_}k);${n$1}${index}if${_}(d)${_}{${n$1}${index}${t}Object.defineProperty(n,${_}k,${_}d.get${_}?${_}d${_}:${_}{${n$1}${index}${t}${t}enumerable:${_}true,${n$1}${index}${t}${t}get:${_}${left}e[k]${right}${n$1}${index}${t}});${n$1}${index}}${n$1}`;
};
const copyPropertyStatic = (_t, index, { _, n: n$1 }) => `${index}n[k]${_}=${_}e[k];${n$1}`;
const getFrozen = (freeze, fragment) => freeze ? `Object.freeze(${fragment})` : fragment;
const getWithToStringTag = (symbols, fragment, { _, getObject }) => symbols ? `Object.defineProperty(${fragment},${_}Symbol.toStringTag,${_}${getToStringTagValue(getObject)})` : fragment;
const HELPER_NAMES = Object.keys(HELPER_GENERATORS);
function getToStringTagValue(getObject) {
	return getObject([["value", "'Module'"]], { lineBreakIndent: null });
}
var Literal$1 = class extends NodeBase {
	deoptimizeArgumentsOnInteractionAtPath() {}
	getLiteralValueAtPath(path$2) {
		if (path$2.length > 0 || this.value === null && this.scope.context.code.charCodeAt(this.start) !== 110 || typeof this.value === "bigint" || this.scope.context.code.charCodeAt(this.start) === 47) return UnknownValue;
		return this.value;
	}
	getReturnExpressionWhenCalledAtPath(path$2) {
		if (path$2.length !== 1) return UNKNOWN_RETURN_EXPRESSION;
		return getMemberReturnExpressionWhenCalled(this.members, path$2[0]);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		switch (interaction.type) {
			case INTERACTION_ACCESSED: return path$2.length > (this.value === null ? 0 : 1);
			case INTERACTION_ASSIGNED: return true;
			case INTERACTION_CALLED: {
				if (this.included && this.value instanceof RegExp && (this.value.global || this.value.sticky)) return true;
				return path$2.length !== 1 || hasMemberEffectWhenCalled(this.members, path$2[0], interaction, context);
			}
		}
	}
	initialise() {
	...
	render(code) {
		if (typeof this.value === "string") code.indentExclusionRanges.push([this.start + 1, this.end - 1]);
	}
};
Literal$1.prototype.includeNode = onlyIncludeSelf;
function getChainElementLiteralValueAtPath(element, object, path$2, recursionTracker, origin) {
	if ("getLiteralValueAtPathAsChainElement" in object) {
		...
		if (calleeValue === IS_SKIPPED_CHAIN || element.optional && calleeValue == null) return IS_SKIPPED_CHAIN;
	} else if (element.optional && object.getLiteralValueAtPath(EMPTY_PATH, SHARED_RECURSION_TRACKER, origin) == null) return IS_SKIPPED_CHAIN;
	return element.getLiteralValueAtPath(path$2, recursionTracker, origin);
}
...
function getResolvableComputedPropertyKey(propertyKey) {
	if (propertyKey instanceof Literal$1) return String(propertyKey.value);
	return null;
}
function getPathIfNotComputed(memberExpression) {
	...
	if (typeof nextPathKey === "string") {
		if (object instanceof Identifier$1) return [{
			key: object.name,
			pos: object.start
		}, {
			key: nextPathKey,
		...
		if (object instanceof MemberExpression$1) {
...
function getStringFromPath(path$2) {
	let pathString = path$2[0].key;
	for (let index = 1; index < path$2.length; index++) pathString += "." + path$2[index].key;
	return pathString;
}
var MemberExpression$1 = class extends NodeBase {
	constructor() {
		super(...arguments);
	...
	bind() {
		this.bound = true;
		const path$2 = getPathIfNotComputed(this);
		const baseVariable = path$2 && this.scope.findVariable(path$2[0].key);
		if (baseVariable?.isNamespace) {
			const resolvedVariable = resolveNamespaceVariables(baseVariable, path$2.slice(1), this.scope.context);
			if (!resolvedVariable) super.bind();
			else if (resolvedVariable === "undefined") this.isUndefined = true;
			else {
				this.variable = resolvedVariable;
				this.scope.addNamespaceMemberAccess(getStringFromPath(path$2), resolvedVariable);
			}
		} else super.bind();
	}
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		if (this.promiseHandler) this.promiseHandler.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
		else if (this.variable) this.variable.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
		else if (!this.isUndefined) if (path$2.length < MAX_PATH_DEPTH) this.object.deoptimizeArgumentsOnInteractionAtPath(interaction, this.propertyKey === UnknownKey ? UNKNOWN_PATH : [this.propertyKey, ...path$2], recursionTracker);
		else deoptimizeInteraction(interaction);
	}
	deoptimizeAssignment(destructuredInitPath, init) {
		this.deoptimizePath(EMPTY_PATH);
		init.deoptimizePath([...destructuredInitPath, UnknownKey]);
	...
	deoptimizeCache() {
		...
		if (this.included) object.includePath(UNKNOWN_PATH, createInclusionContext());
		for (const expression of expressionsToBeDeoptimized) expression.deoptimizeCache();
	}
	deoptimizePath(path$2) {
		if (path$2.length === 0) this.disallowNamespaceReassignment();
		if (this.variable) this.variable.deoptimizePath(path$2);
		else if (!this.isUndefined) {
			const { propertyKey } = this;
			this.object.deoptimizePath([propertyKey === UnknownKey ? UnknownNonAccessorKey : propertyKey, ...path$2.length < MAX_PATH_DEPTH ? path$2 : [...path$2.slice(0, MAX_PATH_DEPTH), UnknownKey]]);
		}
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		if (this.variable) return this.variable.getLiteralValueAtPath(path$2, recursionTracker, origin);
		if (this.isUndefined) return void 0;
		const propertyKey = this.getDynamicPropertyKey();
		if (propertyKey !== UnknownKey && path$2.length < MAX_PATH_DEPTH) {
			if (propertyKey !== this.propertyKey) this.expressionsToBeDeoptimized.push(origin);
			return this.object.getLiteralValueAtPath([propertyKey, ...path$2], recursionTracker, origin);
		}
		return UnknownValue;
	}
	getLiteralValueAtPathAsChainElement(path$2, recursionTracker, origin) {
		if (this.variable) return this.variable.getLiteralValueAtPath(path$2, recursionTracker, origin);
		if (this.isUndefined) return void 0;
		return getChainElementLiteralValueAtPath(this, this.object, path$2, recursionTracker, origin);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		if (this.variable) return this.variable.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
		if (this.isUndefined) return [UNDEFINED_EXPRESSION, false];
		const propertyKey = this.getDynamicPropertyKey();
		if (propertyKey !== UnknownKey && path$2.length < MAX_PATH_DEPTH) {
			if (propertyKey !== this.propertyKey) this.expressionsToBeDeoptimized.push(origin);
			return this.object.getReturnExpressionWhenCalledAtPath([propertyKey, ...path$2], interaction, recursionTracker, origin);
		}
		return UNKNOWN_RETURN_EXPRESSION;
	}
	hasEffects(context) {
	...
	hasEffectsAsChainElement(context) {
		...
		if (this.optional && this.object.getLiteralValueAtPath(EMPTY_PATH, SHARED_RECURSION_TRACKER, this) == null) return objectHasEffects || IS_SKIPPED_CHAIN;
		if (!this.deoptimized) this.applyDeoptimizations();
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (this.variable) return this.variable.hasEffectsOnInteractionAtPath(path$2, interaction, context);
		if (this.isUndefined) return true;
		if (path$2.length < MAX_PATH_DEPTH) return this.object.hasEffectsOnInteractionAtPath([this.getDynamicPropertyKey(), ...path$2], interaction, context);
		return true;
	}
	hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
		return destructuredInitPath.length > 0 && init.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, context);
	...
	includeNode(context) {
		this.included = true;
		if (!this.deoptimized) this.applyDeoptimizations();
		if (this.variable) this.scope.context.includeVariableInModule(this.variable, EMPTY_PATH, context);
		else if (!this.isUndefined) this.object.includePath([this.propertyKey], context);
	}
	...
	includePath(path$2, context) {
		if (!this.included) this.includeNode(context);
		if (this.variable) this.variable?.includePath(path$2, context);
		else if (!this.isUndefined) this.object.includePath([this.propertyKey, ...path$2.length < MAX_PATH_DEPTH ? path$2 : [...path$2.slice(0, MAX_PATH_DEPTH), UnknownKey]], context);
	}
	...
	includeCallArguments(interaction, context) {
		if (this.promiseHandler) this.promiseHandler.includeCallArguments(interaction, context);
		else if (this.variable) this.variable.includeCallArguments(interaction, context);
		else includeInteraction(interaction, context);
	}
	...
	render(code, options, { renderedParentType, isCalleeOfRenderedParent, renderedSurroundingElement } = BLANK) {
		if (this.variable || this.isUndefined) {
			...
			if (renderedParentType && isCalleeOfRenderedParent) code.appendRight(this.start, "0, ");
	...
	applyDeoptimizations() {
		this.deoptimized = true;
		const { propertyReadSideEffects } = this.scope.context.options.treeshake;
		if (this.bound && propertyReadSideEffects && !(this.variable || this.isUndefined || this.promiseHandler)) {
	...
	disallowNamespaceReassignment() {
		if (this.object instanceof Identifier$1) {
			...
			if (variable.isNamespace) {
				if (this.variable) this.scope.context.includeVariableInModule(this.variable, UNKNOWN_PATH, createInclusionContext());
...
function resolveNamespaceVariables(baseVariable, path$2, astContext) {
	if (path$2.length === 0) return baseVariable;
	if (!baseVariable.isNamespace || baseVariable instanceof ExternalVariable) return null;
	const exportName = path$2[0].key;
	const [variable, options] = baseVariable.context.traceExport(exportName);
	if (!variable) {
		if (path$2.length === 1) {
			const fileName = baseVariable.context.fileName;
			astContext.log(LOGLEVEL_WARN, logMissingExport(exportName, astContext.module.id, fileName, !!options?.missingButExportExists), path$2[0].pos);
			return "undefined";
		}
		return null;
	}
	return resolveNamespaceVariables(variable, path$2.slice(1), astContext);
}
...
var MetaProperty = class extends NodeBase {
	...
	getReferencedFileName(outputPluginDriver) {
		...
		if (name === IMPORT) {
			if (metaProperty?.startsWith(FILE_OBJ_PREFIX)) return outputPluginDriver.getFileName(metaProperty.slice(FILE_OBJ_PREFIX.length));
			else if (metaProperty?.startsWith(FILE_PREFIX)) return outputPluginDriver.getFileName(metaProperty.slice(FILE_PREFIX.length));
		}
		return null;
	}
	hasEffects() {
	...
	hasEffectsOnInteractionAtPath(path$2, { type }) {
		return path$2.length > 1 || type !== INTERACTION_ACCESSED;
	}
	include() {
	...
	includeNode() {
		...
		if (this.meta.name === IMPORT) {
			...
			const metaProperty = this.metaProperty = parent instanceof MemberExpression$1 && typeof parent.propertyKey === "string" ? parent.propertyKey : null;
			if (metaProperty?.startsWith(FILE_OBJ_PREFIX)) this.referenceId = metaProperty.slice(FILE_OBJ_PREFIX.length);
			else if (metaProperty?.startsWith(FILE_PREFIX)) this.referenceId = metaProperty.slice(FILE_PREFIX.length);
		}
	}
	render(code, renderOptions) {
		...
		if (referenceId) {
			...
			const replacement$1 = pluginDriver.hookFirstSync("resolveFileUrl", [{
				attributes,
				chunkId,
				fileName,
				format,
			...
			code.overwrite(parent.start, parent.end, replacement$1, { contentOnly: true });
			return;
		}
		let replacement = pluginDriver.hookFirstSync("resolveImportMeta", [metaProperty, {
			attributes,
			chunkId,
			format,
			moduleId
		...
		if (typeof replacement === "string") if (parent instanceof MemberExpression$1) code.overwrite(parent.start, parent.end, replacement, { contentOnly: true });
		else code.overwrite(start, end, replacement, { contentOnly: true });
	}
	setResolution(format, accessedGlobalsByScope, preliminaryChunkId) {
		...
		if (accessedGlobals.length > 0) this.scope.addAccessedGlobals(accessedGlobals, accessedGlobalsByScope);
	}
};
const formatsMaybeAccessDocumentCurrentScript = [
	"cjs",
	"iife",
	"umd"
];
...
const getResolveUrl = (path$2, asObject, URL$1 = "URL") => `new ${URL$1}(${path$2})${asObject ? "" : ".href"}`;
const getRelativeUrlFromDocument = (relativePath, asObject, umd$1 = false) => getResolveUrl(`'${escapeId(relativePath)}', ${umd$1 ? `typeof document === 'undefined' ? location.href : ` : ""}document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI`, asObject);
...
const getFileUrlFromFullPath = (path$2, asObject) => `require('u' + 'rl').pathToFileURL(${path$2})${asObject ? "" : ".href"}`;
const getFileUrlFromRelativePath = (path$2, asObject) => getFileUrlFromFullPath(`__dirname + '/${escapeId(path$2)}'`, asObject);
const getUrlFromDocument = (chunkId, umd$1 = false) => `${umd$1 ? `typeof document === 'undefined' ? location.href : ` : ""}(${DOCUMENT_CURRENT_SCRIPT} && ${DOCUMENT_CURRENT_SCRIPT}.tagName.toUpperCase() === 'SCRIPT' && ${DOCUMENT_CURRENT_SCRIPT}.src || new URL('${escapeId(chunkId)}', document.baseURI).href)`;
...
var ExportDefaultVariable = class ExportDefaultVariable extends LocalVariable {
	constructor(exportDefaultDeclaration, context) {
		...
		} else if (declaration instanceof Identifier$1) this.originalId = declaration;
	}
	addReference(identifier) {
		if (!this.hasId) this.name = identifier.name;
	}
	addUsedPlace(usedPlace) {
		const original = this.getOriginalVariable();
		if (original === this) super.addUsedPlace(usedPlace);
		else original.addUsedPlace(usedPlace);
	}
	forbidName(name) {
		const original = this.getOriginalVariable();
		if (original === this) super.forbidName(name);
		else original.forbidName(name);
	}
...
var NamespaceVariable = class extends Variable {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		if (path$2.length > 1 || path$2.length === 1 && interaction.type === INTERACTION_CALLED) {
			const key = path$2[0];
			if (typeof key === "string") this.module.getExportedVariablesByName().get(key)?.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2.slice(1), recursionTracker);
			else deoptimizeInteraction(interaction);
		}
	}
	deoptimizePath(path$2) {
		if (path$2.length > 1) {
			const key = path$2[0];
			if (typeof key === "string") this.module.getExportedVariablesByName().get(key)?.deoptimizePath(path$2.slice(1));
			else if (!this.areAllMembersDeoptimized) {
				this.areAllMembersDeoptimized = true;
				for (const variable of this.module.getExportedVariablesByName().values()) variable.deoptimizePath(UNKNOWN_PATH);
			}
		}
	}
	getLiteralValueAtPath(path$2) {
		if (path$2[0] === SymbolToStringTag) return "Module";
		return UnknownValue;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		const { type } = interaction;
		if (path$2.length === 0) return true;
		if (path$2.length === 1 && type !== INTERACTION_CALLED) return type === INTERACTION_ASSIGNED;
		const key = path$2[0];
		if (typeof key !== "string") return true;
		const memberVariable = this.module.getExportedVariablesByName().get(key);
		return !memberVariable || memberVariable.hasEffectsOnInteractionAtPath(path$2.slice(1), interaction, context);
	}
	includePath(path$2, context) {
		super.includePath(path$2, context);
		this.includeMemberPath(path$2, context);
	}
	includeMemberPath(path$2, context) {
		if (path$2.length > 0) {
			const [name, ...remainingPath] = path$2;
			if (typeof name === "string") {
				const variable = this.module.getExportedVariablesByName().get(name);
				if (variable) this.context.includeVariableInModule(variable, remainingPath, context);
				else this.includeNonExplicitNamespaces();
			} else if (name) {
	...
	prepare(accessedGlobalsByScope) {
		if (this.mergedNamespaces.length > 0) this.module.scope.addAccessedGlobals([MERGE_NAMESPACES_VARIABLE], accessedGlobalsByScope);
	}
	renderBlock(options) {
		const { exportNamesByVariable, format, freeze, indent: t, symbols, snippets: { _, cnst, getObject, getPropertyAccess, n: n$1, s } } = options;
		const memberVariables = this.module.getExportedVariablesByName();
		const members = [...memberVariables.entries()].filter(([name$1, variable]) => !name$1.startsWith("*") && variable.included).map(([name$1, variable]) => {
			if (this.referencedEarly || variable.isReassigned || variable === this) return [null, `get ${stringifyObjectKeyIfNeeded(name$1)}${_}()${_}{${_}return ${variable.getName(getPropertyAccess)}${s}${_}}`];
			return [name$1, variable.getName(getPropertyAccess)];
		});
		members.unshift([null, `__proto__:${_}null`]);
		let output = getObject(members, { lineBreakIndent: {
			base: "",
			t
		} });
		if (this.mergedNamespaces.length > 0) {
			...
			if (symbols) output = `/*#__PURE__*/Object.defineProperty(${output},${_}Symbol.toStringTag,${_}${getToStringTagValue(getObject)})`;
			if (freeze) output = `/*#__PURE__*/Object.freeze(${output})`;
		}
		const name = this.getName(getPropertyAccess);
		output = `${cnst} ${name}${_}=${_}${output};`;
		if (format === "system" && exportNamesByVariable.has(this)) output += `${n$1}${getSystemExportStatement([this], options)};`;
		return output;
	}
	renderFirst() {
...
const getDynamicNamespaceVariable = (namespace) => Object.create(namespace, { includePath: { value(path$2, context) {
	namespace.includeMemberPath(path$2, context);
} } });
var SyntheticNamedExportVariable = class SyntheticNamedExportVariable extends Variable {
	...
	getBaseVariable() {
		...
		while (baseVariable instanceof ExportDefaultVariable || baseVariable instanceof SyntheticNamedExportVariable) {
			...
			if (baseVariable instanceof SyntheticNamedExportVariable) baseVariable = baseVariable.syntheticNamespace;
		}
	...
	includePath(path$2, context) {
		super.includePath(path$2, context);
		this.context.includeVariableInModule(this.syntheticNamespace, [this.name, ...path$2], context);
	}
	setRenderNames(baseName, name) {
...
var ExternalChunk = class {
	...
	getFileName() {
		...
		return this.fileName = (typeof paths === "function" ? paths(this.id) : paths[this.id]) || (this.renormalizeRenderPath ? normalize(relative$2(this.inputBase, this.id)) : this.id);
	}
...
function formatAttributes(attributes, { getObject }) {
	...
	if (assertionEntries.length > 0) return getObject(assertionEntries, { lineBreakIndent: null });
	return null;
}
...
function getCompleteAmdId(options, chunkId) {
	if (options.autoId) return `${options.basePath ? options.basePath + "/" : ""}${removeJsExtension(chunkId)}`;
	return options.id ?? "";
}
function getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings, reexportProtoFromExternal, mechanism = "return ") {
	const { _, getDirectReturnFunction, getFunctionIntro, getPropertyAccess, n: n$1, s } = snippets;
	if (!namedExportsMode) return `${n$1}${n$1}${mechanism}${getSingleDefaultExport(exports, dependencies, interop, externalLiveBindings, getPropertyAccess)};`;
	let exportBlock = "";
	if (namedExportsMode) for (const { defaultVariableName, importPath, isChunk, name, namedExportsMode: depNamedExportsMode, namespaceVariableName, reexports } of dependencies) {
		if (!reexports) continue;
		for (const specifier of reexports) if (specifier.reexported !== "*") {
			...
			if (exportBlock) exportBlock += n$1;
			if (specifier.imported !== "*" && specifier.needsLiveBinding) {
				...
				exportBlock += `Object.defineProperty(exports,${_}${JSON.stringify(specifier.reexported)},${_}{${n$1}${t}enumerable:${_}true,${n$1}${t}get:${_}${left}${importName}${right}${n$1}});`;
			} else if (specifier.reexported === "__proto__") exportBlock += `Object.defineProperty(exports,${_}"__proto__",${_}{${n$1}${t}enumerable:${_}true,${n$1}${t}value:${_}${importName}${n$1}});`;
			else exportBlock += `exports${getPropertyAccess(specifier.reexported)}${_}=${_}${importName};`;
		}
	}
	for (const { exported, local } of exports) {
		...
		if (lhs !== rhs) {
			if (exportBlock) exportBlock += n$1;
			exportBlock += exported === "__proto__" ? `Object.defineProperty(exports,${_}"__proto__",${_}{${n$1}${t}enumerable:${_}true,${n$1}${t}value:${_}${rhs}${n$1}});` : `${lhs}${_}=${_}${rhs};`;
		}
	}
	if (namedExportsMode) for (const { name, reexports } of dependencies) {
		if (!reexports) continue;
		for (const specifier of reexports) if (specifier.reexported === "*") {
			if (exportBlock) exportBlock += n$1;
			if (!specifier.needsLiveBinding && reexportProtoFromExternal) {
				const protoString = "'__proto__'";
				exportBlock += `Object.prototype.hasOwnProperty.call(${name},${_}${protoString})${_}&&${n$1}${t}!Object.prototype.hasOwnProperty.call(exports,${_}${protoString})${_}&&${n$1}${t}Object.defineProperty(exports,${_}${protoString},${_}{${n$1}${t}${t}enumerable:${_}true,${n$1}${t}${t}value:${_}${name}[${protoString}]${n$1}${t}});${n$1}${n$1}`;
			}
			const copyPropertyIfNecessary = `{${n$1}${t}if${_}(k${_}!==${_}'default'${_}&&${_}!Object.prototype.hasOwnProperty.call(exports,${_}k))${_}${getDefineProperty(name, specifier.needsLiveBinding, t, snippets)}${s}${n$1}}`;
	...
	if (exportBlock) return `${n$1}${n$1}${exportBlock}`;
	return "";
}
function getSingleDefaultExport(exports, dependencies, interop, externalLiveBindings, getPropertyAccess) {
	if (exports.length > 0) return exports[0].local;
	else for (const { defaultVariableName, importPath, isChunk, name, namedExportsMode: depNamedExportsMode, namespaceVariableName, reexports } of dependencies) if (reexports) return getReexportedImportName(name, reexports[0].imported, depNamedExportsMode, isChunk, defaultVariableName, namespaceVariableName, interop, importPath, externalLiveBindings, getPropertyAccess);
}
function getReexportedImportName(moduleVariableName, imported, depNamedExportsMode, isChunk, defaultVariableName, namespaceVariableName, interop, moduleId, externalLiveBindings, getPropertyAccess) {
	...
	if (imported === "*") return (isChunk ? !depNamedExportsMode : namespaceInteropHelpersByInteropType[interop(moduleId)]) ? namespaceVariableName : moduleVariableName;
	return `${moduleVariableName}${getPropertyAccess(imported)}`;
}
function getEsModuleValue(getObject) {
	return getObject([["value", "true"]], { lineBreakIndent: null });
}
function getNamespaceMarkers(hasNamedExports, addEsModule, addNamespaceToStringTag, { _, getObject }) {
	if (hasNamedExports) {
		if (addEsModule) {
			if (addNamespaceToStringTag) return `Object.defineProperties(exports,${_}${getObject([["__esModule", getEsModuleValue(getObject)], [null, `[Symbol.toStringTag]:${_}${getToStringTagValue(getObject)}`]], { lineBreakIndent: null })});`;
			return `Object.defineProperty(exports,${_}'__esModule',${_}${getEsModuleValue(getObject)});`;
		}
		if (addNamespaceToStringTag) return `Object.defineProperty(exports,${_}Symbol.toStringTag,${_}${getToStringTagValue(getObject)});`;
	}
	return "";
}
const getDefineProperty = (name, needsLiveBinding, t, { _, getDirectReturnFunction, n: n$1 }) => {
	if (needsLiveBinding) {
		const [left, right] = getDirectReturnFunction([], {
			functionReturn: true,
			lineBreakIndent: null,
		...
		return `Object.defineProperty(exports,${_}k,${_}{${n$1}${t}${t}enumerable:${_}true,${n$1}${t}${t}get:${_}${left}${name}[k]${right}${n$1}${t}})`;
	}
...
function getInteropBlock(dependencies, interop, externalLiveBindings, freeze, symbols, accessedGlobals, indent, snippets) {
	...
	for (const { defaultVariableName, imports, importPath, isChunk, name, namedExportsMode, namespaceVariableName, reexports } of dependencies) if (isChunk) {
		for (const { imported, reexported } of [...imports || [], ...reexports || []]) if (imported === "*" && reexported !== "*") {
			if (!namedExportsMode) addInteropStatement(namespaceVariableName, INTEROP_NAMESPACE_DEFAULT_ONLY_VARIABLE, name);
			break;
		}
	} else {
		...
		for (const { imported, reexported } of [...imports || [], ...reexports || []]) {
			...
			if (helper) addInteropStatement(variableName, helper, name);
		}
	}
	return `${getHelpersBlock(neededInteropHelpers, accessedGlobals, indent, snippets, externalLiveBindings, freeze, symbols)}${interopStatements.length > 0 ? `${interopStatements.join(n$1)}${n$1}${n$1}` : ""}`;
}
function throwOnPhase(outputFormat, chunkId, dependencies) {
	...
	if (sourcePhaseDependency) error(logSourcePhaseFormatUnsupported(outputFormat, chunkId, sourcePhaseDependency.importPath));
}
...
function amd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, log, outro, snippets }, { amd: amd$1, esModule, externalLiveBindings, freeze, generatedCode: { symbols }, interop, reexportProtoFromExternal, strict }) {
	warnOnBuiltins(log, dependencies);
	throwOnPhase("amd", id, dependencies);
	const deps = dependencies.map((m) => `'${updateExtensionForRelativeAmdId(m.importPath, amd$1.forceJsExtensionForImports)}'`);
	const parameters = dependencies.map((m) => m.name);
	const { n: n$1, getNonArrowFunctionIntro, _ } = snippets;
	...
	const completeAmdId = getCompleteAmdId(amd$1, id);
	...
	if (namespaceMarkers) namespaceMarkers = n$1 + n$1 + namespaceMarkers;
	magicString.append(`${exportBlock}${namespaceMarkers}${outro}`).indent(t).prepend(`${amd$1.define}(${defineParameters}(${getNonArrowFunctionIntro(parameters, {
		isAsync: false,
		name: null
	})}{${useStrict}${n$1}${n$1}`).append(`${n$1}${n$1}}));`);
}
function cjs(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, outro, snippets }, { compact, esModule, externalLiveBindings, freeze, interop, generatedCode: { symbols }, reexportProtoFromExternal, strict }) {
	...
	const { _, n: n$1 } = snippets;
	const useStrict = strict ? `'use strict';${n$1}${n$1}` : "";
	let namespaceMarkers = getNamespaceMarkers(namedExportsMode && hasExports, isEntryFacade && (esModule === true || esModule === "if-default-prop" && hasDefaultExport), isModuleFacade && symbols, snippets);
	if (namespaceMarkers) namespaceMarkers += n$1 + n$1;
...
function getImportBlock$1(dependencies, { _, cnst, n: n$1 }, compact) {
	let importBlock = "";
	let definingVariable = false;
	for (const { importPath, name, reexports, imports } of dependencies) if (!reexports && !imports) {
		if (importBlock) importBlock += compact && !definingVariable ? "," : `;${n$1}`;
		definingVariable = false;
		importBlock += `require('${importPath}')`;
	} else {
		importBlock += compact && definingVariable ? "," : `${importBlock ? `;${n$1}` : ""}${cnst} `;
		definingVariable = true;
		importBlock += `${name}${_}=${_}require('${importPath}')`;
	}
	if (importBlock) return `${importBlock};${n$1}${n$1}`;
	return "";
}
function es(magicString, { accessedGlobals, indent: t, intro, outro, dependencies, exports, snippets }, { externalLiveBindings, freeze, generatedCode: { symbols }, importAttributesKey }) {
	const { n: n$1 } = snippets;
	const importBlock = getImportBlock(dependencies, importAttributesKey, snippets);
	if (importBlock.length > 0) intro += importBlock.join(n$1) + n$1 + n$1;
	...
	if (exportBlock.length > 0) magicString.append(n$1 + n$1 + exportBlock.join(n$1).trim());
...
function getImportBlock(dependencies, importAttributesKey, { _ }) {
	...
	for (const { importPath, reexports, imports, name, attributes, sourcePhaseImport } of dependencies) {
		...
		if (sourcePhaseImport) importBlock.push(`import source ${sourcePhaseImport} from${_}${pathWithAssertion}`);
		if (!reexports && !imports) {
			if (!sourcePhaseImport) importBlock.push(`import${_}${pathWithAssertion}`);
			continue;
		}
		if (imports) {
			...
			let starImport = null;
			const importedNames = [];
			for (const specifier of imports) if (specifier.imported === "default") defaultImport = specifier;
			else if (specifier.imported === "*") starImport = specifier;
			else importedNames.push(specifier);
			if (starImport) importBlock.push(`import${_}*${_}as ${starImport.local} from${_}${pathWithAssertion}`);
			if (defaultImport && importedNames.length === 0) importBlock.push(`import ${defaultImport.local} from${_}${pathWithAssertion}`);
			else if (importedNames.length > 0) importBlock.push(`import ${defaultImport ? `${defaultImport.local},${_}` : ""}{${_}${importedNames.map((specifier) => specifier.imported === specifier.local ? specifier.imported : `${stringifyIdentifierIfNeeded(specifier.imported)} as ${specifier.local}`).join(`,${_}`)}${_}}${_}from${_}${pathWithAssertion}`);
		}
		if (reexports) {
			let starExport = null;
			const namespaceReexports = [];
			const namedReexports = [];
			for (const specifier of reexports) if (specifier.reexported === "*") starExport = specifier;
			else if (specifier.imported === "*") namespaceReexports.push(specifier);
			else namedReexports.push(specifier);
			if (starExport) importBlock.push(`export${_}*${_}from${_}${pathWithAssertion}`);
			if (namespaceReexports.length > 0) {
				if (!imports || !imports.some((specifier) => specifier.imported === "*" && specifier.local === name)) importBlock.push(`import${_}*${_}as ${name} from${_}${pathWithAssertion}`);
				for (const specifier of namespaceReexports) importBlock.push(`export${_}{${_}${name === specifier.reexported ? name : `${name} as ${stringifyIdentifierIfNeeded(specifier.reexported)}`} };`);
			}
...
function getExportBlock(exports, { _, cnst }) {
	...
	for (const specifier of exports) {
		if (specifier.expression) exportBlock.push(`${cnst} ${specifier.local}${_}=${_}${specifier.expression};`);
	...
	if (exportDeclaration.length > 0) exportBlock.push(`export${_}{${_}${exportDeclaration.join(`,${_}`)}${_}};`);
	return exportBlock;
}
const keypath = (keypath$1, getPropertyAccess) => keypath$1.split(".").map(getPropertyAccess).join("");
function setupNamespace(name, root, globals, { _, getPropertyAccess, s }, compact, log) {
	...
	if (log && isReserved) log(LOGLEVEL_WARN, logReservedNamespace(parts[0]));
...
function assignToDeepVariable(deepName, root, globals, assignment, { _, getPropertyAccess }, log) {
	...
	if (parts.length > 0) deepAssignment = `(${deepAssignment})`;
...
function trimEmptyImports(dependencies) {
	...
	while (index--) {
		...
		if (imports || reexports) return dependencies.slice(0, index + 1);
	}
	return [];
}
function iife(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { compact, esModule, extend, freeze, externalLiveBindings, reexportProtoFromExternal, globals, interop, name, generatedCode: { symbols }, strict }) {
	...
	const { _, getNonArrowFunctionIntro, getPropertyAccess, n: n$1 } = snippets;
	const isNamespaced = name && name.includes(".");
	const useVariableAssignment = !extend && !isNamespaced;
	if (name && useVariableAssignment && !isLegal(name)) return error(logIllegalIdentifierAsName(name));
	warnOnBuiltins(log, dependencies);
	const external = trimEmptyImports(dependencies);
	const deps = external.map((dep) => dep.globalName || "null");
	const parameters = external.map((m) => m.name);
	if (hasExports && !name) log(LOGLEVEL_WARN, logMissingNameOptionForIifeExport());
	if (hasExports && (namedExportsMode || exports[0]?.local === "exports.default")) if (extend) {
	...
	const useStrict = strict ? `${t}'use strict';${n$1}` : "";
	const interopBlock = getInteropBlock(dependencies, interop, externalLiveBindings, freeze, symbols, accessedGlobals, t, snippets);
	...
	if (hasExports) {
		if (name && !(extend && namedExportsMode)) wrapperIntro = (useVariableAssignment ? `var ${name}` : `this${keypath(name, getPropertyAccess)}`) + `${_}=${_}${wrapperIntro}`;
		if (isNamespaced) wrapperIntro = setupNamespace(name, "this", globals, snippets, compact, log) + wrapperIntro;
	}
	let wrapperOutro = `${n$1}${n$1}})(${deps.join(`,${_}`)});`;
	if (hasExports && !extend && namedExportsMode) wrapperOutro = `${n$1}${n$1}${t}return exports;${wrapperOutro}`;
	const exportBlock = getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings, reexportProtoFromExternal);
...
function system(magicString, { accessedGlobals, dependencies, exports, hasExports, id, indent: t, intro, snippets, outro, usesTopLevelAwait }, { externalLiveBindings, freeze, name, generatedCode: { symbols }, strict, systemNullSetters }) {
	...
	const { _, getFunctionIntro, getNonArrowFunctionIntro, n: n$1, s } = snippets;
	...
	let wrapperStart = `System.register(${registeredName}[` + dependencies.map(({ importPath }) => `'${importPath}'`).join(`,${_}`) + `],${_}(${getNonArrowFunctionIntro(wrapperParameters, {
		isAsync: false,
		name: null
	})}{${n$1}${t}${strict ? "'use strict';" : ""}` + getStarExcludesBlock(starExcludes, t, snippets) + getImportBindingsBlock(importBindings, t, snippets) + `${n$1}${t}return${_}{${setters.length > 0 ? `${n$1}${t}${t}setters:${_}[${setters.map((setter) => setter ? `${getFunctionIntro(["module"], {
		isAsync: false,
		name: null
	})}{${n$1}${t}${t}${t}${setter}${n$1}${t}${t}}` : systemNullSetters ? `null` : `${getFunctionIntro([], {
		isAsync: false,
		name: null
	})}{}`).join(`,${_}`)}],` : ""}${n$1}`;
	...
	const wrapperEnd = `${t}${t}})${n$1}${t}}${s}${n$1}}));`;
	magicString.prepend(intro + getHelpersBlock(null, accessedGlobals, t, snippets, externalLiveBindings, freeze, symbols) + getHoistedExportsBlock(exports, t, snippets)).append(`${outro}${n$1}${n$1}` + getSyntheticExportsBlock(exports, t, snippets) + getMissingExportsBlock(exports, t, snippets)).indent(`${t}${t}${t}`).append(wrapperEnd).prepend(wrapperStart);
}
function analyzeDependencies(dependencies, exports, t, { _, cnst, getObject, getPropertyAccess, n: n$1 }) {
	const importBindings = [];
	const setters = [];
	let starExcludes = null;
	for (const { imports, reexports } of dependencies) {
		const setter = [];
		if (imports) for (const specifier of imports) {
			importBindings.push(specifier.local);
			if (specifier.imported === "*") setter.push(`${specifier.local}${_}=${_}module;`);
			else setter.push(`${specifier.local}${_}=${_}module${getPropertyAccess(specifier.imported)};`);
		}
		if (reexports) {
			const reexportedNames = [];
			let hasStarReexport = false;
			for (const { imported, reexported } of reexports) if (reexported === "*") hasStarReexport = true;
			else reexportedNames.push([reexported, imported === "*" ? "module" : `module${getPropertyAccess(imported)}`]);
			if (reexportedNames.length > 1 || hasStarReexport) if (hasStarReexport) {
				if (!starExcludes) starExcludes = getStarExcludes({
					dependencies,
					exports
				});
		...
		setters.push(setter.join(`${n$1}${t}${t}${t}`));
	}
	return {
		importBindings,
		setters,
...
const getStarExcludes = ({ dependencies, exports }) => {
	...
	for (const { reexports } of dependencies) if (reexports) {
		for (const reexport of reexports) if (reexport.reexported !== "*") starExcludes.add(reexport.reexported);
	}
	return starExcludes;
};
const getStarExcludesBlock = (starExcludes, t, { _, cnst, getObject, n: n$1 }) => {
	if (starExcludes) {
		const fields = [...starExcludes].map((property) => [property, "1"]);
		fields.unshift([null, `__proto__:${_}null`]);
		return `${n$1}${t}${cnst} _starExcludes${_}=${_}${getObject(fields, { lineBreakIndent: {
			base: t,
			t
		} })};`;
	}
...
const getImportBindingsBlock = (importBindings, t, { _, n: n$1 }) => importBindings.length > 0 ? `${n$1}${t}var ${importBindings.join(`,${_}`)};` : "";
const getHoistedExportsBlock = (exports, t, snippets) => getExportsBlock(exports.filter((expt) => expt.hoisted).map((expt) => ({
	name: expt.exported,
	value: expt.local
})), t, snippets);
function getExportsBlock(exports, t, { _, n: n$1 }) {
	if (exports.length === 0) return "";
	if (exports.length === 1) return `exports(${JSON.stringify(exports[0].name)},${_}${exports[0].value});${n$1}${n$1}`;
	return `exports({${n$1}` + exports.map(({ name, value }) => `${t}${stringifyObjectKeyIfNeeded(name)}:${_}${value}`).join(`,${n$1}`) + `${n$1}});${n$1}${n$1}`;
}
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { amd: amd$1, compact, esModule, extend, externalLiveBindings, freeze, interop, name, generatedCode: { symbols }, globals, noConflict, reexportProtoFromExternal, strict }) {
	const { _, cnst, getFunctionIntro, getNonArrowFunctionIntro, getPropertyAccess, n: n$1, s } = snippets;
	const factoryVariable = compact ? "f" : "factory";
	const globalVariable = compact ? "g" : "global";
	if (hasExports && !name) return error(logMissingNameOptionForUmdExport());
	throwOnPhase("umd", id, dependencies);
	warnOnBuiltins(log, dependencies);
	const amdDeps = dependencies.map((m) => `'${updateExtensionForRelativeAmdId(m.importPath, amd$1.forceJsExtensionForImports)}'`);
	...
	const define = amd$1.define;
	const cjsExport = !namedExportsMode && hasExports ? `module.exports${_}=${_}` : ``;
	const useStrict = strict ? `${_}'use strict';${n$1}` : ``;
	let iifeExport;
	if (noConflict) {
		const noConflictExportsVariable = compact ? "e" : "exports";
		let factory;
		if (!namedExportsMode && hasExports) factory = `${cnst} ${noConflictExportsVariable}${_}=${_}${assignToDeepVariable(name, globalVariable, globals, `${factoryVariable}(${globalDeps.join(`,${_}`)})`, snippets, log)};`;
		else {
			const module = globalDeps.shift();
			factory = `${cnst} ${noConflictExportsVariable}${_}=${_}${module};${n$1}${t}${t}${factoryVariable}(${[noConflictExportsVariable, ...globalDeps].join(`,${_}`)});`;
		}
		iifeExport = `(${getFunctionIntro([], {
			isAsync: false,
			name: null
		})}{${n$1}${t}${t}${cnst} current${_}=${_}${safeAccess(name, globalVariable, snippets)};${n$1}${t}${t}${factory}${n$1}${t}${t}${noConflictExportsVariable}.noConflict${_}=${_}${getFunctionIntro([], {
			isAsync: false,
			name: null
		})}{${_}${globalProperty(name, globalVariable, getPropertyAccess)}${_}=${_}current;${_}return ${noConflictExportsVariable}${s}${_}};${n$1}${t}})()`;
	} else {
		iifeExport = `${factoryVariable}(${globalDeps.join(`,${_}`)})`;
		if (!namedExportsMode && hasExports) iifeExport = assignToDeepVariable(name, globalVariable, globals, iifeExport, snippets, log);
	}
	const iifeNeedsGlobal = hasExports || noConflict && namedExportsMode || globalDeps.length > 0;
	const wrapperParameters = [factoryVariable];
	if (iifeNeedsGlobal) wrapperParameters.unshift(globalVariable);
	...
	const cjsIntro = iifeNeedsGlobal ? `${t}typeof exports${_}===${_}'object'${_}&&${_}typeof module${_}!==${_}'undefined'${_}?${_}${cjsExport}${factoryVariable}(${cjsDeps.join(`,${_}`)})${_}:${n$1}` : "";
	const wrapperIntro = `(${getNonArrowFunctionIntro(wrapperParameters, {
		isAsync: false,
		name: null
	})}{${n$1}` + cjsIntro + `${t}typeof ${define}${_}===${_}'function'${_}&&${_}${define}.amd${_}?${_}${define}(${amdParameters}${factoryVariable})${_}:${n$1}${t}${iifeStart}${iifeExport}${iifeEnd};${n$1}})(${globalArgument}(${getNonArrowFunctionIntro(factoryParameters, {
		isAsync: false,
		name: null
	})}{${useStrict}${n$1}`;
	const wrapperOutro = n$1 + n$1 + "}));";
	magicString.prepend(`${intro}${getInteropBlock(dependencies, interop, externalLiveBindings, freeze, symbols, accessedGlobals, t, snippets)}`);
	const exportBlock = getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings, reexportProtoFromExternal);
	let namespaceMarkers = getNamespaceMarkers(namedExportsMode && hasExports, esModule === true || esModule === "if-default-prop" && hasDefaultExport, symbols, snippets);
	if (namespaceMarkers) namespaceMarkers = n$1 + n$1 + namespaceMarkers;
...
function getAugmentedNamespace(n$1) {
	if (Object.prototype.hasOwnProperty.call(n$1, "__esModule")) return n$1;
	var f = n$1.default;
	if (typeof f == "function") {
		var a = function a$1() {
			var isInstance = false;
			try {
				isInstance = this instanceof a$1;
			} catch (e) {}
	...
	Object.keys(n$1).forEach(function(k) {
		var d = Object.getOwnPropertyDescriptor(n$1, k);
...
function requireScan() {
	...
	hasRequiredScan = 1;
	const utils$1 = /* @__PURE__ */ requireUtils();
	...
	*/
	const scan = (input, options) => {
		...
		if (base$1 && isGlob === true && lastIndex > 0) {
			base$1 = str.slice(0, lastIndex);
			glob = str.slice(lastIndex);
		} else if (isGlob === true) {
			base$1 = "";
			glob = str;
		} else base$1 = str;
		if (base$1 && base$1 !== "" && base$1 !== "/" && base$1 !== str) {
		...
		if (opts.unescape === true) {
			...
			if (base$1 && backslashes === true) base$1 = utils$1.removeBackslashes(base$1);
		}
		const state = {
			prefix,
			input,
...
function requireParse() {
	...
	hasRequiredParse = 1;
	const constants$2 = /* @__PURE__ */ requireConstants();
	const utils$1 = /* @__PURE__ */ requireUtils();
	/**
	* Constants
	*/
	...
	*/
	...
	const getStarExtglobSequenceOutput = (pattern) => {
		...
		if (chars$2.length < 1) return;
		const source = chars$2.length === 1 ? utils$1.escapeRegex(chars$2[0]) : `[${chars$2.map((ch) => utils$1.escapeRegex(ch)).join("")}]`;
		return `${source}*`;
	};
	...
	const analyzeRepeatedExtglob = (body, options) => {
		...
		const max = typeof options.maxExtglobRecursion === "number" ? options.maxExtglobRecursion : constants$2.DEFAULT_MAX_EXTGLOB_RECURSION;
	...
	const parse = (input, options) => {
		...
		*/
		...
		const extglobClose = (token) => {
			...
			if ((token.type === "plus" || token.type === "star") && analysis.risky) {
				...
				open.type = "text";
				open.value = literal;
				open.output = safeOutput || utils$1.escapeRegex(literal);
		...
		*/
		if (opts.fastpaths !== false && !/(^[*!]|[/()[\]{}"])/.test(input)) {
			let backslashes = false;
			let output = input.replace(REGEX_SPECIAL_CHARS_BACKREF, (m, esc, chars$2, first, rest, index) => {
				if (first === "\\") {
					backslashes = true;
					return m;
				}
				if (first === "?") {
					...
					return QMARK.repeat(chars$2.length);
				}
				if (first === ".") return DOT_LITERAL.repeat(chars$2.length);
				if (first === "*") {
				...
				if (opts.posix !== false && value === ":") {
					...
					if (inner.includes("[")) {
						...
						if (inner.includes(":")) {
							...
							const posix$2 = POSIX_REGEX_SOURCE[rest$1];
							if (posix$2) {
								prev.value = pre + posix$2;
								state.backtrack = true;
								advance();
	...
	*/
	parse.fastpaths = (input, options) => {
		...
		if (len > max) throw new SyntaxError(`Input length: ${len}, exceeds maximum allowed length: ${max}`);
		input = REPLACEMENTS[input] || input;
		const { DOT_LITERAL, SLASH_LITERAL, ONE_CHAR, DOTS_SLASH, NO_DOT, NO_DOTS, NO_DOTS_SLASH, STAR, START_ANCHOR } = constants$2.globChars(opts.windows);
...
function requirePicomatch$1() {
	...
	* @api public
	*/
	picomatch$1.test = (input, regex, options, { glob, posix: posix$2 } = {}) => {
		...
		const opts = options || {};
		const format = opts.format || (posix$2 ? utils$1.toPosixSlashes : null);
		...
		if (match === false || opts.capture === true) if (opts.matchBase === true || opts.basename === true) match = picomatch$1.matchBase(input, regex, options, posix$2);
...
function requirePicomatch() {
	...
	function picomatch$1(glob, options, returnState = false) {
		if (options && (options.windows === null || options.windows === void 0)) options = {
			...options,
			windows: utils$1.isWindows()
		};
		return pico(glob, options, returnState);
	}
	Object.assign(picomatch$1, pico);
	picomatch_1 = picomatch$1;
	return picomatch_1;
}
...
const createFilter$1 = function createFilter$2(include, exclude, options) {
...
var ArrayPattern = class extends NodeBase {
	addExportedVariables(variables, exportNamesByVariable) {
		for (const element of this.elements) element?.addExportedVariables(variables, exportNamesByVariable);
	}
	declare(kind, destructuredInitPath, init) {
		const variables = [];
		const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
		for (const element of this.elements) if (element !== null) variables.push(...element.declare(kind, includedPatternPath, init));
		return variables;
	}
	deoptimizeAssignment(destructuredInitPath, init) {
		const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
		for (const element of this.elements) element?.deoptimizeAssignment(includedPatternPath, init);
	}
	deoptimizePath() {
		for (const element of this.elements) element?.deoptimizePath(EMPTY_PATH);
	}
	hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
		const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
		for (const element of this.elements) if (element?.hasEffectsWhenDestructuring(context, includedPatternPath, init)) return true;
		return false;
	}
	hasEffectsOnInteractionAtPath(_path, interaction, context) {
		for (const element of this.elements) if (element?.hasEffectsOnInteractionAtPath(EMPTY_PATH, interaction, context)) return true;
		return false;
	}
	includeDestructuredIfNecessary(context, destructuredInitPath, init) {
		let included = false;
		const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
		for (const element of [...this.elements].reverse()) if (element) {
			if (included && !element.included) element.includeNode(context);
	...
	markDeclarationReached() {
		for (const element of this.elements) element?.markDeclarationReached();
	}
};
ArrayPattern.prototype.includeNode = onlyIncludeSelf;
const getIncludedPatternPath = (destructuredInitPath) => destructuredInitPath.at(-1) === UnknownKey ? destructuredInitPath : [...destructuredInitPath, UnknownInteger];
var ArrowFunctionExpression$1 = class extends FunctionBase {
	constructor() {
		super(...arguments);
		this.objectEntity = null;
	}
	...
	include(context, includeChildrenRecursively) {
		super.include(context, includeChildrenRecursively);
		for (const parameter of this.params) if (!(parameter instanceof Identifier$1)) parameter.include(context, includeChildrenRecursively);
	}
	includeNode(context) {
		this.included = true;
		this.body.includePath(UNKNOWN_PATH, context);
		for (const parameter of this.params) if (!(parameter instanceof Identifier$1)) parameter.includePath(UNKNOWN_PATH, context);
	}
	getObjectEntity() {
		if (this.objectEntity !== null) return this.objectEntity;
...
var ObjectPattern = class extends NodeBase {
	addExportedVariables(variables, exportNamesByVariable) {
		for (const property of this.properties) if (property.type === Property) property.value.addExportedVariables(variables, exportNamesByVariable);
		else property.argument.addExportedVariables(variables, exportNamesByVariable);
	}
	declare(kind, destructuredInitPath, init) {
		const variables = [];
		for (const property of this.properties) variables.push(...property.declare(kind, destructuredInitPath, init));
		return variables;
	}
	deoptimizeAssignment(destructuredInitPath, init) {
		for (const property of this.properties) property.deoptimizeAssignment(destructuredInitPath, init);
	}
	deoptimizePath(path$2) {
		if (path$2.length === 0) for (const property of this.properties) property.deoptimizePath(path$2);
	}
	hasEffectsOnInteractionAtPath(_path, interaction, context) {
		for (const property of this.properties) if (property.hasEffectsOnInteractionAtPath(EMPTY_PATH, interaction, context)) return true;
		return false;
	}
	hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
		for (const property of this.properties) if (property.hasEffectsWhenDestructuring(context, destructuredInitPath, init)) return true;
		return false;
	}
	includeDestructuredIfNecessary(context, destructuredInitPath, init) {
		...
		for (const property of this.properties.slice(0, -1)) {
			if (lastPropertyIsRestElement && included && !property.included) property.includeNode(context);
			included = property.includeDestructuredIfNecessary(context, destructuredInitPath, init) || included;
		}
		if (!this.included && included) this.includeNode(context);
		return this.included;
	}
	markDeclarationReached() {
		for (const property of this.properties) property.markDeclarationReached();
	}
	render(code, options) {
		if (this.properties.length > 0) {
			...
			if (lastSeparatorPos) code.remove(lastSeparatorPos, this.end - 1);
		}
	}
};
...
var AssignmentExpression = class extends NodeBase {
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return interaction.type === INTERACTION_ASSIGNED && this.left.included || this.right.hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	include(context, includeChildrenRecursively) {
		...
		if (includeChildrenRecursively || isConstReassignment || operator !== "=" || left.included || left.hasEffectsAsAssignmentTarget(hasEffectsContext, false) || left.hasEffectsWhenDestructuring?.(hasEffectsContext, EMPTY_PATH, right)) left.includeAsAssignmentTarget(context, includeChildrenRecursively, operator !== "=");
	...
	initialise() {
		super.initialise();
		if (this.left instanceof Identifier$1) {
	...
	render(code, options, { preventASI, renderedParentType, renderedSurroundingElement } = BLANK) {
		...
		if (left.included) {
			...
			if (preventASI) removeLineBreaks(code, inclusionStart, right.start);
			right.render(code, options, {
				renderedParentType: renderedParentType || parent.type,
				renderedSurroundingElement: renderedSurroundingElement || parent.type
			});
		}
		if (options.format === "system") if (left instanceof Identifier$1) {
			const variable = left.variable;
			const exportNames = options.exportNamesByVariable.get(variable);
			if (exportNames) {
				if (exportNames.length === 1) renderSystemExportExpression(variable, start, end, code, options);
				else renderSystemExportSequenceAfterExpression(variable, start, end, parent.type !== ExpressionStatement, code, options);
				return;
			}
		} else {
...
var AssignmentPattern = class extends NodeBase {
	...
	deoptimizePath(path$2) {
		if (path$2.length === 0) this.left.deoptimizePath(path$2);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return path$2.length > 0 || this.left.hasEffectsOnInteractionAtPath(EMPTY_PATH, interaction, context);
	}
	hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
...
var AwaitExpression$1 = class extends NodeBase {
	deoptimizePath(path$2) {
		this.argument.deoptimizePath(path$2);
	}
	hasEffects() {
		if (!this.deoptimized) this.applyDeoptimizations();
		return true;
	...
	includePath(path$2, context) {
		if (!this.deoptimized) this.applyDeoptimizations();
		if (!this.included) this.includeNode(context);
		this.argument.includePath(path$2, context);
	}
};
...
function getRenderedLiteralValue(value) {
	...
	if (typeof value === "number") return getSimplifiedNumber(value);
	return UnknownValue;
}
function getSimplifiedNumber(value) {
	if (Object.is(-0, value)) return "-0";
	const exp = value.toExponential();
	const [base$1, exponent] = exp.split("e");
	const floatLength = base$1.split(".")[1]?.length || 0;
	const finalizedExp = `${base$1.replace(".", "")}e${parseInt(exponent) - floatLength}`;
...
var BinaryExpression = class extends NodeBase {
	...
	getRenderedLiteralValue() {
		if (this.operator !== "in" || !(this.right.variable instanceof NamespaceVariable)) return UnknownValue;
		if (this.renderedLiteralValue !== UNASSIGNED$1) return this.renderedLiteralValue;
	...
	includeNode(context) {
		this.included = true;
		if (this.operator === "in" && typeof this.getRenderedLiteralValue() === "symbol") this.right.includePath(UNKNOWN_PATH, context);
	}
	removeAnnotations(code) {
	...
	render(code, options, { renderedSurroundingElement } = BLANK) {
		...
		if (typeof renderedLiteralValue !== "symbol") code.overwrite(this.start, this.end, renderedLiteralValue);
		else {
...
function renderCallArguments(code, options, node) {
	if (node.arguments.length > 0) if (node.arguments[node.arguments.length - 1].included) for (const argument of node.arguments) argument.render(code, options);
	else {
		let lastIncludedIndex = node.arguments.length - 2;
		while (lastIncludedIndex >= 0 && !node.arguments[lastIncludedIndex].included) lastIncludedIndex--;
		if (lastIncludedIndex >= 0) {
			for (let index = 0; index <= lastIncludedIndex; index++) node.arguments[index].render(code, options);
			code.remove(findFirstOccurrenceOutsideComment(code.original, ",", node.arguments[lastIncludedIndex].end), node.end - 1);
		} else code.remove(findFirstOccurrenceOutsideComment(code.original, "(", node.callee.end) + 1, node.end - 1);
	}
}
var CallExpressionBase = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		...
		if (returnExpression === UNKNOWN_EXPRESSION) for (const expression of deoptimizedExpressions) expression.deoptimizePath(UNKNOWN_PATH);
		else recursionTracker.withTrackedEntityAtPath(path$2, returnExpression, () => {
			for (const expression of deoptimizedExpressions) this.expressionsToBeDeoptimized.add(expression);
			returnExpression.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
		}, null);
	}
	deoptimizeCache() {
		if (this.returnExpression?.[0] !== UNKNOWN_EXPRESSION) {
			...
			for (const expression of deoptimizableDependentExpressions) expression.deoptimizeCache();
			for (const expression of expressionsToBeDeoptimized) expression.deoptimizePath(UNKNOWN_PATH);
		}
	}
	deoptimizePath(path$2) {
		if (path$2.length === 0 || this.scope.context.deoptimizationTracker.trackEntityAtPathAndGetIfTracked(path$2, this)) return;
		const [returnExpression] = this.getReturnExpression();
		if (returnExpression !== UNKNOWN_EXPRESSION) returnExpression.deoptimizePath(path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		const [returnExpression] = this.getReturnExpression(recursionTracker);
		if (returnExpression === UNKNOWN_EXPRESSION) return UnknownValue;
		return recursionTracker.withTrackedEntityAtPath(path$2, returnExpression, () => {
			this.deoptimizableDependentExpressions.push(origin);
			return returnExpression.getLiteralValueAtPath(path$2, recursionTracker, origin);
		}, UnknownValue);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		const returnExpression = this.getReturnExpression(recursionTracker);
		if (returnExpression[0] === UNKNOWN_EXPRESSION) return returnExpression;
		return recursionTracker.withTrackedEntityAtPath(path$2, returnExpression, () => {
			this.deoptimizableDependentExpressions.push(origin);
			const [expression, isPure] = returnExpression[0].getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		...
		if (type === INTERACTION_CALLED) {
			...
			if ((withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path$2, args, this)) return false;
		} else if ((type === INTERACTION_ASSIGNED ? context.assigned : context.accessed).trackEntityAtPathAndGetIfTracked(path$2, this)) return false;
		const [returnExpression, isPure] = this.getReturnExpression();
		return (type === INTERACTION_ASSIGNED || !isPure) && returnExpression.hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
};
var CallExpression$1 = class extends CallExpressionBase {
	...
	bind() {
		super.bind();
		this.interaction = {
			args: [this.callee instanceof MemberExpression$1 && !this.callee.variable ? this.callee.object : null, ...this.arguments],
			type: INTERACTION_CALLED,
			withNew: false
		};
	}
	getLiteralValueAtPathAsChainElement(path$2, recursionTracker, origin) {
		return getChainElementLiteralValueAtPath(this, this.callee, path$2, recursionTracker, origin);
	}
	hasEffects(context) {
		if (!this.deoptimized) this.applyDeoptimizations();
		for (const argument of this.arguments) if (argument.hasEffects(context)) return true;
		if (this.annotationPure) return false;
	...
	hasEffectsAsChainElement(context) {
		...
		if (this.optional && this.callee.getLiteralValueAtPath(EMPTY_PATH, SHARED_RECURSION_TRACKER, this) == null) return !this.annotationPure && calleeHasEffects || IS_SKIPPED_CHAIN;
		if (!this.deoptimized) this.applyDeoptimizations();
		for (const argument of this.arguments) if (argument.hasEffects(context)) return true;
	...
	include(context, includeChildrenRecursively) {
		...
		if (includeChildrenRecursively) {
			...
			if (includeChildrenRecursively === INCLUDE_PARAMETERS && this.callee instanceof Identifier$1 && this.callee.variable) this.callee.variable.markCalledFromTryStatement();
		} else {
	...
	initialise() {
		super.initialise();
		if (this.annotations && this.scope.context.options.treeshake.annotations) this.annotationPure = this.annotations.some((comment) => comment.type === "pure");
	}
	render(code, options, { renderedSurroundingElement } = BLANK) {
		...
		if (this.callee instanceof Identifier$1 && !this.hasCheckedForWarnings) {
			this.hasCheckedForWarnings = true;
			const variable = this.scope.findVariable(this.callee.name);
			if (variable.isNamespace) this.scope.context.log(LOGLEVEL_WARN, logCannotCallNamespace(this.callee.name), this.start);
			if (this.callee.name === "eval") this.scope.context.log(LOGLEVEL_WARN, logEval(this.scope.context.module.id), this.start);
		}
	}
	applyDeoptimizations() {
		this.deoptimized = true;
...
var CatchClause$1 = class extends NodeBase {
	createScope(parentScope) {
	...
	parseNode(esTreeNode) {
		...
		this.body = new BlockStatement$1(this, this.scope.bodyScope).parseNode(body);
		return super.parseNode(esTreeNode);
	}
};
CatchClause$1.prototype.preventChildBlockScope = true;
CatchClause$1.prototype.includeNode = onlyIncludeSelf;
var ChainExpression = class extends NodeBase {
	deoptimizeCache() {}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		const literalValue = this.expression.getLiteralValueAtPathAsChainElement(path$2, recursionTracker, origin);
	...
	includePath(path$2, context) {
		this.included = true;
		this.expression.includePath(path$2, context);
	}
	removeAnnotations(code) {
		this.expression.removeAnnotations(code);
	}
...
var ClassBody = class extends NodeBase {
	...
	include(context, includeChildrenRecursively) {
		...
		for (const definition of this.body) definition.include(context, includeChildrenRecursively);
	}
	parseNode(esTreeNode) {
		const body = this.body = new Array(esTreeNode.body.length);
		let index = 0;
		for (const definition of esTreeNode.body) body[index++] = new (this.scope.context.getNodeConstructor(definition.type))(this, definition.static ? this.scope : this.scope.instanceScope).parseNode(definition);
		return super.parseNode(esTreeNode);
	}
};
...
function tryCastLiteralValueToBoolean(literalValue) {
	if (typeof literalValue === "symbol") {
		if (literalValue === UnknownFalsyValue) return false;
		if (literalValue === UnknownTruthyValue) return true;
		return UnknownValue;
	}
	return !!literalValue;
}
var MultiExpression = class MultiExpression extends ExpressionEntity {
	constructor(expressions) {
		super();
		this.expressions = expressions;
	}
	deoptimizePath(path$2) {
		for (const expression of this.expressions) expression.deoptimizePath(path$2);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		return [new MultiExpression(this.expressions.map((expression) => expression.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin)[0])), false];
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		for (const expression of this.expressions) if (expression.hasEffectsOnInteractionAtPath(path$2, interaction, context)) return true;
		return false;
	}
};
var ConditionalExpression = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.consequent.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
		this.alternate.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizeCache() {
		...
		if (this.usedBranch !== null) {
			...
			if (this.included) unusedBranch.includePath(UNKNOWN_PATH, createInclusionContext());
	...
	deoptimizePath(path$2) {
		...
		if (usedBranch) usedBranch.deoptimizePath(path$2);
		else {
			this.consequent.deoptimizePath(path$2);
			this.alternate.deoptimizePath(path$2);
		}
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		const usedBranch = this.getUsedBranch();
		if (!usedBranch) {
			if (this.hasDeoptimizedCache) return UnknownValue;
			const consequentValue = this.consequent.getLiteralValueAtPath(path$2, recursionTracker, origin);
			...
			const alternateValue = this.alternate.getLiteralValueAtPath(path$2, recursionTracker, origin);
			...
			this.expressionsToBeDeoptimized.push(origin);
			if (consequentValue !== alternateValue) return castedConsequentValue ? UnknownTruthyValue : UnknownFalsyValue;
			return consequentValue;
		}
		this.expressionsToBeDeoptimized.push(origin);
		return usedBranch.getLiteralValueAtPath(path$2, recursionTracker, origin);
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		const usedBranch = this.getUsedBranch();
		if (!usedBranch) return [new MultiExpression([this.consequent.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin)[0], this.alternate.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin)[0]]), false];
		this.expressionsToBeDeoptimized.push(origin);
		return usedBranch.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin);
	}
	hasEffects(context) {
		if (this.test.hasEffects(context)) return true;
		const usedBranch = this.getUsedBranch();
		if (!usedBranch) return this.consequent.hasEffects(context) || this.alternate.hasEffects(context);
		return usedBranch.hasEffects(context);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		const usedBranch = this.getUsedBranch();
		if (!usedBranch) return this.consequent.hasEffectsOnInteractionAtPath(path$2, interaction, context) || this.alternate.hasEffectsOnInteractionAtPath(path$2, interaction, context);
		return usedBranch.hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	include(context, includeChildrenRecursively) {
		this.included = true;
		const usedBranch = this.getUsedBranch();
		...
		} else usedBranch.include(context, includeChildrenRecursively);
	}
	includePath(path$2, context) {
		this.included = true;
		const usedBranch = this.getUsedBranch();
		if (usedBranch === null || this.test.shouldBeIncluded(context)) {
			this.consequent.includePath(path$2, context);
			this.alternate.includePath(path$2, context);
		} else usedBranch.includePath(path$2, context);
	}
	includeCallArguments(interaction, context) {
		const usedBranch = this.getUsedBranch();
		if (usedBranch) usedBranch.includeCallArguments(interaction, context);
		else {
	...
	render(code, options, { isCalleeOfRenderedParent, preventASI, renderedParentType, renderedSurroundingElement } = BLANK) {
		if (this.test.included) {
			...
			if (preventASI) removeLineBreaks(code, inclusionStart, usedBranch.start);
			code.remove(this.start, inclusionStart);
			if (this.consequent.included) code.remove(colonPos, this.end);
			this.test.removeAnnotations(code);
...
var ExportNamedDeclaration = class extends NodeBase {
	...
	render(code, options, nodeRenderOptions) {
		const { start, end } = nodeRenderOptions;
		if (this.declaration === null) code.remove(start, end);
		else {
			...
			if (this.declaration instanceof ClassDeclaration) {
				...
				for (const decorator of decorators) endBoundary = Math.min(endBoundary, decorator.start);
				if (endBoundary <= this.start) endBoundary = this.declaration.start;
			}
...
var ForInStatement = class extends NodeBase {
	...
	render(code, options) {
		...
		if (code.original.charCodeAt(this.right.start - 1) === 110) code.prependLeft(this.right.start, " ");
...
var ForOfStatement = class extends NodeBase {
	...
	render(code, options) {
		this.left.render(code, options, NO_SEMICOLON);
		this.right.render(code, options, NO_SEMICOLON);
		if (code.original.charCodeAt(this.right.start - 1) === 102) code.prependLeft(this.right.start, " ");
		this.body.render(code, options);
	}
	applyDeoptimizations() {
		this.deoptimized = true;
...
var FunctionExpression$1 = class extends FunctionNode {
	createScope(parentScope) {
		super.createScope(this.idScope = new ChildScope(parentScope, parentScope.context));
	}
	parseNode(esTreeNode) {
		if (esTreeNode.id !== null) this.id = new Identifier$1(this, this.idScope).parseNode(esTreeNode.id);
		return super.parseNode(esTreeNode);
	}
	onlyFunctionCallUsed() {
...
var IfStatement = class IfStatement extends NodeBase {
	constructor() {
		super(...arguments);
	...
	include(context, includeChildrenRecursively) {
		this.included = true;
		if (includeChildrenRecursively) this.includeRecursively(includeChildrenRecursively, context);
		else {
			const testValue = this.getTestValue();
			if (typeof testValue === "symbol") this.includeUnknownTest(context);
			else this.includeKnownTest(context, testValue);
		}
	}
	parseNode(esTreeNode) {
		...
		if (esTreeNode.alternate) this.alternate = new (this.scope.context.getNodeConstructor(esTreeNode.alternate.type))(this, this.alternateScope = new TrackingScope(this.scope)).parseNode(esTreeNode.alternate);
		return super.parseNode(esTreeNode);
	}
	render(code, options) {
		const { snippets: { getPropertyAccess } } = options;
		...
		if (includesIfElse) this.test.render(code, options);
		else code.remove(this.start, this.consequent.start);
		if (this.consequent.included && (noTreeshake || typeof testValue === "symbol" || testValue)) this.consequent.render(code, options);
		else {
		...
		if (this.alternate) if (this.alternate.included && (noTreeshake || typeof testValue === "symbol" || !testValue)) {
			if (includesIfElse) {
				if (code.original.charCodeAt(this.alternate.start - 1) === 101) code.prependLeft(this.alternate.start, " ");
			} else code.remove(this.consequent.end, this.alternate.start);
			this.alternate.render(code, options);
		} else {
			if (includesIfElse && this.shouldKeepAlternateBranch()) code.overwrite(this.alternate.start, this.end, ";");
			else code.remove(this.consequent.end, this.end);
			hoistedDeclarations.push(...this.alternateScope.hoistedDeclarations);
		}
		this.renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess);
	}
	getTestValue() {
		if (this.testValue === unset) return this.testValue = tryCastLiteralValueToBoolean(this.test.getLiteralValueAtPath(EMPTY_PATH, SHARED_RECURSION_TRACKER, this));
		return this.testValue;
	}
	includeKnownTest(context, testValue) {
		if (this.test.shouldBeIncluded(context)) this.test.include(context, false);
		if (testValue && this.consequent.shouldBeIncluded(context)) this.consequent.include(context, false, { asSingleStatement: true });
		if (!testValue && this.alternate?.shouldBeIncluded(context)) this.alternate.include(context, false, { asSingleStatement: true });
	}
	...
	renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess) {
		...
		if (hoistedVariables) {
			...
			if (needsBraces) code.appendLeft(this.end, ` }`);
		}
	}
	shouldKeepAlternateBranch() {
		let currentParent = this.parent;
		do {
			if (currentParent instanceof IfStatement && currentParent.alternate) return true;
			if (currentParent instanceof BlockStatement$1) return false;
...
var ObjectPromiseHandler = class {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		deoptimizeInteraction(interaction);
		if (interaction.type === INTERACTION_CALLED && path$2.length === 0 && (isFunctionExpressionNode(interaction.args[1]) || isArrowFunctionExpressionNode(interaction.args[1]))) interaction.args[1].deoptimizeArgumentsOnInteractionAtPath(this.interaction, [], recursionTracker);
	}
	includeCallArguments(interaction, context) {
		includeInteractionWithoutThis(interaction, context);
		if (interaction.type === INTERACTION_CALLED && (isFunctionExpressionNode(interaction.args[1]) || isArrowFunctionExpressionNode(interaction.args[1]))) interaction.args[1].includeCallArguments(this.interaction, context);
	}
};
...
var ImportExpression = class extends NodeBase {
	...
	bind() {
		...
		source.bind();
		options?.bind();
		if (typeof resolution !== "object" || !resolution || !("namespace" in resolution)) return;
		...
		while (true) {
			if (currentParent.computed || currentParent.object !== callExpression || !isIdentifierNode(currentParent.property) || !isCallExpressionNode(currentParent.parent)) break;
	...
	deoptimizePath(path$2) {
		this.localResolution?.resolution?.namespace.deoptimizePath(path$2);
	}
	hasEffects() {
		return true;
	}
	include(context, includeChildrenRecursively) {
		...
		if (this.shouldIncludeDynamicAttributes) this.options?.include(context, includeChildrenRecursively);
		if (includeChildrenRecursively) this.localResolution?.resolution.includeAllExports();
	}
	includeNode(context) {
		this.included = true;
		const { localResolution, scope, shouldIncludeDynamicAttributes } = this;
		if (shouldIncludeDynamicAttributes) this.options?.includePath(UNKNOWN_PATH, context);
		scope.context.includeDynamicImport(this);
		scope.addAccessedDynamicImport(this);
		if (localResolution) if (localResolution.tracked) localResolution.resolution.includeModuleInExecution();
		else localResolution.resolution.includeAllExports();
	}
	includePath(path$2, context) {
		if (!this.included) this.includeNode(context);
		this.localResolution?.resolution?.namespace.includeMemberPath(path$2, context);
	}
	initialise() {
		super.initialise();
	...
	render(code, options) {
		...
		} else this.source.render(code, options);
		if (this.attributes !== true) {
			if (this.options) code.overwrite(this.source.end, this.end - 1, "", { contentOnly: true });
			if (this.attributes) code.appendLeft(this.end - 1, `,${_}${getObject([[importAttributesKey, this.attributes]], { lineBreakIndent: null })}`);
		}
	}
	setExternalResolution(exportMode, options, snippets, pluginDriver, accessedGlobalsByScope, resolutionString, namespaceExportName, attributes, ownChunk, targetChunk) {
		...
		if (helper) accessedGlobals.push(helper);
		if (accessedGlobals.length > 0) this.scope.addAccessedGlobals(accessedGlobals, accessedGlobalsByScope);
	}
	...
	getDynamicImportMechanismAndHelper(exportMode, { compact, dynamicImportInCjs, format, generatedCode: { arrowFunctions }, interop }, { _, getDirectReturnFunction, getDirectReturnIifeLeft }, pluginDriver, ownChunk, targetChunk) {
		...
		const mechanism = pluginDriver.hookFirstSync("renderDynamicImport", [{
			...
			getTargetChunkImports() {
				...
				for (const dep of targetChunk.dependencies) {
					...
					if (dep instanceof ExternalChunk) chunkInfos.push({
						fileName: dep.getFileName(),
						resolvedImportPath,
						type: "external"
					});
					else chunkInfos.push({
		...
		switch (format) {
			case "cjs": {
				if (dynamicImportInCjs && (!resolution || typeof resolution === "string" || resolution instanceof ExternalModule)) return {
					helper: null,
					mechanism: null
				};
			...
			case "amd": {
				const resolve$2 = compact ? "c" : "resolve";
				...
				const resolveNamespace = helper ? `${resolveLeft}${resolve$2}(/*#__PURE__*/${helper}(m))${resolveRight}` : resolve$2;
				const [handlerLeft, handlerRight] = getDirectReturnFunction([resolve$2, reject], {
					functionReturn: false,
					lineBreakIndent: null,
					name: null
				});
...
var JSXIdentifier = class extends IdentifierBase {
	...
	bind() {
		...
		if (type === 0) {
			this.variable = this.scope.findVariable(this.name);
			this.variable.addReference(this);
		} else if (type === 1) this.isNativeElement = true;
	}
	include(context) {
		if (!this.included) this.includeNode(context);
	}
	...
	render(code, { snippets: { getPropertyAccess }, useOriginalName }) {
		if (this.variable) {
			const name = this.variable.getName(getPropertyAccess, useOriginalName);
			if (name !== this.name) code.overwrite(this.start, this.end, name, {
				contentOnly: true,
				storeName: true
			});
		} else if (this.isNativeElement && this.scope.context.options.jsx.mode !== "preserve") code.update(this.start, this.end, JSON.stringify(this.name));
	}
	getType() {
		switch (this.parent.type) {
			case "JSXOpeningElement":
			case "JSXClosingElement": return this.name.startsWith(this.name.charAt(0).toUpperCase()) ? 0 : 1;
			case "JSXMemberExpression": return this.parent.object === this ? 0 : 2;
			case "JSXAttribute":
			case "JSXNamespacedName": return 2;
			default:
 /* istanbul ignore next */
	...
	render(code, options, { jsxMode } = BLANK) {
		...
		if (["classic", "automatic"].includes(jsxMode)) {
			...
			if (!(jsxMode === "automatic" && key === "key")) {
				...
				if (key !== safeKey) code.overwrite(name.start, name.end, safeKey, { contentOnly: true });
				if (value) {
					code.overwrite(name.end, value.start, ": ", { contentOnly: true });
					if (value instanceof Literal$1 && typeof value.value === "string" && value.value.includes("\n")) code.overwrite(value.start, value.end, JSON.stringify(value.value), { contentOnly: true });
				} else code.appendLeft(name.end, ": true");
			}
		}
	}
};
...
var JSXClosingBase = class extends NodeBase {
	render(code, options) {
		...
		if (mode !== "preserve") code.overwrite(this.start, this.end, ")", { contentOnly: true });
		else super.render(code, options);
	}
};
...
var JSXText = class extends NodeBase {
	...
	render(code) {
		const { mode } = this.scope.context.options.jsx;
		if (mode !== "preserve") code.overwrite(this.start, this.end, JSON.stringify(this.getRenderedText()), { contentOnly: true });
	}
	getRenderedText() {
		if (this.renderedText === void 0) this.renderedText = this.value.replace(RE_WHITESPACE_TRIM, "").replace(RE_WHITESPACE_MERGE, " ");
...
function getRenderedJsxChildren(children) {
	...
	for (const child of children) if (!(child instanceof JSXExpressionContainer && child.expression instanceof JSXEmptyExpression) && (!(child instanceof JSXText) || child.shouldRender())) renderedChildren++;
...
function getAndIncludeFactoryVariable(factory, preserve, importSource, node, context) {
	...
	} else factoryVariable = node.scope.findGlobal(baseName);
...
var JSXElementBase = class extends NodeBase {
	...
	initialise() {
		...
		if (importSource) this.scope.context.addImportSource(importSource);
	}
	include(context, includeChildrenRecursively) {
		if (!this.included) this.includeNode(context);
		for (const child of this.children) child.include(context, includeChildrenRecursively);
	}
	includeNode(context) {
		this.included = true;
	...
	renderChildren(code, options, openingEnd) {
		...
		for (const child of children) {
			if (child instanceof JSXExpressionContainer && child.expression instanceof JSXEmptyExpression || child instanceof JSXText && !child.shouldRender()) code.remove(childrenEnd, child.end);
			else {
				code.appendLeft(childrenEnd, ", ");
				child.render(code, options);
				if (firstChild) hasMultipleChildren = true;
...
var JSXElement = class extends JSXElementBase {
	...
	render(code, options) {
		switch (this.jsxMode.mode) {
			...
			default: super.render(code, options);
		}
	}
	getRenderingMode() {
		const jsx = this.scope.context.options.jsx;
		...
		if (mode === "automatic") {
			let hasSpread = false;
			for (const attribute of this.openingElement.attributes) if (attribute instanceof JSXSpreadAttribute) hasSpread = true;
			else if (hasSpread && attribute.name.name === "key") return {
				factory,
				importSource,
				mode: "classic"
			};
	...
	renderClassicMode(code, options) {
		...
		if (selfClosing) code.appendLeft(end, ")");
		else closingElement.render(code, options);
	}
	renderAutomaticMode(code, options) {
		...
		if (firstChild) {
			...
			if (hasMultipleChildren) code.appendLeft(previousEnd, "]");
		}
		...
		if (keyAttribute) {
			...
			if (value) code.move(value.start, value.end, attributesEnd);
			else code.appendLeft(attributesEnd, "true");
		}
		if (selfClosing) code.appendLeft(keyAttribute?.value?.end || end, ")");
		else closingElement.render(code, options);
	}
	renderAttributes(code, options, factoryName, extractKeyAttribute) {
		...
		for (const attribute of attributes) {
			if (attribute instanceof JSXAttribute) {
				...
				if (inObject) {
					if (hasAttributes) code.appendLeft(previousEnd, " ");
					code.appendLeft(previousEnd, "},");
					inObject = false;
				} else code.appendLeft(previousEnd, ",");
				hasSpread = true;
			}
	...
	wrapAttributes(code, inObject, hasAttributes, hasSpread, firstAttribute, missingAttributesFallback, attributesEnd) {
		if (inObject) code.appendLeft(attributesEnd, " }");
		if (hasSpread) {
			if (hasAttributes) {
				const { start } = firstAttribute;
				if (firstAttribute instanceof JSXSpreadAttribute) code.prependRight(start, "{}, ");
				code.prependRight(start, "Object.assign(");
				code.appendLeft(attributesEnd, ")");
			}
		} else if (!hasAttributes) code.appendLeft(attributesEnd, `, ${missingAttributesFallback}`);
	}
};
var JSXFragment = class extends JSXElementBase {
	...
	renderClassicMode(code, options) {
		...
		code.prependRight(start, `/*#__PURE__*/${[factoryVariable.getName(getPropertyAccess, useOriginalName), ...nestedName].join(".")}(`);
	...
	renderAutomaticMode(code, options) {
		...
		if (firstChild) {
			code.prependRight(firstChild.start, `{ children: ${hasMultipleChildren ? "[" : ""}`);
			if (hasMultipleChildren) code.appendLeft(closingFragment.start, "]");
			code.appendLeft(childrenEnd, " }");
		} else code.appendLeft(openingFragment.end, ", {}");
		closingFragment.render(code, options);
	}
};
var JSXMemberExpression = class extends NodeBase {
	...
	includePath(path$2, context) {
		if (!this.included) this.includeNode(context);
		this.object.includePath([this.property.name, ...path$2], context);
	}
};
...
var JSXOpeningElement = class extends NodeBase {
	render(code, options, { jsxMode = this.scope.context.options.jsx.mode } = {}) {
		...
		for (const attribute of this.attributes) attribute.render(code, options, { jsxMode });
	}
};
...
var LabeledStatement = class extends NodeBase {
	hasEffects(context) {
		...
		if (this.body.hasEffects(context)) bodyHasEffects = true;
		else {
	...
	render(code, options) {
		if (this.label.included) this.label.render(code, options);
		else code.remove(this.start, findNonWhiteSpace(code.original, findFirstOccurrenceOutsideComment(code.original, ":", this.label.end) + 1));
		this.body.render(code, options);
	}
};
...
var LogicalExpression = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.left.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
		this.right.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizeCache() {
		if (this.hasDeoptimizedCache) return;
		this.hasDeoptimizedCache = true;
	...
	deoptimizePath(path$2) {
		...
		if (usedBranch) usedBranch.deoptimizePath(path$2);
			...
			this.left.deoptimizePath(path$2);
			this.right.deoptimizePath(path$2);
		}
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		...
		if (usedBranch) {
			this.expressionsToBeDeoptimized.push(origin);
			return usedBranch.getLiteralValueAtPath(path$2, recursionTracker, origin);
		} else if (!this.hasDeoptimizedCache && !path$2.length) {
			const rightValue = this.right.getLiteralValueAtPath(path$2, recursionTracker, origin);
	...
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		...
		return [new MultiExpression([this.left.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin)[0], this.right.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin)[0]]), false];
	}
	hasEffects(context) {
		if (this.left.hasEffects(context)) return true;
		if (this.getUsedBranch() !== this.left) return this.right.hasEffects(context);
		return false;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		const usedBranch = this.getUsedBranch();
		if (usedBranch) return usedBranch.hasEffectsOnInteractionAtPath(path$2, interaction, context);
		return this.left.hasEffectsOnInteractionAtPath(path$2, interaction, context) || this.right.hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	include(context, includeChildrenRecursively) {
		this.included = true;
		const usedBranch = this.getUsedBranch();
	...
	includePath(path$2, context) {
		...
		if (!usedBranch || usedBranch === this.right && this.left.shouldBeIncluded(context)) {
			this.left.includePath(path$2, context);
			this.right.includePath(path$2, context);
		} else usedBranch.includePath(path$2, context);
	}
	removeAnnotations(code) {
		this.left.removeAnnotations(code);
	...
	render(code, options, { isCalleeOfRenderedParent, preventASI, renderedParentType, renderedSurroundingElement } = BLANK) {
		if (!this.left.included || !this.right.included) {
			...
			if (this.right.included) {
				...
				if (preventASI) removeLineBreaks(code, removePos, this.right.start);
				this.left.removeAnnotations(code);
			} else code.remove(findLastWhiteSpaceReverse(code.original, this.left.end, operatorPos), this.end);
	...
	getUsedBranch() {
		if (!this.isBranchResolutionAnalysed) {
			...
			if (typeof booleanOrUnknown === "symbol" || this.operator === "??" && typeof leftValue === "symbol") return null;
			else this.usedBranch = this.operator === "||" && booleanOrUnknown || this.operator === "&&" && !booleanOrUnknown || this.operator === "??" && leftValue != null ? this.left : this.right;
		}
...
var NewExpression = class extends NodeBase {
	...
	hasEffectsOnInteractionAtPath(path$2, { type }) {
		return path$2.length > 0 || type !== INTERACTION_ACCESSED;
	}
	include(context, includeChildrenRecursively) {
		if (!this.included) this.includeNode(context);
		if (includeChildrenRecursively) super.include(context, true);
		else {
			this.callee.include(context, false);
			this.callee.includeCallArguments(this.interaction, context);
		}
...
var ObjectExpression$1 = class extends NodeBase {
	constructor() {
		super(...arguments);
		this.objectEntity = null;
		this.protoProp = null;
	...
	includePath(path$2, context) {
		if (!this.included) this.includeNode(context);
		this.getObjectEntity().includePath(path$2, context);
	}
	render(code, options, { renderedSurroundingElement } = BLANK) {
	...
	getObjectEntity() {
		...
		for (const property of this.properties) {
			...
			if (property.computed) {
				...
				key = property.key instanceof Identifier$1 ? property.key.name : String(property.key.value);
				if (key === "__proto__" && property.kind === "init") {
					this.protoProp = property;
					prototype = property.value instanceof Literal$1 && property.value.value === null ? null : property.value;
					continue;
				}
			}
			properties.push({
...
ObjectExpression$1.prototype.applyDeoptimizations = doNotDeoptimize;
...
var Program$1 = class extends NodeBase {
	constructor() {
		super(...arguments);
	...
	hasEffects(context) {
		for (const node of this.body) if (node.hasEffects(context)) {
	...
	initialise() {
		super.initialise();
		if (this.invalidAnnotations) for (const { start, end, type } of this.invalidAnnotations) {
			this.scope.context.magicString.remove(start, end);
			if (type === "pure" || type === "noSideEffects") this.scope.context.log(LOGLEVEL_WARN, logInvalidAnnotation(this.scope.context.code.slice(start, end), this.scope.context.module.id, type), start);
		}
	}
	render(code, options) {
...
Program$1.prototype.includeNode = onlyIncludeSelfNoDeoptimize;
Program$1.prototype.applyDeoptimizations = doNotDeoptimize;
var Property$1 = class extends MethodBase {
	get method() {
		return isFlagSet(
			this.flags,
			262144
	...
	includeDestructuredIfNecessary(context, destructuredInitPath, init) {
		const path$2 = this.getPathInProperty(destructuredInitPath);
		let included = this.value.includeDestructuredIfNecessary(context, path$2, init) || this.included;
		if (included ||= this.key.hasEffects(createHasEffectsContext())) {
			this.key.include(context, false);
			if (!this.value.included) {
				this.value.includeNode(context);
				this.value.includeDestructuredIfNecessary(context, path$2, init);
			}
		}
		if (!this.included && included) this.includeNode(context);
		return this.included;
	...
	includePath(path$2, context) {
		this.included = true;
		this.value.includePath(path$2, context);
	}
	markDeclarationReached() {
		this.value.markDeclarationReached();
	}
	render(code, options) {
		if (!this.shorthand) this.key.render(code, options);
		this.value.render(code, options, { isShorthandProperty: this.shorthand });
	}
	getPathInProperty(destructuredInitPath) {
		return destructuredInitPath.at(-1) === UnknownKey ? destructuredInitPath : this.computed ? [...destructuredInitPath, UnknownKey] : this.key instanceof Identifier$1 ? [...destructuredInitPath, this.key.name] : [...destructuredInitPath, String(this.key.value)];
	}
};
Property$1.prototype.includeNode = onlyIncludeSelfNoDeoptimize;
Property$1.prototype.applyDeoptimizations = doNotDeoptimize;
var PropertyDefinition = class extends NodeBase {
	...
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.value?.deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizePath(path$2) {
		this.value?.deoptimizePath(path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		return this.value ? this.value.getLiteralValueAtPath(path$2, recursionTracker, origin) : UnknownValue;
	}
	getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) {
		return this.value ? this.value.getReturnExpressionWhenCalledAtPath(path$2, interaction, recursionTracker, origin) : UNKNOWN_RETURN_EXPRESSION;
	}
	hasEffects(context) {
		return this.key.hasEffects(context) || this.static && !!this.value?.hasEffects(context) || checkEffectForNodes(this.decorators, context);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return !this.value || this.value.hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	includeNode(context) {
		this.included = true;
		this.value?.includePath(UNKNOWN_PATH, context);
		for (const decorator of this.decorators) decorator.includePath(UNKNOWN_PATH, context);
	}
};
PropertyDefinition.prototype.applyDeoptimizations = doNotDeoptimize;
var ReturnStatement$1 = class extends NodeBase {
	hasEffects(context) {
	...
	render(code, options) {
		if (this.argument) {
			...
			if (this.argument.start === this.start + 6) code.prependLeft(this.start + 6, " ");
		}
	}
};
ReturnStatement$1.prototype.applyDeoptimizations = doNotDeoptimize;
var SequenceExpression = class extends NodeBase {
	deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker) {
		this.expressions[this.expressions.length - 1].deoptimizeArgumentsOnInteractionAtPath(interaction, path$2, recursionTracker);
	}
	deoptimizePath(path$2) {
		this.expressions[this.expressions.length - 1].deoptimizePath(path$2);
	}
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		return this.expressions[this.expressions.length - 1].getLiteralValueAtPath(path$2, recursionTracker, origin);
	}
	hasEffects(context) {
		for (const expression of this.expressions) if (expression.hasEffects(context)) return true;
		return false;
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		return this.expressions[this.expressions.length - 1].hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	include(context, includeChildrenRecursively) {
		this.included = true;
		const lastExpression = this.expressions[this.expressions.length - 1];
		for (const expression of this.expressions) if (includeChildrenRecursively || expression === lastExpression && !(this.parent instanceof ExpressionStatement$1) || expression.shouldBeIncluded(context)) expression.include(context, includeChildrenRecursively);
	}
	includePath(path$2, context) {
		this.included = true;
		this.expressions[this.expressions.length - 1].includePath(path$2, context);
	}
	removeAnnotations(code) {
	...
	render(code, options, { renderedParentType, isCalleeOfRenderedParent, preventASI } = BLANK) {
		...
		for (const { node, separator, start, end } of getCommaSeparatedNodesWithBoundaries(this.expressions, code, this.start, this.end)) {
			...
			includedNodes++;
			lastSeparatorPos = separator;
			if (includedNodes === 1 && preventASI) removeLineBreaks(code, start, node.start);
			if (includedNodes === 1) {
			...
			} else node.render(code, options);
		}
		if (lastSeparatorPos) code.remove(lastSeparatorPos, this.end);
	}
};
...
var SwitchCase = class extends NodeBase {
	...
	include(context, includeChildrenRecursively) {
		this.included = true;
		this.test?.include(context, includeChildrenRecursively);
		for (const node of this.consequent) if (includeChildrenRecursively || node.shouldBeIncluded(context)) node.include(context, includeChildrenRecursively);
	}
	render(code, options, nodeRenderOptions) {
		if (this.test) {
			this.test.render(code, options);
			if (this.test.start === this.start + 4) code.prependLeft(this.test.start, " ");
		}
...
var SwitchStatement = class extends NodeBase {
	...
	hasEffects(context) {
		...
		if (this.defaultCase !== null) context.brokenFlow = onlyHasBrokenFlow;
		ignore.breaks = breaks;
		context.hasBreak = hasBreak;
		return false;
	}
	include(context, includeChildrenRecursively) {
		...
		for (let caseIndex = this.cases.length - 1; caseIndex >= 0; caseIndex--) {
			...
			if (switchCase.included) isCaseIncluded = true;
			if (!isCaseIncluded) {
				const hasEffectsContext = createHasEffectsContext();
			...
			if (isCaseIncluded) {
				...
				context.hasBreak = false;
				context.brokenFlow = brokenFlow;
			} else onlyHasBrokenFlow = brokenFlow;
		}
		if (isCaseIncluded && this.defaultCase !== null) context.brokenFlow = onlyHasBrokenFlow;
		context.hasBreak = hasBreak;
	}
	initialise() {
		super.initialise();
		for (let caseIndex = 0; caseIndex < this.cases.length; caseIndex++) if (this.cases[caseIndex].test === null) {
	...
	render(code, options) {
		...
		if (this.cases.length > 0) renderStatementList(this.cases, code, this.cases[0].start, this.end - 1, options);
	}
};
...
var TaggedTemplateExpression = class extends CallExpressionBase {
	...
	hasEffects(context) {
		if (!this.deoptimized) this.applyDeoptimizations();
		for (const argument of this.quasi.expressions) if (argument.hasEffects(context)) return true;
	...
	initialise() {
		...
		this.interaction = {
			args: [this.tag instanceof MemberExpression$1 && !this.tag.variable ? this.tag.object : null, ...this.args],
			type: INTERACTION_CALLED,
			withNew: false
		};
	}
	render(code, options) {
		...
		if (!this.hasCheckedForWarnings && this.tag.type === Identifier) {
			...
			if (variable.isNamespace) this.scope.context.log(LOGLEVEL_WARN, logCannotCallNamespace(name), this.start);
		}
	}
	applyDeoptimizations() {
		this.deoptimized = true;
...
var TemplateLiteral$1 = class extends NodeBase {
	deoptimizeArgumentsOnInteractionAtPath() {}
	getLiteralValueAtPath(path$2) {
		if (path$2.length > 0 || this.quasis.length !== 1) return UnknownValue;
		return this.quasis[0].value.cooked;
	}
	getReturnExpressionWhenCalledAtPath(path$2) {
		if (path$2.length !== 1) return UNKNOWN_RETURN_EXPRESSION;
		return getMemberReturnExpressionWhenCalled(literalStringMembers, path$2[0]);
	}
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (interaction.type === INTERACTION_ACCESSED) return path$2.length > 1;
		if (interaction.type === INTERACTION_CALLED && path$2.length === 1) return hasMemberEffectWhenCalled(literalStringMembers, path$2[0], interaction, context);
	...
	includeNode(context) {
		this.included = true;
		if (!this.deoptimized) this.applyDeoptimizations();
		for (const node of this.expressions) node.includePath(UNKNOWN_PATH, context);
	}
	render(code, options) {
...
var ModuleScope = class extends ChildScope {
	...
	addDeclaration(identifier, context, init, destructuredInitPath, kind) {
		if (this.importDescriptions.has(identifier.name)) context.error(logRedeclarationError(identifier.name), identifier.start);
		return super.addDeclaration(identifier, context, init, destructuredInitPath, kind);
	}
...
var ThisExpression = class extends NodeBase {
	...
	hasEffectsOnInteractionAtPath(path$2, interaction, context) {
		if (path$2.length === 0) return interaction.type !== INTERACTION_ACCESSED;
		return this.variable.hasEffectsOnInteractionAtPath(path$2, interaction, context);
	}
	include(context) {
		if (!this.included) this.includeNode(context);
	}
	...
	includePath(path$2, context) {
		if (!this.included) {
			this.included = true;
			this.scope.context.includeVariableInModule(this.variable, path$2, context);
		} else if (path$2.length > 0) this.variable.includePath(path$2, context);
		const functionScope = findFunctionScope(this.scope, this.variable);
		if (functionScope && functionScope.functionNode.parent instanceof Property$1 && functionScope.functionNode.parent.parent instanceof ObjectExpression$1) functionScope.functionNode.parent.parent.includePath(path$2, context);
	}
	initialise() {
		super.initialise();
		this.alias = this.scope.findLexicalBoundary() instanceof ModuleScope ? this.scope.context.moduleContext : null;
		if (this.alias === "undefined") this.scope.context.log(LOGLEVEL_WARN, logThisIsUndefined(), this.start);
	}
	render(code) {
		if (this.alias !== null) code.overwrite(this.start, this.end, this.alias, {
			contentOnly: false,
			storeName: true
		});
	}
...
function findFunctionScope(scope, thisVariable) {
	while (!(scope instanceof FunctionScope && scope.thisVariable === thisVariable)) {
		if (!(scope instanceof ChildScope)) return null;
		scope = scope.parent;
	}
	return scope;
}
var ThrowStatement = class extends NodeBase {
	...
	render(code, options) {
		this.argument.render(code, options, { preventASI: true });
		if (this.argument.start === this.start + 5) code.prependLeft(this.start + 5, " ");
	}
};
var TryStatement = class extends NodeBase {
	...
	include(context, includeChildrenRecursively) {
		...
		if (!this.directlyIncluded || !tryCatchDeoptimization) {
			...
			if (includedLabels.size > 0) this.includedLabelsAfterBlock = [...includedLabels];
			context.brokenFlow = brokenFlow;
		} else if (this.includedLabelsAfterBlock) for (const label of this.includedLabelsAfterBlock) includedLabels.add(label);
...
var UnaryExpression = class extends NodeBase {
	...
	getLiteralValueAtPath(path$2, recursionTracker, origin) {
		...
		if (typeof argumentValue === "symbol") {
			if (this.operator === "void") return void 0;
			if (this.operator === "!") {
	...
	hasEffectsOnInteractionAtPath(path$2, { type }) {
		return type !== INTERACTION_ACCESSED || path$2.length > (this.operator === "void" ? 0 : 1);
	}
	applyDeoptimizations() {
		this.deoptimized = true;
	...
	include(context, includeChildrenRecursively, _options) {
		if (!this.deoptimized) this.applyDeoptimizations();
		this.included = true;
		const shouldPreserveArgument = this.argument instanceof Identifier$1 && this.argument.variable?.included;
	...
	render(code, options) {
		if (typeof this.renderedLiteralValue === "symbol") super.render(code, options);
		else {
			let value = this.renderedLiteralValue;
			if (!CHARACTERS_THAT_DO_NOT_REQUIRE_SPACE.test(code.original[this.start - 1])) value = ` ${value}`;
...
var UpdateExpression = class extends NodeBase {
	...
	render(code, options) {
		...
		if (format === "system") {
			...
			if (exportNames) if (this.prefix) if (exportNames.length === 1) renderSystemExportExpression(variable, this.start, this.end, code, options);
			else renderSystemExportSequenceAfterExpression(variable, this.start, this.end, this.parent.type !== ExpressionStatement, code, options);
			else {
...
var VariableDeclaration = class extends NodeBase {
	deoptimizePath() {
		for (const declarator of this.declarations) declarator.deoptimizePath(EMPTY_PATH);
	}
	...
	include(context, includeChildrenRecursively, { asSingleStatement } = BLANK) {
		...
		for (const declarator of this.declarations) {
			...
			if (asSingleStatement) id.include(context, includeChildrenRecursively);
			if (init && id.included && !init.included && (id instanceof ObjectPattern || id instanceof ArrayPattern)) init.include(context, includeChildrenRecursively);
		}
	}
	initialise() {
		super.initialise();
		for (const declarator of this.declarations) declarator.declareDeclarator(this.kind);
	}
	removeAnnotations(code) {
	...
	render(code, options, nodeRenderOptions = BLANK) {
		if (this.areAllDeclarationsIncludedAndNotExported(options.exportNamesByVariable)) {
			for (const declarator of this.declarations) declarator.render(code, options);
			if (!nodeRenderOptions.isNoStatement && code.original.charCodeAt(this.end - 1) !== 59) code.appendLeft(this.end, ";");
		} else this.renderReplacedDeclarations(code, options);
	}
	renderDeclarationEnd(code, separatorString, lastSeparatorPos, actualContentEnd, renderedContentEnd, systemPatternExports, options) {
		if (code.original.charCodeAt(this.end - 1) === 59) code.remove(this.end - 1, this.end);
		separatorString += ";";
		if (lastSeparatorPos === null) code.appendLeft(renderedContentEnd, separatorString);
		else {
			...
			if (actualContentEnd === lastSeparatorPos + 1) code.overwrite(lastSeparatorPos, renderedContentEnd, separatorString);
			else {
				code.overwrite(lastSeparatorPos, lastSeparatorPos + 1, separatorString);
				code.remove(actualContentEnd, renderedContentEnd);
			}
		}
		if (systemPatternExports.length > 0) code.appendLeft(renderedContentEnd, ` ${getSystemExportStatement(systemPatternExports, options)};`);
	}
	renderReplacedDeclarations(code, options) {
		...
		for (const { node, start, separator, contentEnd, end } of separatedNodes) {
			...
			leadingString = "";
			nextSeparatorString = "";
			if (!node.id.included || node.id instanceof Identifier$1 && isReassignedExportsMember(node.id.variable, options.exportNamesByVariable)) {
				if (hasRenderedContent) separatorString += ";";
				...
				if (isInDeclaration) separatorString += ",";
				else {
					if (hasRenderedContent) separatorString += ";";
			...
			if (renderedContentEnd === lastSeparatorPos + 1) code.overwrite(lastSeparatorPos, renderedContentEnd, separatorString + leadingString);
			else {
				code.overwrite(lastSeparatorPos, lastSeparatorPos + 1, separatorString);
	...
	areAllDeclarationsIncludedAndNotExported(exportNamesByVariable) {
		if (this.kind === "await using" || this.kind === "using") return true;
		for (const declarator of this.declarations) {
...
function gatherSystemExportsAndGetSingleExport(separatedNodes, options, aggregatedSystemExports) {
	...
	if (options.format === "system") {
		for (const { node } of separatedNodes) if (node.id instanceof Identifier$1 && node.init && aggregatedSystemExports.length === 0 && options.exportNamesByVariable.get(node.id.variable)?.length === 1) {
			singleSystemExport = node.id.variable;
			aggregatedSystemExports.push(singleSystemExport);
		} else node.id.addExportedVariables(aggregatedSystemExports, options.exportNamesByVariable);
		if (aggregatedSystemExports.length > 1) singleSystemExport = null;
		else if (singleSystemExport) aggregatedSystemExports.length = 0;
	}
...
var VariableDeclarator$1 = class extends NodeBase {
	...
	include(context, includeChildrenRecursively) {
		...
		if (includeChildrenRecursively) id.include(context, includeChildrenRecursively);
		else id.includeDestructuredIfNecessary(context, EMPTY_PATH, init || UNDEFINED_EXPRESSION);
	}
	removeAnnotations(code) {
	...
	render(code, options) {
		...
		if (renderId) id.render(code, options);
		else {
		...
		if (init) {
			if (id instanceof Identifier$1 && init instanceof ClassExpression && !init.id) {
				const renderedVariable = id.variable.getName(getPropertyAccess);
				if (renderedVariable !== id.name) code.appendLeft(init.start + 5, ` ${id.name}`);
			}
			init.render(code, options, renderId ? BLANK : { renderedSurroundingElement: ExpressionStatement });
		} else if (id instanceof Identifier$1 && isReassignedExportsMember(id.variable, exportNamesByVariable)) code.appendLeft(end, `${_}=${_}void 0`);
	}
	includeNode(context) {
		this.included = true;
		const { id, init } = this;
		if (init) {
			if (this.isUsingDeclaration) init.includePath(SYMBOL_DISPOSE_PATH, context);
			else if (this.isAsyncUsingDeclaration) init.includePath(SYMBOL_ASYNC_DISPOSE_PATH, context);
			if (id instanceof Identifier$1 && init instanceof ClassExpression && !init.id) {
				const { name, variable } = id;
				for (const accessedVariable of init.scope.accessedOutsideVariables.values()) if (accessedVariable !== variable) accessedVariable.forbidName(name);
			}
		}
	}
};
VariableDeclarator$1.prototype.applyDeoptimizations = doNotDeoptimize;
...
function convertProgram(buffer, parent, parentScope) {
	return convertNode$1(parent, parentScope, 0, getAstBuffer(buffer));
}
...
const bufferParsers = [
	...
	function arrowFunctionExpression(node, position, buffer) {
		...
		scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement$1);
		node.body = convertNode$1(node, scope.bodyScope, buffer[position + 3], buffer);
	},
	function assignmentExpression(node, position, buffer) {
		const { scope } = node;
		node.operator = FIXED_STRINGS[buffer[position]];
		node.left = convertNode$1(node, scope, buffer[position + 1], buffer);
		node.right = convertNode$1(node, scope, buffer[position + 2], buffer);
	},
	function assignmentPattern(node, position, buffer) {
		const { scope } = node;
		node.left = convertNode$1(node, scope, buffer[position], buffer);
		node.right = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	function awaitExpression(node, position, buffer) {
		const { scope } = node;
		node.argument = convertNode$1(node, scope, buffer[position], buffer);
	},
	...
	function breakStatement(node, position, buffer) {
		...
		node.label = labelPosition === 0 ? null : convertNode$1(node, scope, labelPosition, buffer);
	},
	function callExpression(node, position, buffer) {
		...
		node.callee = convertNode$1(node, scope, buffer[position + 2], buffer);
		node.arguments = convertNodeList(node, scope, buffer[position + 3], buffer);
	},
	function catchClause(node, position, buffer) {
		const { scope } = node;
		const parameterPosition = buffer[position];
		const parameter = node.param = parameterPosition === 0 ? null : convertNode$1(node, scope, parameterPosition, buffer);
		parameter?.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION);
		node.body = convertNode$1(node, scope.bodyScope, buffer[position + 1], buffer);
	},
	function chainExpression(node, position, buffer) {
		const { scope } = node;
		node.expression = convertNode$1(node, scope, buffer[position], buffer);
	},
	function classBody(node, position, buffer) {
		...
		if (bodyPosition) {
			...
			for (let index = 0; index < length; index++) {
				...
				body[index] = convertNode$1(node, buffer[nodePosition] !== 79 && (buffer[nodePosition + 3] & 1) === 0 ? scope.instanceScope : scope, nodePosition, buffer);
			}
		} else node.body = [];
	},
	function classDeclaration(node, position, buffer) {
		...
		node.id = idPosition === 0 ? null : convertNode$1(node, scope.parent, idPosition, buffer);
		const superClassPosition = buffer[position + 2];
		node.superClass = superClassPosition === 0 ? null : convertNode$1(node, scope, superClassPosition, buffer);
		node.body = convertNode$1(node, scope, buffer[position + 3], buffer);
	},
	function classExpression(node, position, buffer) {
		const { scope } = node;
		node.decorators = convertNodeList(node, scope, buffer[position], buffer);
		const idPosition = buffer[position + 1];
		node.id = idPosition === 0 ? null : convertNode$1(node, scope, idPosition, buffer);
		const superClassPosition = buffer[position + 2];
		node.superClass = superClassPosition === 0 ? null : convertNode$1(node, scope, superClassPosition, buffer);
		node.body = convertNode$1(node, scope, buffer[position + 3], buffer);
	},
	function conditionalExpression(node, position, buffer) {
		const { scope } = node;
		node.test = convertNode$1(node, scope, buffer[position], buffer);
		node.consequent = convertNode$1(node, scope, buffer[position + 1], buffer);
		node.alternate = convertNode$1(node, scope, buffer[position + 2], buffer);
	},
	...
	function directive(node, position, buffer) {
		...
		node.expression = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	function doWhileStatement(node, position, buffer) {
		const { scope } = node;
		node.body = convertNode$1(node, scope, buffer[position], buffer);
		node.test = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	function emptyStatement() {},
	function exportAllDeclaration(node, position, buffer) {
		const { scope } = node;
		const exportedPosition = buffer[position];
		node.exported = exportedPosition === 0 ? null : convertNode$1(node, scope, exportedPosition, buffer);
		node.source = convertNode$1(node, scope, buffer[position + 1], buffer);
	...
	function exportDefaultDeclaration(node, position, buffer) {
		const { scope } = node;
		node.declaration = convertNode$1(node, scope, buffer[position], buffer);
	},
	function exportNamedDeclaration(node, position, buffer) {
		...
		node.source = sourcePosition === 0 ? null : convertNode$1(node, scope, sourcePosition, buffer);
		node.attributes = convertNodeList(node, scope, buffer[position + 2], buffer);
		const declarationPosition = buffer[position + 3];
		node.declaration = declarationPosition === 0 ? null : convertNode$1(node, scope, declarationPosition, buffer);
	},
	function exportSpecifier(node, position, buffer) {
		const { scope } = node;
		node.local = convertNode$1(node, scope, buffer[position], buffer);
		const exportedPosition = buffer[position + 1];
		node.exported = exportedPosition === 0 ? node.local : convertNode$1(node, scope, exportedPosition, buffer);
	},
	...
	function forInStatement(node, position, buffer) {
		...
		node.left = convertNode$1(node, scope, buffer[position], buffer);
		node.right = convertNode$1(node, scope, buffer[position + 1], buffer);
		node.body = convertNode$1(node, scope, buffer[position + 2], buffer);
	},
	...
	function forStatement(node, position, buffer) {
		...
		node.init = initPosition === 0 ? null : convertNode$1(node, scope, initPosition, buffer);
		const testPosition = buffer[position + 1];
		node.test = testPosition === 0 ? null : convertNode$1(node, scope, testPosition, buffer);
		const updatePosition = buffer[position + 2];
		node.update = updatePosition === 0 ? null : convertNode$1(node, scope, updatePosition, buffer);
		node.body = convertNode$1(node, scope, buffer[position + 3], buffer);
	},
	function functionDeclaration(node, position, buffer) {
		...
		scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement$1);
		node.body = convertNode$1(node, scope.bodyScope, buffer[position + 4], buffer);
	},
	function functionExpression(node, position, buffer) {
		...
		node.annotationNoSideEffects = annotations.some((comment) => comment.type === "noSideEffects");
		const idPosition = buffer[position + 2];
		node.id = idPosition === 0 ? null : convertNode$1(node, node.idScope, idPosition, buffer);
		const parameters = node.params = convertNodeList(node, scope, buffer[position + 3], buffer);
		scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement$1);
		node.body = convertNode$1(node, scope.bodyScope, buffer[position + 4], buffer);
	},
	...
	function ifStatement(node, position, buffer) {
		const { scope } = node;
		node.test = convertNode$1(node, scope, buffer[position], buffer);
		node.consequent = convertNode$1(node, node.consequentScope = new TrackingScope(scope), buffer[position + 1], buffer);
		const alternatePosition = buffer[position + 2];
		node.alternate = alternatePosition === 0 ? null : convertNode$1(node, node.alternateScope = new TrackingScope(scope), alternatePosition, buffer);
	},
	function importAttribute(node, position, buffer) {
		const { scope } = node;
		node.key = convertNode$1(node, scope, buffer[position], buffer);
		node.value = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	...
	function importExpression(node, position, buffer) {
		const { scope } = node;
		node.source = convertNode$1(node, scope, buffer[position], buffer);
		node.sourceAstNode = convertNode(buffer[position], buffer);
		const optionsPosition = buffer[position + 1];
		node.options = optionsPosition === 0 ? null : convertNode$1(node, scope, optionsPosition, buffer);
	...
	function importSpecifier(node, position, buffer) {
		...
		node.local = convertNode$1(node, scope, buffer[position + 1], buffer);
		node.imported = importedPosition === 0 ? node.local : convertNode$1(node, scope, importedPosition, buffer);
	},
	function jsxAttribute(node, position, buffer) {
		const { scope } = node;
		node.name = convertNode$1(node, scope, buffer[position], buffer);
		const valuePosition = buffer[position + 1];
		node.value = valuePosition === 0 ? null : convertNode$1(node, scope, valuePosition, buffer);
	},
	...
	function jsxElement(node, position, buffer) {
		const { scope } = node;
		node.openingElement = convertNode$1(node, scope, buffer[position], buffer);
		node.children = convertNodeList(node, scope, buffer[position + 1], buffer);
		const closingElementPosition = buffer[position + 2];
		node.closingElement = closingElementPosition === 0 ? null : convertNode$1(node, scope, closingElementPosition, buffer);
	},
	...
	function jsxFragment(node, position, buffer) {
		const { scope } = node;
		node.openingFragment = convertNode$1(node, scope, buffer[position], buffer);
		node.children = convertNodeList(node, scope, buffer[position + 1], buffer);
		node.closingFragment = convertNode$1(node, scope, buffer[position + 2], buffer);
	},
	...
	function jsxMemberExpression(node, position, buffer) {
		const { scope } = node;
		node.object = convertNode$1(node, scope, buffer[position], buffer);
		node.property = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	function jsxNamespacedName(node, position, buffer) {
		const { scope } = node;
		node.namespace = convertNode$1(node, scope, buffer[position], buffer);
		node.name = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	...
	function labeledStatement(node, position, buffer) {
		const { scope } = node;
		node.label = convertNode$1(node, scope, buffer[position], buffer);
		node.body = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	...
	function memberExpression(node, position, buffer) {
		...
		node.object = convertNode$1(node, scope, buffer[position + 1], buffer);
		node.property = convertNode$1(node, scope, buffer[position + 2], buffer);
	},
	function metaProperty(node, position, buffer) {
		const { scope } = node;
		node.meta = convertNode$1(node, scope, buffer[position], buffer);
		node.property = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	function methodDefinition(node, position, buffer) {
		...
		node.key = convertNode$1(node, scope, buffer[position + 2], buffer);
		node.value = convertNode$1(node, scope, buffer[position + 3], buffer);
	...
	function newExpression(node, position, buffer) {
		...
		node.callee = convertNode$1(node, scope, buffer[position + 1], buffer);
	...
	function property(node, position, buffer) {
		...
		node.value = convertNode$1(node, scope, buffer[position + 2], buffer);
		node.kind = FIXED_STRINGS[buffer[position + 3]];
		node.key = keyPosition === 0 ? node.value : convertNode$1(node, scope, keyPosition, buffer);
	},
	...
	function returnStatement(node, position, buffer) {
		...
		node.argument = argumentPosition === 0 ? null : convertNode$1(node, scope, argumentPosition, buffer);
	},
	...
	function switchStatement(node, position, buffer) {
		const { scope } = node;
		node.discriminant = convertNode$1(node, node.parentScope, buffer[position], buffer);
		node.cases = convertNodeList(node, scope, buffer[position + 1], buffer);
	},
	function taggedTemplateExpression(node, position, buffer) {
		const { scope } = node;
		node.tag = convertNode$1(node, scope, buffer[position], buffer);
		node.quasi = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	...
	function tryStatement(node, position, buffer) {
		const { scope } = node;
		node.block = convertNode$1(node, scope, buffer[position], buffer);
		const handlerPosition = buffer[position + 1];
		node.handler = handlerPosition === 0 ? null : convertNode$1(node, scope, handlerPosition, buffer);
		const finalizerPosition = buffer[position + 2];
		node.finalizer = finalizerPosition === 0 ? null : convertNode$1(node, scope, finalizerPosition, buffer);
	},
	function unaryExpression(node, position, buffer) {
		const { scope } = node;
		node.operator = FIXED_STRINGS[buffer[position]];
		node.argument = convertNode$1(node, scope, buffer[position + 1], buffer);
	},
	function updateExpression(node, position, buffer) {
		...
		node.argument = convertNode$1(node, scope, buffer[position + 2], buffer);
	},
	...
	function variableDeclarator(node, position, buffer) {
		const { scope } = node;
		node.id = convertNode$1(node, scope, buffer[position], buffer);
...
function convertNode$1(parent, parentScope, position, buffer) {
...
function convertNodeList(parent, parentScope, position, buffer) {
	...
	for (let index = 0; index < length; index++) {
		...
		list[index] = nodePosition ? convertNode$1(parent, parentScope, nodePosition, buffer) : null;
	}
	return list;
}
...
var BuildPhase;
(function(BuildPhase$1) {
	BuildPhase$1[BuildPhase$1["LOAD_AND_PARSE"] = 0] = "LOAD_AND_PARSE";
	BuildPhase$1[BuildPhase$1["ANALYSE"] = 1] = "ANALYSE";
	BuildPhase$1[BuildPhase$1["GENERATE"] = 2] = "GENERATE";
...
function resetCacheToEncoded(cache) {
	if (cache.encodedMappings === void 0 && cache.decodedMappings) cache.encodedMappings = encode(cache.decodedMappings);
	cache.decodedMappings = void 0;
}
function resetSourcemapCache(map, sourcemapChain) {
	if (map) {
		const cache = sourceMapCache.get(map);
		if (cache) resetCacheToEncoded(cache);
	}
	if (!sourcemapChain) return;
	for (const map$1 of sourcemapChain) {
		if (map$1.missing) continue;
		resetSourcemapCache(map$1);
	}
}
function decodedSourcemap(map) {
	...
	const decodedMap = {
		...map,
		get mappings() {
			if (cache.decodedMappings) return cache.decodedMappings;
...
function getAttributesFromImportExpression(node) {
	...
	if (!(options instanceof ObjectExpression$1)) {
		if (options) context.module.log(LOGLEVEL_WARN, logImportAttributeIsInvalid(context.module.id), start);
		return EMPTY_OBJECT;
	}
	const assertProperty = options.properties.find((property) => ATTRIBUTE_KEYWORDS.has(getPropertyKey(property)))?.value;
	if (!assertProperty) return EMPTY_OBJECT;
	if (!(assertProperty instanceof ObjectExpression$1)) {
	...
	const assertFields = assertProperty.properties.map((property) => {
		...
		if (typeof key === "string" && typeof property.value.value === "string") return [key, property.value.value];
		context.module.log(LOGLEVEL_WARN, logImportAttributeIsInvalid(context.module.id), property.start);
		return null;
	}).filter((property) => !!property);
	if (assertFields.length > 0) return Object.fromEntries(assertFields);
	return EMPTY_OBJECT;
}
...
function timeStartImpl(label, level = 3) {
	...
	if (timer === void 0) timers.set(label, {
		memory: 0,
		startMemory,
		startTime,
		time: 0,
...
function getTimings() {
	const newTimings = {};
	for (const [label, { memory, time, totalMemory }] of timers) newTimings[label] = [
		time,
		memory,
		totalMemory
	];
...
function getPluginWithTimers(plugin, index) {
	...
	for (const hook of TIMED_PLUGIN_HOOKS) if (hook in plugin) {
		let timerLabel = `plugin ${index}`;
		if (plugin.name) timerLabel += ` (${plugin.name})`;
		timerLabel += ` - ${hook}`;
...
function getVariableForExportNameRecursive(target, name, importerForSideEffects, isExportAllSearch, searchedNamesAndModules = new Map(), importChain, sideEffectModules, exportOrReexportModules) {
	...
	if (searchedModules) {
		if (searchedModules.has(target)) return isExportAllSearch ? [null] : error(logCircularReexport(name, target.id));
		searchedModules.add(target);
	} else searchedNamesAndModules.set(name, new Set([target]));
...
function getAndExtendSideEffectModules(variable, module) {
	...
	while (true) {
		...
		if (!currentVariable || referencedVariables.has(currentVariable)) break;
		...
		if (originalSideEffects) for (const module$1 of originalSideEffects) sideEffectModules.add(module$1);
	}
...
var Module = class Module {
	constructor(graph, id, options, isEntry, moduleSideEffects, syntheticNamedExports, meta, attributes) {
		...
		this.info = {
			...
			get dynamicallyImportedIds() {
				return dynamicImports.map(({ id: id$1 }) => id$1).filter((id$1) => id$1 != null);
			},
			get dynamicImporters() {
				return dynamicImporters.sort();
			},
			get exportedBindings() {
				...
				for (const [name, { source }] of reexportDescriptions) (exportBindings[source] ??= []).push(name);
				for (const source of exportAllSources) (exportBindings[source] ??= []).push("*");
			...
			isExternal: false,
			get isIncluded() {
				if (graph.phase !== BuildPhase.GENERATE) return null;
	...
	basename() {
		const base$1 = basename$1(this.id);
		const extension = extname$1(this.id);
		return makeLegal(extension ? base$1.slice(0, -extension.length) : base$1);
	}
	bindReferences() {
		this.ast.bind();
	}
	...
	error(properties, pos) {
		if (pos !== void 0) this.addLocationToLogProps(properties, pos);
		return error(properties);
	}
	estimateSize() {
		let size = 0;
		for (const node of this.ast.body) if (node.included) size += node.end - node.start;
		return size;
	}
	getDependenciesToBeIncluded() {
		...
		if (this.info.isEntry || this.includedDynamicImporters.length > 0 || this.namespace.included || this.implicitlyLoadedAfter.size > 0) {
			for (const variable of this.getExportedVariablesByName().values()) if (variable.included) dependencyVariables.add(variable);
		}
		for (let variable of dependencyVariables) {
			const sideEffectDependencies = this.sideEffectDependenciesByVariable.get(variable);
			if (sideEffectDependencies) for (const module of sideEffectDependencies) alwaysCheckedDependencies.add(module);
			if (variable instanceof SyntheticNamedExportVariable) variable = variable.getBaseVariable();
			else if (variable instanceof ExportDefaultVariable) variable = variable.getOriginalVariable();
			necessaryDependencies.add(variable.module);
		}
		if (!this.options.treeshake || this.info.moduleSideEffects === "no-treeshake") for (const dependency of this.dependencies) this.relevantDependencies.add(dependency);
		else this.addRelevantSideEffectDependencies(this.relevantDependencies, necessaryDependencies, alwaysCheckedDependencies);
		for (const dependency of necessaryDependencies) this.relevantDependencies.add(dependency);
		return this.relevantDependencies;
	}
	getExportedVariablesByName() {
		if (this.exportedVariablesByName) return this.exportedVariablesByName;
		const exportedVariablesByName = this.exportedVariablesByName = new Map();
		for (const name of this.exportDescriptions.keys()) if (name !== this.info.syntheticNamedExports) {
			const [exportedVariable] = this.getVariableForExportName(name);
			if (exportedVariable) exportedVariablesByName.set(name, exportedVariable);
			else return error(logMissingEntryExport(name, this.id));
		}
		...
		for (const module of this.exportAllModules) {
			if (module instanceof ExternalModule) {
				exportedVariablesByName.set(`*${module.id}`, module.getVariableForExportName("*", { importChain: [this.id] })[0]);
				continue;
			}
			for (const name of module.getExportedVariablesByName().keys()) if (name !== "default" && !exportedVariablesByName.has(name)) {
				const [exportedVariable] = this.getVariableForExportName(name);
				if (exportedVariable) exportedVariablesByName.set(name, exportedVariable);
			}
		}
	...
	getExportNamesByVariable() {
		...
		for (const [exportName, variable] of this.getExportedVariablesByName().entries()) {
			...
			if (!variable || !(variable.included || variable instanceof ExternalVariable)) continue;
			const existingExportNames = exportNamesByVariable.get(tracedVariable);
			if (existingExportNames) existingExportNames.push(exportName);
			else exportNamesByVariable.set(tracedVariable, [exportName]);
		}
	...
	getRenderedExports() {
		...
		for (const exportName of this.exportDescriptions.keys()) (this.getExportedVariablesByName().get(exportName)?.included ? renderedExports : removedExports).push(exportName);
		return {
			removedExports,
			renderedExports
		};
	...
	getSyntheticNamespace() {
		...
		if (!this.syntheticNamespace) return error(logSyntheticNamedExportsNeedNamespaceExport(this.id, this.info.syntheticNamedExports));
	...
	getVariableForExportName(name, { importerForSideEffects, importChain = [], isExportAllSearch, onlyExplicit, searchedNamesAndModules, sideEffectModules, exportOrReexportModules } = EMPTY_OBJECT) {
		if (name[0] === "*") {
			if (name.length === 1) return [this.namespace];
			const module = this.graph.modulesById.get(name.slice(1));
			return module.getVariableForExportName("*", { importChain: [...importChain, this.id] });
		}
		...
		if (reexportDeclaration) {
			...
			if (!variable) return this.error(logMissingExport(reexportDeclaration.localName, this.id, reexportDeclaration.module.id, !!options?.missingButExportExists), reexportDeclaration.start);
			if (importerForSideEffects) {
				setAlternativeExporterIfCyclic(variable, importerForSideEffects, this);
				if (this.info.moduleSideEffects) getOrCreate(importerForSideEffects.sideEffectDependenciesByVariable, variable, getNewSet).add(this);
			}
			if (this.info.moduleSideEffects) sideEffectModules?.add(this);
			exportOrReexportModules?.add(this);
			return [variable];
		}
		const exportDeclaration = this.exportDescriptions.get(name);
		if (exportDeclaration) {
			if (exportDeclaration === MISSING_EXPORT_SHIM_DESCRIPTION) return [this.exportShimVariable];
			const name$1 = exportDeclaration.localName;
			const variable = this.traceVariable(name$1, {
				exportOrReexportModules,
				importerForSideEffects,
				searchedNamesAndModules,
				sideEffectModules
			});
			if (!variable) return [null, { missingButExportExists: true }];
			if (importerForSideEffects) {
				setAlternativeExporterIfCyclic(variable, importerForSideEffects, this);
		...
		if (name !== "default") {
			...
			if (foundNamespaceReexport[0]) {
				...
				if (importerForSideEffects) {
					const { exportOrReexportModules: exportOrReexportModules$1, sideEffectModules: sideEffectModules$1 } = namespaceReexportOptions;
					for (const module of exportOrReexportModules$1) {
						if (importerForSideEffects.alternativeReexportModules.has(namespaceReexportVariable)) continue;
						setAlternativeExporterIfCyclic(namespaceReexportVariable, importerForSideEffects, module);
					}
					for (const module of sideEffectModules$1) getOrCreate(importerForSideEffects.sideEffectDependenciesByVariable, namespaceReexportVariable, getNewSet).add(module);
				}
				return foundNamespaceReexport;
			}
		}
		if (this.info.syntheticNamedExports) return [getOrCreate(this.syntheticExports, name, () => new SyntheticNamedExportVariable(this.astContext, name, this.getSyntheticNamespace()))];
	...
	includeAllExports() {
		...
		for (const variable of this.getExportedVariablesByName().values()) {
			...
			variable.deoptimizePath(UNKNOWN_PATH);
			if (variable instanceof ExternalVariable) variable.module.reexported = true;
		}
	}
	includeAllInBundle() {
	...
	render(options) {
		...
		if (usesTopLevelAwait && options.format !== "es" && options.format !== "system") return error(logInvalidFormatForTopLevelAwait(this.id, options.format));
		return {
			source,
			usesTopLevelAwait
		};
	...
	async setSource({ ast, code, customTransformCache, originalCode, originalSourcemap, resolvedIds, sourcemapChain, transformDependencies, transformFiles, safeVariableNames,...moduleOptions }) {
		...
		if (transformFiles) this.transformFiles = transformFiles;
		...
		if (ast) {
			...
			Object.defineProperty(this.info, "ast", { get: () => {
				if (this.graph.astLru.has(fileName)) return this.graph.astLru.get(fileName);
				else {
					const parsedAst = this.tryParse();
					if (this.options.cache !== false) {
						Object.defineProperty(this.info, "ast", { value: parsedAst });
						return parsedAst;
					}
	...
	traceVariable(name, { importerForSideEffects, isExportAllSearch, searchedNamesAndModules, sideEffectModules, exportOrReexportModules } = EMPTY_OBJECT) {
		...
		if (importDescription) {
			...
			if (otherModule instanceof Module && importDescription.name === "*") return otherModule.namespace;
			...
			if (!declaration) return this.error(logMissingExport(importDescription.name, this.id, otherModule.id, !!options?.missingButExportExists), importDescription.start);
			return declaration;
		}
		return null;
	}
	updateOptions({ meta, moduleSideEffects, syntheticNamedExports }) {
		if (moduleSideEffects != null) this.info.moduleSideEffects = moduleSideEffects;
		if (syntheticNamedExports != null) this.info.syntheticNamedExports = syntheticNamedExports;
		if (meta != null) Object.assign(this.info.meta, meta);
	}
	addDynamicImport(node) {
		let argument = node.sourceAstNode;
		if (argument.type === TemplateLiteral) {
			if (argument.quasis.length === 1 && typeof argument.quasis[0].value.cooked === "string") argument = argument.quasis[0].value.cooked;
		} else if (argument.type === Literal && typeof argument.value === "string") argument = argument.value;
	...
	assertUniqueExportName(name, nodeStart) {
		if (this.exportDescriptions.has(name) || this.reexportDescriptions.has(name)) this.error(logDuplicateExportError(name), nodeStart);
	}
	addExport(node) {
		if (node instanceof ExportDefaultDeclaration$1) {
			...
			if (node.exported) {
				const name = node.exported instanceof Literal$1 ? node.exported.value : node.exported.name;
			...
			} else this.exportAllSources.add(source);
		} else if (node.source instanceof Literal$1) {
			const source = node.source.value;
			this.addSource(source, node);
			for (const { exported, local, start } of node.specifiers) {
				const name = exported instanceof Literal$1 ? exported.value : exported.name;
				this.assertUniqueExportName(name, start);
				this.reexportDescriptions.set(name, {
					localName: local instanceof Literal$1 ? local.value : local.name,
					module: null,
					source,
					start
				});
			...
			if (declaration instanceof VariableDeclaration) for (const declarator of declaration.declarations) for (const localName of extractAssignedNames(declarator.id)) {
				this.assertUniqueExportName(localName, declarator.id.start);
				this.exportDescriptions.set(localName, {
					identifier: null,
					localName
				});
			}
		...
		} else for (const { local, exported } of node.specifiers) {
			const localName = local.name;
			const exportedName = exported instanceof Identifier$1 ? exported.name : exported.value;
			this.assertUniqueExportName(exportedName, exported.start);
			this.exportDescriptions.set(exportedName, {
				identifier: null,
				localName
			});
		}
	...
	addImport(node) {
		...
		for (const specifier of node.specifiers) {
			...
			if (this.scope.variables.has(localName) || this.importDescriptions.has(localName)) this.error(logRedeclarationError(localName), specifier.local.start);
			const name = node.phase === "source" ? SOURCE_PHASE_IMPORT : specifier instanceof ImportDefaultSpecifier ? "default" : specifier instanceof ImportNamespaceSpecifier ? "*" : specifier.imported instanceof Identifier$1 ? specifier.imported.name : specifier.imported.value;
	...
	addImportSource(importSource) {
		if (importSource && !this.sourcesWithAttributes.has(importSource)) this.sourcesWithAttributes.set(importSource, EMPTY_OBJECT);
	}
	addImportMeta(node) {
	...
	addRelevantSideEffectDependencies(relevantDependencies, necessaryDependencies, alwaysCheckedDependencies) {
		...
		const addSideEffectDependencies = (possibleDependencies) => {
			for (const dependency of possibleDependencies) {
				if (handledDependencies.has(dependency)) continue;
				handledDependencies.add(dependency);
				if (necessaryDependencies.has(dependency)) {
					relevantDependencies.add(dependency);
					continue;
				}
				if (!(dependency.info.moduleSideEffects || alwaysCheckedDependencies.has(dependency))) continue;
	...
	addSource(source, declaration) {
		...
		if (existingAttributes) {
			if (doAttributesDiffer(existingAttributes, parsedAttributes)) this.log(LOGLEVEL_WARN, logInconsistentImportAttributes(existingAttributes, parsedAttributes, source, this.id), declaration.start);
		} else this.sourcesWithAttributes.set(source, parsedAttributes);
		if (declaration.phase === "source") this.sourcePhaseSources.add(source);
	}
	getImportedJsxFactoryVariable(baseName, nodeStart, importSource) {
		...
		if (!variable) return this.error(logMissingJsxExport(baseName, id, this.id), nodeStart);
		return variable;
	}
	getVariableFromNamespaceReexports(name, importerForSideEffects, searchedNamesAndModules, importChain) {
		...
		for (const module of this.exportAllModules) {
			if (module.info.syntheticNamedExports === name) continue;
			...
			if (module instanceof ExternalModule || options?.indirectExternal) foundExternalDeclarations.add(variable);
			else if (variable instanceof SyntheticNamedExportVariable) {
				if (!foundSyntheticDeclaration) foundSyntheticDeclaration = variable;
			} else if (variable) foundInternalDeclarations.set(variable, module);
		}
		if (foundInternalDeclarations.size > 0) {
			...
			if (foundDeclarationList.length === 1) return [usedDeclaration, {
				exportOrReexportModules,
				sideEffectModules
			}];
		...
		if (foundExternalDeclarations.size > 0) {
			...
			if (foundDeclarationList.length > 1) this.options.onLog(LOGLEVEL_WARN, logAmbiguousExternalNamespaces(name, this.id, usedDeclaration.module.id, foundDeclarationList.map((declaration) => declaration.module.id)));
			return [usedDeclaration, {
				exportOrReexportModules,
				indirectExternal: true,
				sideEffectModules
			}];
		}
		if (foundSyntheticDeclaration) return [foundSyntheticDeclaration, {
			exportOrReexportModules,
			sideEffectModules
		}];
		return [null];
	...
	includeAndGetAdditionalMergedNamespaces() {
		...
		for (const module of [this, ...this.exportAllModules]) if (module instanceof ExternalModule) {
			const [externalVariable] = module.getVariableForExportName("*", { importChain: [this.id] });
	...
	includeDynamicImport(node) {
		...
		if (resolution instanceof Module) {
			if (!resolution.includedDynamicImporters.includes(this)) {
				...
				if (this.astContext.usesTopLevelAwait) resolution.includedTopLevelAwaitingDynamicImporters.add(this);
			}
		}
	}
	includeVariable(variable, path$2, context) {
		const { included, module: variableModule } = variable;
		variable.includePath(path$2, context);
		if (included) {
			if (variableModule instanceof Module && variableModule !== this) getAndExtendSideEffectModules(variable, this);
			return;
		}
		this.graph.needsTreeshakingPass = true;
		if (!(variableModule instanceof Module)) return;
		variableModule.includeModuleInExecution();
		if (variableModule !== this) {
			const sideEffectModules = getAndExtendSideEffectModules(variable, this);
			for (const module of sideEffectModules) module.includeModuleInExecution();
		}
	}
	includeVariableInModule(variable, path$2, context) {
		this.includeVariable(variable, path$2, context);
		const variableModule = variable.module;
		if (variableModule && variableModule !== this) this.includedImports.add(variable);
	}
...
function setAlternativeExporterIfCyclic(variable, importer, reexporter) {
	if (variable.module instanceof Module && variable.module !== reexporter) {
		...
		if (exporterCycles.size > 0) {
			...
			for (const cycleSymbol of importerCycles) if (exporterCycles.has(cycleSymbol)) {
...
const copyNameToModulesMap = (searchedNamesAndModules) => searchedNamesAndModules && new Map(Array.from(searchedNamesAndModules, ([name, modules]) => [name, new Set(modules)]));
...
function deconflictChunk(modules, dependenciesToBeDeconflicted, imports, usedNames, format, interop, preserveModules, externalLiveBindings, chunkByModule, externalChunkByModule, syntheticExports, exportNamesByVariable, accessedGlobalsByScope, includedNamespaces) {
	...
	for (const module of reversedModules) module.scope.addUsedOutsideNames(usedNames, format, exportNamesByVariable, accessedGlobalsByScope);
	...
	for (const module of reversedModules) module.scope.deconflict(format, exportNamesByVariable, accessedGlobalsByScope);
}
function deconflictImportsEsmOrSystem(usedNames, imports, dependenciesToBeDeconflicted, _interop, preserveModules, _externalLiveBindings, chunkByModule, externalChunkByModule, syntheticExports) {
	for (const dependency of dependenciesToBeDeconflicted.dependencies) if (preserveModules || dependency instanceof ExternalChunk) dependency.variableName = getSafeName(dependency.suggestedVariableName, usedNames, null);
	for (const variable of imports) {
		const module = variable.module;
		const name = variable.name;
		if (variable.isNamespace && (preserveModules || module instanceof ExternalModule)) variable.setRenderNames(null, (module instanceof ExternalModule ? externalChunkByModule.get(module) : chunkByModule.get(module)).variableName);
		else if (module instanceof ExternalModule && variable.isSourcePhase) variable.setRenderNames(null, getSafeName(module.suggestedVariableName + "__source", usedNames, variable.forbiddenNames));
		else if (module instanceof ExternalModule && name === "default") variable.setRenderNames(null, getSafeName([...module.exportedVariables].some(([exportedVariable, exportedName]) => exportedName === "*" && exportedVariable.included) ? module.suggestedVariableName + "__default" : module.suggestedVariableName, usedNames, variable.forbiddenNames));
		else variable.setRenderNames(null, getSafeName(makeLegal(name), usedNames, variable.forbiddenNames));
	}
	for (const variable of syntheticExports) variable.setRenderNames(null, getSafeName(variable.name, usedNames, variable.forbiddenNames));
}
function deconflictImportsOther(usedNames, imports, { deconflictedDefault, deconflictedNamespace, dependencies }, interop, preserveModules, externalLiveBindings, chunkByModule, externalChunkByModule) {
	for (const chunk of dependencies) chunk.variableName = getSafeName(chunk.suggestedVariableName, usedNames, null);
	for (const chunk of deconflictedNamespace) chunk.namespaceVariableName = getSafeName(`${chunk.suggestedVariableName}__namespace`, usedNames, null);
	for (const externalModule of deconflictedDefault) externalModule.defaultVariableName = deconflictedNamespace.has(externalModule) && canDefaultBeTakenFromNamespace(interop(externalModule.id), externalLiveBindings) ? externalModule.namespaceVariableName : getSafeName(`${externalModule.suggestedVariableName}__default`, usedNames, null);
	for (const variable of imports) {
		const module = variable.module;
		if (module instanceof ExternalModule) {
			...
			if (name === "default") {
				...
				if (isDefaultAProperty(moduleInterop, externalLiveBindings)) variable.setRenderNames(variableName, "default");
				else variable.setRenderNames(null, variableName);
			} else if (name === "*") variable.setRenderNames(null, namespaceInteropHelpersByInteropType[interop(module.id)] ? chunk.namespaceVariableName : chunk.variableName);
			else variable.setRenderNames(chunk.variableName, null);
		} else {
			const chunk = chunkByModule.get(module);
			if (preserveModules && variable.isNamespace) variable.setRenderNames(null, chunk.exportMode === "default" ? chunk.namespaceVariableName : chunk.variableName);
			else if (chunk.exportMode === "default") variable.setRenderNames(null, chunk.variableName);
			else variable.setRenderNames(chunk.variableName, chunk.getVariableExportName(variable));
		}
	}
}
function deconflictTopLevelVariables(usedNames, modules, includedNamespaces) {
	for (const module of modules) {
		...
		for (const variable of module.scope.variables.values()) if (variable.included && !(variable.renderBaseName || variable instanceof ExportDefaultVariable && variable.getOriginalVariable() !== variable)) {
...
function assignExportsToMangledNames(exports, exportsByName, exportNamesByVariable) {
	...
	for (const variable of exports) {
		...
		if (exportsByName.has(exportName)) do {
...
function assignExportsToNames(exports, exportsByName, exportNamesByVariable) {
	for (const variable of exports) {
		let nameIndex = 0;
		let exportName = variable.name;
		while (exportsByName.has(exportName)) exportName = variable.name + "$" + ++nameIndex;
		exportsByName.set(exportName, variable);
		exportNamesByVariable.set(variable, [exportName]);
	}
}
function getExportMode(chunk, { exports: exportMode, name, format }, facadeModuleId, log) {
	...
	if (exportMode === "default") {
		if (exportKeys.length !== 1 || exportKeys[0] !== "default") return error(logIncompatibleExportOptionValue("default", exportKeys, facadeModuleId));
	} else if (exportMode === "none" && exportKeys.length > 0) return error(logIncompatibleExportOptionValue("none", exportKeys, facadeModuleId));
	if (exportMode === "auto") if (exportKeys.length === 0) exportMode = "none";
	else if (exportKeys.length === 1 && exportKeys[0] === "default") exportMode = "default";
	else {
		if (format !== "es" && format !== "system" && exportKeys.includes("default")) log(LOGLEVEL_WARN, logMixedExport(facadeModuleId, name));
		exportMode = "named";
	}
	return exportMode;
}
...
function getStaticDependencies(chunk, orderedModules, chunkByModule, externalChunkByModule) {
	...
	for (const block of staticDependencyBlocks) for (const dependency of block) dependencies.add(dependency);
	return dependencies;
}
...
const getHashPlaceholderGenerator = () => {
	...
	return (optionName, hashSize) => {
		if (hashSize > MAX_HASH_SIZE) return error(logFailedValidation(`Hashes cannot be longer than ${MAX_HASH_SIZE} characters, received ${hashSize}. Check the "${optionName}" option.`));
		...
		if (placeholder.length > hashSize) return error(logFailedValidation(`To generate hashes for this number of chunks (currently ${nextIndex}), you need a minimum hash size of ${placeholder.length}, received ${hashSize}. Check the "${optionName}" option.`));
		return placeholder;
	};
};
...
const FILE_PLACEHOLDER = { type: "placeholder" };
const getOutputBundle = (outputBundleBase) => {
	const reservedLowercaseBundleKeys = new Set();
	return new Proxy(outputBundleBase, {
		deleteProperty(target, key) {
			if (typeof key === "string") reservedLowercaseBundleKeys.delete(key.toLowerCase());
			return Reflect.deleteProperty(target, key);
		},
		get(target, key) {
			if (key === lowercaseBundleKeys) return reservedLowercaseBundleKeys;
			return Reflect.get(target, key);
		},
		set(target, key, value) {
			if (typeof key === "string") reservedLowercaseBundleKeys.add(key.toLowerCase());
...
const removeUnreferencedAssets = (outputBundle) => {
	...
	for (const asset of bundleEntries) if (asset.type === "asset" && asset.needsCodeReference) unreferencedAssets.add(asset.fileName);
	for (const chunk of bundleEntries) if (chunk.type === "chunk") {
		for (const referencedFile of chunk.referencedFiles) if (unreferencedAssets.has(referencedFile)) unreferencedAssets.delete(referencedFile);
	}
	for (const file of unreferencedAssets) delete outputBundle[file];
};
function renderNamePattern(pattern, patternName, replacements) {
	if (isPathFragment(pattern)) return error(logFailedValidation(`Invalid pattern "${pattern}" for "${patternName}", patterns can be neither absolute nor relative paths. If you want your files to be stored in a subdirectory, write its name without a leading slash like this: subdirectory/pattern.`));
	return pattern.replace(/\[(\w+)(:\d+)?]/g, (_match, type, size) => {
		if (!replacements.hasOwnProperty(type) || size && type !== "hash") return error(logFailedValidation(`"[${type}${size || ""}]" is not a valid placeholder in the "${patternName}" pattern.`));
		const replacement = replacements[type](size && Number.parseInt(size.slice(1)));
		if (isPathFragment(replacement)) return error(logFailedValidation(`Invalid substitution "${replacement}" for placeholder "[${type}]" in "${patternName}" pattern, can be neither absolute nor relative path.`));
		return replacement;
	});
}
function makeUnique(name, { [lowercaseBundleKeys]: reservedLowercaseBundleKeys }) {
	...
	const base$1 = directory + (dotIndex === -1 ? fileName : fileName.slice(0, dotIndex));
	const extension = dotIndex === -1 ? "" : fileName.slice(dotIndex);
	let uniqueName, uniqueIndex = 1;
	while (reservedLowercaseBundleKeys.has((uniqueName = base$1 + ++uniqueIndex + extension).toLowerCase()));
	return uniqueName;
}
...
var Chunk = class Chunk {
	constructor(orderedModules, inputOptions, outputOptions, unsetOptions, pluginDriver, modulesById, chunkByModule, externalChunkByModule, facadeChunkByModule, includedNamespaces, manualChunkAlias, getPlaceholder, bundle, inputBase, snippets) {
		...
		for (const module of orderedModules) {
			...
			if (module.namespace.included && !outputOptions.preserveModules) includedNamespaces.add(module);
			if (this.isEmpty && module.isIncluded()) this.isEmpty = false;
			if (module.info.isEntry || outputOptions.preserveModules) this.entryModules.push(module);
			for (const importer of module.includedDynamicImporters) if (!chunkModules.has(importer)) {
			...
			if (module.implicitlyLoadedAfter.size > 0) this.implicitEntryModules.push(module);
		}
	...
	static generateFacade(inputOptions, outputOptions, unsetOptions, pluginDriver, modulesById, chunkByModule, externalChunkByModule, facadeChunkByModule, includedNamespaces, facadedModule, facadeName, getPlaceholder, bundle, inputBase, snippets) {
		...
		if (!facadeChunkByModule.has(facadedModule)) facadeChunkByModule.set(facadedModule, chunk);
		for (const dependency of facadedModule.getDependenciesToBeIncluded()) chunk.dependencies.add(dependency instanceof Module ? chunkByModule.get(dependency) : externalChunkByModule.get(dependency));
		if (!chunk.dependencies.has(chunkByModule.get(facadedModule)) && facadedModule.info.moduleSideEffects && facadedModule.hasEffects()) chunk.dependencies.add(chunkByModule.get(facadedModule));
	...
	canModuleBeFacade(module, exposedVariables) {
		...
		for (const exposedVariable of this.exports) if (!moduleExportNamesByVariable.has(exposedVariable)) return false;
		for (const exposedVariable of exposedVariables) if (!(exposedVariable.module === module || moduleExportNamesByVariable.has(exposedVariable) || exposedVariable instanceof SyntheticNamedExportVariable && moduleExportNamesByVariable.has(exposedVariable.getBaseVariable()))) return false;
		return true;
	}
	finalizeChunk(code, map, sourcemapFileName, hashesByPlaceholder) {
		const renderedChunkInfo = this.getRenderedChunkInfo();
		const finalize = (code$1) => replacePlaceholders(code$1, hashesByPlaceholder);
		...
		return {
			...
			importedBindings: Object.fromEntries(Object.entries(renderedChunkInfo.importedBindings).map(([fileName$1, bindings]) => [finalize(fileName$1), bindings])),
	...
	generateExports() {
		...
		if (this.facadeModule !== null && (this.facadeModule.preserveSignature !== false || this.strictFacade)) {
			...
			for (const [variable, exportNames] of exportNamesByVariable) {
				...
				for (const exportName of exportNames) this.exportsByName.set(exportName, variable);
		...
		if (this.outputOptions.minifyInternalExports) assignExportsToMangledNames(remainingExports, this.exportsByName, this.exportNamesByVariable);
		else assignExportsToNames(remainingExports, this.exportsByName, this.exportNamesByVariable);
		if (this.outputOptions.preserveModules || this.facadeModule && this.facadeModule.info.isEntry) this.exportMode = getExportMode(this, this.outputOptions, this.facadeModule.id, this.inputOptions.onLog);
	}
	generateFacades() {
		const facades = [];
		...
		for (const module of entryModules) {
			...
			if (requiredFacades.length === 0 && module.isUserDefinedEntryPoint) requiredFacades.push({});
			requiredFacades.push(...Array.from(module.chunkFileNames, (fileName) => ({ fileName })));
			if (requiredFacades.length === 0) requiredFacades.push({});
			if (!this.facadeModule) {
				...
				if (!needsStrictFacade || this.canModuleBeFacade(module, exposedVariables)) {
					this.facadeModule = module;
					this.facadeChunkByModule.set(module, this);
					if (module.preserveSignature) this.strictFacade = needsStrictFacade;
					this.assignFacadeName(requiredFacades.shift(), module, this.outputOptions.preserveModules);
				}
			}
			for (const facadeName of requiredFacades) facades.push(Chunk.generateFacade(this.inputOptions, this.outputOptions, this.unsetOptions, this.pluginDriver, this.modulesById, this.chunkByModule, this.externalChunkByModule, this.facadeChunkByModule, this.includedNamespaces, module, facadeName, this.getPlaceholder, this.bundle, this.inputBase, this.snippets));
		}
		for (const module of this.dynamicEntryModules) {
			...
			} else if (this.facadeModule === module && !this.strictFacade && this.canModuleBeFacade(module, exposedVariables)) this.strictFacade = true;
			else if (!this.facadeChunkByModule.get(module)?.strictFacade) {
				this.includedNamespaces.add(module);
				this.exports.add(module.namespace);
			}
		}
		if (!this.outputOptions.preserveModules) this.addNecessaryImportsForFacades();
		return facades;
	}
	canPreserveModuleExports(module) {
		const exportNamesByVariable = module.getExportNamesByVariable();
		for (const [variable, exportNames] of exportNamesByVariable) for (const exportName of exportNames) {
			const existingVariable = this.exportsByName.get(exportName);
			if (existingVariable && existingVariable !== variable) return false;
		}
		for (const [variable, exportNames] of exportNamesByVariable) for (const exportName of exportNames) this.exportsByName.set(exportName, variable);
		return true;
	}
	getChunkName() {
	...
	getPreliminaryFileName() {
		...
		if (file) fileName = basename$1(file);
			...
			if (!hashPlaceholder) fileName = makeUnique(fileName, this.bundle);
		} else fileName = this.fileName;
		if (!hashPlaceholder) this.bundle[fileName] = FILE_PLACEHOLDER;
	...
	getPreliminarySourcemapFileName() {
		...
		if (!sourcemapFileNames) return null;
		let hashPlaceholder = null;
		...
		if (!hashPlaceholder) sourcemapFileName = makeUnique(sourcemapFileName, this.bundle);
	...
	getRenderedChunkInfo() {
		if (this.renderedChunkInfo) return this.renderedChunkInfo;
	...
	getVariableExportName(variable) {
		if (this.outputOptions.preserveModules && variable instanceof NamespaceVariable) return "*";
	...
	inlineTransitiveImports() {
		...
		if (hoistTransitiveImports && !preserveModules && facadeModule !== null) {
			for (const dep of dependencies) if (dep instanceof Chunk) this.inlineChunkDependencies(dep);
		}
	}
	async render() {
		...
		for (const renderedDependency of renderedDependencies) {
			...
			if (reexports?.length) {
				...
				if (format === "es") renderedDependency.reexports = reexports.filter(({ reexported }) => !renderedExports.find(({ exported }) => exported === reexported));
			}
		}
		if (!hasDefaultExport) {
			for (const { exported } of renderedExports) if (exported === "default") {
		...
		if (format === "es" || format === "cjs") {
			...
			if (shebang) magicString.prepend(`#!${shebang}\n`);
		}
	...
	addImplicitlyLoadedBeforeFromModule(baseModule) {
		...
		for (const module of baseModule.implicitlyLoadedBefore) {
			const chunk = chunkByModule.get(module);
			if (chunk && chunk !== this) implicitlyLoadedBefore.add(chunk);
		}
	}
	addNecessaryImportsForFacades() {
		for (const [module, variables] of this.includedReexportsByModule) if (this.includedNamespaces.has(module)) for (const variable of variables) this.imports.add(variable);
	}
	assignFacadeName({ fileName, name }, facadedModule, preservePath) {
		if (fileName) this.fileName = fileName;
		else this.name = this.outputOptions.sanitizeFileName(name || (preservePath ? this.getPreserveModulesChunkNameFromModule(facadedModule) : getChunkNameFromModule(facadedModule)));
	}
		...
		if (variableModule instanceof Module) {
			...
			do {
				...
				if (alternativeReexportModule) {
					...
					if (exportingChunk !== exportChunk) this.inputOptions.onLog(LOGLEVEL_WARN, logCyclicCrossChunkReexport(
			...
			if (!(importedVariable instanceof NamespaceVariable && this.outputOptions.preserveModules)) {
				...
				if (exportingModule instanceof Module) {
					...
					if (chunk && chunk !== this) {
						...
						if (isSynthetic) this.imports.add(importedVariable);
					}
				}
			}
		}
		if (includedReexports.length > 0) this.includedReexportsByModule.set(module, includedReexports);
	}
	generateVariableName() {
		...
		if (moduleForNaming) return getChunkNameFromModule(moduleForNaming);
		return "chunk";
	}
	getChunkExportDeclarations(format) {
		...
		for (const exportName of this.getExportNames()) {
			...
			if (variable instanceof LocalVariable) {
				for (const declaration of variable.declarations) if (declaration.parent instanceof FunctionDeclaration || declaration instanceof ExportDefaultDeclaration$1 && declaration.declaration instanceof FunctionDeclaration) {
					hoisted = true;
					break;
				}
			} else if (variable instanceof SyntheticNamedExportVariable) {
				expression = local;
				if (format === "es") local = variable.renderName;
			}
			exports.push({
				exported: exportName,
				expression,
	...
	getDependenciesToBeDeconflicted(addNonNamespacesAndInteropHelpers, addDependenciesWithoutBindings, interop) {
		...
		for (const variable of [...this.exportNamesByVariable.keys(), ...this.imports]) if (addNonNamespacesAndInteropHelpers || variable.isNamespace) {
			const module = variable.module;
			if (module instanceof ExternalModule) {
				...
				if (addNonNamespacesAndInteropHelpers) {
					if (variable.name === "default") {
						if (defaultInteropHelpersByInteropType[interop(module.id)]) deconflictedDefault.add(chunk);
					} else if (variable.isNamespace && namespaceInteropHelpersByInteropType[interop(module.id)] && (this.imports.has(variable) || !this.exportNamesByVariable.get(variable)?.every((name) => name[0] === "*"))) deconflictedNamespace.add(chunk);
				}
			} else {
				const chunk = this.chunkByModule.get(module);
				if (chunk !== this) {
					dependencies.add(chunk);
					if (addNonNamespacesAndInteropHelpers && chunk.exportMode === "default" && variable.isNamespace) deconflictedNamespace.add(chunk);
				}
			}
		}
		if (addDependenciesWithoutBindings) for (const dependency of this.dependencies) dependencies.add(dependency);
		return {
			deconflictedDefault,
	...
	getDynamicImportStringAndAttributes(resolution, fileName, node) {
		...
		if (resolution instanceof ExternalModule) {
			...
			return [`'${chunk.getImportPath(fileName)}'`, dynamicAttributes || (keepExternalImportAttributes ? true : null)];
		}
		let attributes = null;
	...
	getFallbackChunkName() {
		if (this.manualChunkAlias) return this.manualChunkAlias;
		if (this.dynamicName) return this.dynamicName;
		if (this.fileName) return getAliasName(this.fileName);
	...
	getImportSpecifiers() {
		...
		for (const variable of this.imports) {
			...
			if (module instanceof ExternalModule) {
				...
				if (!isSourcePhase && imported !== "default" && imported !== "*" && interop(module.id) === "defaultOnly") return error(logUnexpectedNamedImport(module.id, imported, false));
			} else {
	...
	getIncludedDynamicImports() {
		...
		for (const module of this.orderedModules) for (const { node } of module.dynamicImports) {
	...
	getPreserveModulesChunkNameFromModule(module) {
		...
		const { preserveModulesRoot, sanitizeFileName: sanitizeFileName$1 } = this.outputOptions;
		const sanitizedId = sanitizeFileName$1(normalize(module.id.split(QUERY_HASH_REGEX, 1)[0]));
		...
		if (isAbsolute(idWithoutExtension)) if (preserveModulesRoot && resolve$1(idWithoutExtension).startsWith(preserveModulesRoot)) return idWithoutExtension.slice(preserveModulesRoot.length).replace(/^[/\\]/, "");
		else {
			if (this.inputBase === "/" && idWithoutExtension[0] !== "/") return relative(this.inputBase, idWithoutExtension.replace(/^[a-zA-Z]:[/\\]/, "/"));
			return relative(this.inputBase, idWithoutExtension);
		}
		else return this.outputOptions.virtualDirname.replace(/\/$/, "") + "/" + basename$1(idWithoutExtension);
	}
	getReexportSpecifiers() {
		...
		for (let exportName of this.getExportNames()) {
			...
			if (exportName[0] === "*") {
				...
				if (interop(id) === "defaultOnly") this.inputOptions.onLog(LOGLEVEL_WARN, logUnexpectedNamespaceReexport(id));
				...
				if (module instanceof Module) {
					...
					dependency = this.externalChunkByModule.get(module);
					imported = variable.name;
					if (imported !== "default" && imported !== "*" && interop(module.id) === "defaultOnly") return error(logUnexpectedNamedImport(module.id, imported, true));
	...
	getReferencedFiles() {
		...
		for (const module of this.orderedModules) for (const meta of module.importMetas) {
	...
	getRenderedDependencies() {
		...
		for (const dependency of this.dependencies) {
			...
			if (imports === null && reexports === null && dependency instanceof ExternalChunk && !dependency.moduleSideEffects && !this.outputOptions.hoistTransitiveImports) continue;
	...
	renderModules(fileName) {
		...
		const magicString = new Bundle$1({ separator: `${n$1}${n$1}` });
		...
		for (const module of orderedModules) {
			...
			if (module.isIncluded() || includedNamespaces.has(module)) {
				...
				if (!renderOptions.accessedDocumentCurrentScript && formatsMaybeAccessDocumentCurrentScript.includes(format)) this.accessedGlobalsByScope.get(module.scope)?.delete(DOCUMENT_CURRENT_SCRIPT);
				...
				const namespace = module.namespace;
				if (includedNamespaces.has(module)) {
					const rendered$1 = namespace.renderBlock(renderOptions);
					if (namespace.renderFirst()) hoistedSource += n$1 + rendered$1;
					else magicString.addSource(new MagicString(rendered$1));
				}
				const accessedGlobalVariables = accessedGlobalsByScope.get(module.scope);
				if (accessedGlobalVariables) for (const name of accessedGlobalVariables) accessedGlobals.add(name);
			}
		...
		if (hoistedSource) magicString.prepend(hoistedSource + n$1 + n$1);
		if (this.needsExportsShim) magicString.prepend(`${n$1}${cnst} ${MISSING_EXPORT_SHIM_VARIABLE}${_}=${_}void 0;${n$1}${n$1}`);
		const renderedSource = compact ? magicString : magicString.trim();
		if (isEmpty && this.getExportNames().length === 0 && dependencies.size === 0) onLog(LOGLEVEL_WARN, logEmptyChunk(this.getChunkName()));
		return {
			accessedGlobals,
			indent,
			magicString,
	...
	setDynamicImportResolutions(fileName) {
		...
		for (const resolvedDynamicImport of this.getIncludedDynamicImports()) if (resolvedDynamicImport.chunk) {
			const { chunk, facadeChunk, node, resolution } = resolvedDynamicImport;
			if (chunk === this) node.setInternalResolution(resolution.namespace);
			else node.setExternalResolution((facadeChunk || chunk).exportMode, outputOptions, snippets, pluginDriver, accessedGlobalsByScope, `'${(facadeChunk || chunk).getImportPath(fileName)}'`, !facadeChunk?.strictFacade && chunk.exportNamesByVariable.get(resolution.namespace)[0], null, this, facadeChunk || chunk);
		} else {
	...
	setIdentifierRenderResolutions() {
		...
		for (const module of this.orderedModules) for (const variable of module.scope.variables.values()) variable.setRenderNames(null, null);
		const syntheticExports = new Set();
		for (const exportName of this.getExportNames()) {
			const exportVariable = this.exportsByName.get(exportName);
			if (format !== "es" && format !== "system" && exportVariable.isReassigned && !exportVariable.isId) exportVariable.setRenderNames("exports", exportName);
			else if (exportVariable instanceof SyntheticNamedExportVariable) syntheticExports.add(exportVariable);
			else exportVariable.setRenderNames(null, null);
		}
		for (const module of this.orderedModules) if (module.needsExportShim) {
			this.needsExportsShim = true;
			break;
		}
		const usedNames = new Set(RESERVED_USED_NAMES);
		if (this.needsExportsShim) usedNames.add(MISSING_EXPORT_SHIM_VARIABLE);
		if (symbols) usedNames.add("Symbol");
	...
	setImportMetaResolutions(fileName) {
		...
		for (const module of orderedModules) {
			for (const importMeta of module.importMetas) importMeta.setResolution(format, accessedGlobalsByScope, fileName);
			if (includedNamespaces.has(module)) module.namespace.prepare(accessedGlobalsByScope);
		}
	}
	setUpChunkImportsAndExportsForModule(module) {
		...
		if (!this.outputOptions.preserveModules && this.includedNamespaces.has(module)) {
			for (const variable of module.getExportedVariablesByName().values()) if (variable.included) moduleImports.add(variable);
		}
		for (let variable of moduleImports) {
			if (variable instanceof ExportDefaultVariable) variable = variable.getOriginalVariable();
			if (variable instanceof SyntheticNamedExportVariable) variable = variable.getBaseVariable();
			...
			if (chunk !== this) {
				...
				if (variable.module instanceof Module) {
					...
					if (!(variable instanceof NamespaceVariable && this.outputOptions.preserveModules)) chunk.exports.add(variable);
				}
			}
		}
		if (this.includedNamespaces.has(module) || module.info.isEntry && module.preserveSignature !== false || module.includedDynamicImporters.some((importer) => this.chunkByModule.get(importer) !== this)) this.ensureReexportsAreAvailableForModule(module);
		for (const { node: { included, resolution } } of module.dynamicImports) if (included && resolution instanceof Module && this.chunkByModule.get(resolution) === this && !this.includedNamespaces.has(resolution)) {
...
function getImportedBindingsPerDependency(renderedDependencies, resolveFileName$1) {
	const importedBindingsPerDependency = {};
	for (const [dependency, declaration] of renderedDependencies) {
		const specifiers = new Set();
		if (declaration.imports) for (const { imported } of declaration.imports) specifiers.add(imported);
		if (declaration.reexports) for (const { imported } of declaration.reexports) specifiers.add(imported);
		importedBindingsPerDependency[resolveFileName$1(dependency)] = [...specifiers];
	}
...
function getChunkDefinitionsFromManualChunks(manualChunkAliasByEntry, isManualChunksFunctionForm, onlyExplicitManualChunks) {
	...
	for (const [entry, alias] of sortedEntriesWithAlias) {
		...
		if (isManualChunksFunctionForm && onlyExplicitManualChunks) chunkModules.push(entry);
		else addStaticDependenciesToManualChunk(entry, chunkModules, modulesInManualChunks);
	}
	...
	for (const [alias, modules] of manualChunks) {
		...
		for (const module of modules) manualChunkModulesByModule.set(module, modules);
		index++;
	}
	return {
		chunkDefinitions,
...
function addStaticDependenciesToManualChunk(entry, manualChunkModules, modulesInManualChunks) {
	...
	for (const module of modulesToHandle) {
		...
		for (const dependency of module.dependencies) if (!(dependency instanceof ExternalModule || modulesInManualChunks.has(dependency))) modulesToHandle.add(dependency);
	}
}
	...
	for (const currentEntryModules of allEntriesAndManualChunks) {
		...
		for (const module of staticDependencies) {
			...
			if (manualChunkMembers && !addedManualChunks.has(manualChunkMembers)) {
				...
				for (const manualChunkMember of manualChunkMembers) staticDependencies.add(manualChunkMember);
			}
			for (const dependency of module.getDependenciesToBeIncluded()) if (!(dependency instanceof ExternalModule)) staticDependencies.add(dependency);
			for (const { node: { resolution } } of module.dynamicImports) if (resolution instanceof Module && resolution.includedDynamicImporters.length > 0 && !allEntriesSet.has(resolution)) {
				dynamicEntryModules.add(resolution);
				allEntriesSet.add(resolution);
				allEntriesAndManualChunks.push([resolution]);
				dynamicImportsForCurrentEntry.add(resolution);
				for (const includedTopLevelAwaitingDynamicImporter of resolution.includedTopLevelAwaitingDynamicImporters) if (staticDependencies.has(includedTopLevelAwaitingDynamicImporter)) {
					awaitedDynamicEntryModules.add(resolution);
					awaitedDynamicImportsForCurrentEntry.add(resolution);
					break;
				}
			}
			for (const dependency of module.implicitlyLoadedBefore) if (!allEntriesSet.has(dependency)) {
...
function getDynamicEntries(allEntriesAndManualChunks, dynamicEntryModules, dynamicImportModulesByEntry, awaitedDynamicEntryModules, awaitedDynamicImportModulesByEntry) {
	...
	for (const [entryIndex, entryModules] of allEntriesAndManualChunks.entries()) for (const entryModule of entryModules) {
		entryIndexByModule.set(entryModule, entryIndex);
		if (dynamicEntryModules.has(entryModule)) dynamicEntries.add(entryIndex);
		if (awaitedDynamicEntryModules.has(entryModule)) awaitedDynamicEntries.add(entryIndex);
	}
...
function getDynamicImportsByEntry(dynamicImportModulesByEntry, entryIndexByModule) {
	...
	for (const dynamicImportModules of dynamicImportModulesByEntry) {
		...
		for (const dynamicEntry of dynamicImportModules) dynamicImports.add(entryIndexByModule.get(dynamicEntry));
...
function getDynamicallyDependentEntriesByDynamicEntry(dependentEntriesByModule, dynamicEntries, allEntriesAndManualChunks, getDynamicImporters) {
	...
	for (const dynamicEntryIndex of dynamicEntries) {
		...
		for (const dynamicEntryModule of dynamicEntryModules) for (const importer of concatLazy([getDynamicImporters(dynamicEntryModule), dynamicEntryModule.implicitlyLoadedAfter])) {
			const importerEntries = dependentEntriesByModule.get(importer);
			if (!importerEntries) continue;
			for (const entry of importerEntries) dynamicallyDependentEntries.add(entry);
		}
	}
...
function getChunksWithSameDependentEntries(moduleWithDependentEntries) {
	...
	for (const { dependentEntries, module } of moduleWithDependentEntries) {
		...
		for (const entryIndex of dependentEntries) chunkSignature |= 1n << BigInt(entryIndex);
...
function* getModulesWithDependentEntriesAndHandleTLACycles(dependentEntriesByModule, modulesInManualChunks, chunkDefinitions) {
	for (const [module, dependentEntries] of dependentEntriesByModule) if (!modulesInManualChunks.has(module)) {
...
function getStaticDependencyAtomsByEntry(entriesAndManualChunksCount, chunkAtoms) {
	...
	for (const { dependentEntries } of chunkAtoms) {
		for (const entryIndex of dependentEntries) staticDependencyAtomsByEntry[entryIndex] |= atomMask;
		atomMask <<= 1n;
	}
...
function getAlreadyLoadedAtomsByEntry(staticDependencyAtomsByEntry, dynamicallyDependentEntriesByDynamicEntry, dynamicImportsByEntry, allEntriesCount) {
	...
	for (const [dynamicEntryIndex, dynamicallyDependentEntries] of dynamicallyDependentEntriesByDynamicEntry) {
		...
		for (const entryIndex of dynamicallyDependentEntries) updatedLoadedAtoms &= staticDependencyAtomsByEntry[entryIndex] | alreadyLoadedAtomsByEntry[entryIndex];
		if (updatedLoadedAtoms !== knownLoadedAtoms) {
			alreadyLoadedAtomsByEntry[dynamicEntryIndex] = updatedLoadedAtoms;
			for (const dynamicImport of dynamicImportsByEntry[dynamicEntryIndex]) getOrCreate(dynamicallyDependentEntriesByDynamicEntry, dynamicImport, getNewSet).add(dynamicEntryIndex);
		}
	}
...
function removeUnnecessaryDependentEntries(chunkAtoms, alreadyLoadedAtomsByEntry, awaitedAlreadyLoadedAtomsByEntry) {
	let chunkMask = 1n;
	for (const { dependentEntries } of chunkAtoms) {
		for (const entryIndex of dependentEntries) if ((alreadyLoadedAtomsByEntry[entryIndex] & chunkMask) === chunkMask && (awaitedAlreadyLoadedAtomsByEntry[entryIndex] & chunkMask) === 0n) dependentEntries.delete(entryIndex);
		chunkMask <<= 1n;
	}
}
function getChunksWithSameDependentEntriesAndCorrelatedAtoms(chunkAtoms, staticDependencyAtomsByEntry, alreadyLoadedAtomsByEntry, minChunkSize) {
	...
	for (const { dependentEntries, modules } of chunkAtoms) {
		...
		if (!pure) sideEffectAtoms |= atomMask;
...
function addChunkDependenciesAndGetExternalSideEffectAtoms(chunks, chunkByModule, nextAvailableAtomMask) {
	...
	for (const chunk of chunks) {
		...
		for (const module of modules) for (const dependency of module.getDependenciesToBeIncluded()) if (dependency instanceof ExternalModule) {
			if (dependency.info.moduleSideEffects) {
				const signature = getOrCreate(signatureByExternalModule, dependency, () => {
					const signature$1 = nextAvailableAtomMask;
					nextAvailableAtomMask <<= 1n;
					externalSideEffectAtoms |= signature$1;
					return signature$1;
				});
...
function getOptimizedChunks(chunks, minChunkSize, sideEffectAtoms, sizeByAtom, log) {
	...
	if (minChunkSize > 1) log("info", logOptimizeChunkStatus(chunks.length, chunkPartition.small.size, "Initially"));
	mergeChunks(chunkPartition, minChunkSize, sideEffectAtoms, sizeByAtom);
	if (minChunkSize > 1) log("info", logOptimizeChunkStatus(chunkPartition.small.size + chunkPartition.big.size, chunkPartition.small.size, "After merging chunks"));
	timeEnd("optimize chunks", 3);
...
function getPartitionedChunks(chunks, minChunkSize) {
	const smallChunks = [];
	const bigChunks = [];
	for (const chunk of chunks) (chunk.size < minChunkSize ? smallChunks : bigChunks).push(chunk);
	if (smallChunks.length === 0) return null;
...
function mergeChunks(chunkPartition, minChunkSize, sideEffectAtoms, sizeByAtom) {
	...
	for (const mergedChunk of small) {
		...
		if (bestTargetChunk) {
			...
			for (const entry of mergedChunk.dependentEntries) dependentEntries.add(entry);
...
function getAdditionalSizeIfNoTransitiveDependencyOrNonCorrelatedSideEffect(dependentChunk, dependencyChunk, currentAdditionalSize, sideEffectAtoms, sizeByAtom) {
	...
	if ((correlatedAtoms & dependentContainedSideEffects) !== dependentContainedSideEffects) return Infinity;
	...
	for (const { dependencies, containedAtoms } of chunksToCheck) {
		...
		if ((correlatedAtoms & containedSideEffects) !== containedSideEffects) return Infinity;
		for (const dependency of dependencies) {
			if (dependency === dependencyChunk) return Infinity;
...
function getAtomsSizeIfBelowLimit(atoms, currentAdditionalSize, sizeByAtom) {
	...
	for (; atomIndex < length; atomIndex++) {
		if ((atoms & atomSignature) === atomSignature) size += sizeByAtom[atomIndex];
		atomSignature <<= 1n;
		if (size >= currentAdditionalSize) return Infinity;
	}
	return size;
}
...
function commondir(files) {
	...
	if (files.length === 1) return dirname$1(files[0]);
	const commonSegments = files.slice(1).reduce((commonSegments$1, file) => {
		const pathSegments = file.split(/\/+|\\+/);
		let index;
		for (index = 0; commonSegments$1[index] === pathSegments[index] && index < Math.min(commonSegments$1.length, pathSegments.length); index++);
		return commonSegments$1.slice(0, index);
	}, files[0].split(/\/+|\\+/));
...
function analyseModuleExecution(entryModules) {
	...
	const handleSyncLoadedModule = (module, parent) => {
		if (parents.has(module)) {
			if (!analysedModules.has(module)) cyclePaths.push(getCyclePath(module, parent, parents));
			return;
		}
	...
	const analyseModule = (module) => {
		if (module instanceof Module) {
			for (const dependency of module.dependencies) handleSyncLoadedModule(dependency, module);
			for (const dependency of module.implicitlyLoadedBefore) dynamicImports.add(dependency);
			for (const { node: { resolution, scope } } of module.dynamicImports) if (resolution instanceof Module) if (scope.context.usesTopLevelAwait) handleSyncLoadedModule(resolution, module);
			else dynamicImports.add(resolution);
			orderedModules.push(module);
		}
		module.execIndex = nextExecIndex++;
		analysedModules.add(module);
	};
	for (const currentEntry of entryModules) if (!parents.has(currentEntry)) {
		parents.set(currentEntry, null);
		analyseModule(currentEntry);
	}
	for (const currentEntry of dynamicImports) if (!parents.has(currentEntry)) {
		parents.set(currentEntry, null);
		analyseModule(currentEntry);
	}
	return {
...
function getGenerateCodeSnippets({ compact, generatedCode: { arrowFunctions, constBindings, objectShorthand, reservedNamesAsProps } }) {
	...
	const getDirectReturnFunction = (parameters, { functionReturn, lineBreakIndent, name }) => [`${getFunctionIntro(parameters, {
		isAsync: false,
		name
	})}${arrowFunctions ? lineBreakIndent ? `${n$1}${lineBreakIndent.base}${lineBreakIndent.t}` : "" : `{${lineBreakIndent ? `${n$1}${lineBreakIndent.base}${lineBreakIndent.t}` : _}${functionReturn ? "return " : ""}`}`, arrowFunctions ? `${name ? ";" : ""}${lineBreakIndent ? `${n$1}${lineBreakIndent.base}` : ""}` : `${s}${lineBreakIndent ? `${n$1}${lineBreakIndent.base}` : _}}`];
	...
	return {
		...
		getObject(fields, { lineBreakIndent }) {
			const prefix = lineBreakIndent ? `${n$1}${lineBreakIndent.base}${lineBreakIndent.t}` : _;
			...
			}).join(`,`)}${fields.length === 0 ? "" : lineBreakIndent ? `${n$1}${lineBreakIndent.base}` : _}}`;
		},
...
var Link = class {
	...
	traceMappings() {
		...
		for (const line of this.mappings) {
			...
			for (const segment of line) {
				...
				if (traced) {
					const { column, line: line$1, name, source: { content, filename } } = traced;
					...
					} else if (sourcesContent[sourceIndex] == null) sourcesContent[sourceIndex] = content;
					else if (content != null && sourcesContent[sourceIndex] !== content) return error(logConflictingSourcemapSources(filename));
					const tracedSegment = [
						segment[0],
						sourceIndex,
						line$1,
	...
	traceSegment(line, column, name) {
		...
		while (searchStart <= searchEnd) {
			...
			if (segment[0] > column) searchEnd = m - 1;
			else searchStart = m + 1;
		}
		return null;
	}
};
function getLinkMap(log) {
	return function linkMap(source, map) {
		if (!map.missing) return new Link(map, [source]);
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
	...
	if (originalSourcemap) {
		...
		const baseSources = sources.map((source$1, index) => new Source(resolve$1(directory, sourceRoot, source$1), sourcesContent[index]));
		source = new Link(originalSourcemap, baseSources);
	} else source = new Source(id, originalCode);
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, log) {
	...
	if (file) {
		...
		sources = sources.map((source$1) => relative$2(directory, source$1));
		file = basename$1(file);
	}
	for (const module of modules) resetSourcemapCache(module.originalSourcemap, module.sourcemapChain);
...
function collapseSourcemap(id, originalCode, originalSourcemap, sourcemapChain, log) {
	if (sourcemapChain.length === 0) return originalSourcemap;
...
let textEncoder;
const getHash64 = (input) => (0, import_native.xxhashBase64Url)(ensureBuffer(input));
const getHash36 = (input) => (0, import_native.xxhashBase36)(ensureBuffer(input));
const getHash16 = (input) => (0, import_native.xxhashBase16)(ensureBuffer(input));
const hasherByType = {
	base36: getHash36,
	base64: getHash64,
	hex: getHash16
...
function reserveEntryChunksInBundle(chunks) {
	for (const chunk of chunks) if (chunk.facadeModule && chunk.facadeModule.isUserDefinedEntryPoint) chunk.getPreliminaryFileName();
}
...
async function transformChunk(magicString, fileName, usedModules, chunkGraph, options, outputPluginDriver, log) {
	...
	let code = await outputPluginDriver.hookReduceArg0("renderChunk", [
		magicString.toString(),
		chunkGraph[fileName],
		options,
		{ chunks: chunkGraph }
	], (code$1, result, plugin) => {
		if (result == null) return code$1;
		if (typeof result === "string") result = {
			code: result,
			map: void 0
		};
		if (result.map !== null) {
			const map$1 = decodedSourcemap(result.map);
			sourcemapChain.push(map$1 || {
				missing: true,
				plugin: plugin.name
			});
		}
	...
	if (sourcemap) {
		...
		let resultingFile;
		if (file) resultingFile = resolve$1(sourcemapFile || file);
		else if (dir) resultingFile = resolve$1(dir, fileName);
		else resultingFile = resolve$1(fileName);
		...
		for (let sourcesIndex = 0; sourcesIndex < map.sources.length; ++sourcesIndex) {
			...
			if (typeof ignoreList !== "boolean") error(logFailedValidation("sourcemapIgnoreList function must return a boolean."));
			if (ignoreList) {
				if (map.x_google_ignoreList === void 0) map.x_google_ignoreList = [];
				if (!map.x_google_ignoreList.includes(sourcesIndex)) map.x_google_ignoreList.push(sourcesIndex);
			}
			if (sourcemapPathTransform) {
				sourcePath = sourcemapPathTransform(sourcePath, sourcemapPath);
				if (typeof sourcePath !== "string") error(logFailedValidation(`sourcemapPathTransform function must return a string.`));
			}
...
async function transformChunksAndGenerateContentHashes(renderedChunks, chunkGraph, outputOptions, pluginDriver, getHash, log) {
	...
	for (const { preliminaryFileName: { hashPlaceholder } } of renderedChunks) if (hashPlaceholder) placeholders.add(hashPlaceholder);
	await Promise.all(renderedChunks.map(async ({ chunk, preliminaryFileName: { fileName, hashPlaceholder }, preliminarySourcemapFileName, magicString, usedModules }) => {
		...
		if (hashPlaceholder) {
			...
			const hashAugmentation = pluginDriver.hookReduceValueSync("augmentChunkHash", "", [chunk.getRenderedChunkInfo()], (augmentation, pluginHash) => {
				if (pluginHash) augmentation += pluginHash;
				return augmentation;
			});
			if (hashAugmentation) contentToHash += hashAugmentation;
		...
		} else nonHashedChunksWithPlaceholders.push(transformedChunk);
		const sourcemapHashPlaceholder = preliminarySourcemapFileName?.hashPlaceholder;
		if (map && sourcemapHashPlaceholder) initialHashesByPlaceholder.set(preliminarySourcemapFileName.hashPlaceholder, getHash(map.toString()).slice(0, preliminarySourcemapFileName.hashPlaceholder.length));
	}));
	return {
...
function generateFinalHashes(renderedChunksByPlaceholder, hashDependenciesByPlaceholder, initialHashesByPlaceholder, placeholders, bundle, getHash) {
	...
	for (const placeholder of placeholders) {
		...
		for (const dependencyPlaceholder of hashDependencyPlaceholders) {
			...
			for (const containedPlaceholder of containedPlaceholders) hashDependencyPlaceholders.add(containedPlaceholder);
		}
		let finalFileName;
		let finalHash;
		do {
			if (finalHash) contentToHash = finalHash;
...
function addChunksToBundle(renderedChunksByPlaceholder, hashesByPlaceholder, bundle, nonHashedChunksWithPlaceholders, pluginDriver, options) {
	for (const { chunk, code, fileName, sourcemapFileName, map } of renderedChunksByPlaceholder.values()) {
		...
		if (map) {
			if (options.sourcemapDebugIds) updatedCode += calculateDebugIdAndGetComment(updatedCode, map);
...
function emitSourceMapAndGetComment(fileName, map, pluginDriver, { sourcemap, sourcemapBaseUrl }) {
	let url;
	if (sourcemap === "inline") url = map.toUrl();
	else {
...
var Bundle = class {
	...
	async generate(isWrite) {
		...
		try {
			...
			if (chunks.length > 1) validateOptionsForMultiChunkOutput(this.outputOptions, this.inputOptions.onLog);
	...
	async addManualChunks(manualChunks) {
		...
		for (const { alias, entries } of chunkEntries) for (const entry of entries) addModuleToManualChunk(alias, entry, manualChunkAliasByEntry);
	...
	assignManualChunks(getManualChunk) {
		...
		for (const module of this.graph.modulesById.values()) if (module instanceof Module) {
			const manualChunkAlias = getManualChunk(module.id, manualChunksApi);
			if (typeof manualChunkAlias === "string") addModuleToManualChunk(manualChunkAlias, module, manualChunkAliasByEntry);
		}
		return manualChunkAliasByEntry;
	}
	finaliseAssets(bundle) {
		if (this.outputOptions.validate) {
			for (const file of Object.values(bundle)) if ("code" in file) try {
	...
	async generateChunks(bundle, getHashPlaceholder) {
		...
		if (!inlineDynamicImports && !preserveModules) this.checkCircularChunks(chunks);
		const facades = [];
	...
	checkCircularChunks(chunks) {
		...
		const analyseChunk = (chunk) => {
			for (const dependency of chunk.dependencies) if (dependency instanceof Chunk) handleDependency(dependency, chunk);
			visited.add(chunk);
		};
		for (const chunk of chunks) if (!parents.has(chunk)) analyseChunk(chunk);
	}
};
function validateOptionsForMultiChunkOutput(outputOptions, log) {
	if (outputOptions.format === "umd" || outputOptions.format === "iife") return error(logInvalidOption("output.format", URL_OUTPUT_FORMAT, "UMD and IIFE output formats are not supported for code-splitting builds", outputOptions.format));
	if (typeof outputOptions.file === "string") return error(logInvalidOption("output.file", URL_OUTPUT_DIR, "when building multiple chunks, the \"output.dir\" option must be used, not \"output.file\". To inline dynamic imports, set the \"inlineDynamicImports\" option"));
	if (outputOptions.sourcemapFile) return error(logInvalidOption("output.sourcemapFile", URL_OUTPUT_SOURCEMAPFILE, "\"output.sourcemapFile\" is only supported for single-file builds"));
	if (!outputOptions.amd.autoId && outputOptions.amd.id) log(LOGLEVEL_WARN, logInvalidOption("output.amd.id", URL_OUTPUT_AMD_ID, "this option is only properly supported for single-file builds. Use \"output.amd.autoId\" and \"output.amd.basePath\" instead"));
}
function getIncludedModules(modulesById) {
	const includedModules = [];
	for (const module of modulesById.values()) if (module instanceof Module && (module.isIncluded() || module.info.isEntry || module.includedDynamicImporters.length > 0)) includedModules.push(module);
	return includedModules;
}
function getAbsoluteEntryModulePaths(includedModules, preserveModules) {
	const absoluteEntryModulePaths = [];
	for (const module of includedModules) if ((module.info.isEntry || preserveModules) && isAbsolute(module.id)) absoluteEntryModulePaths.push(module.id);
	return absoluteEntryModulePaths;
}
function getExternalChunkByModule(modulesById, outputOptions, inputBase) {
	const externalChunkByModule = new Map();
	for (const module of modulesById.values()) if (module instanceof ExternalModule) externalChunkByModule.set(module, new ExternalChunk(module, outputOptions, inputBase));
	return externalChunkByModule;
}
function addModuleToManualChunk(alias, module, manualChunkAliasByEntry) {
	const existingAlias = manualChunkAliasByEntry.get(module);
	if (typeof existingAlias === "string" && existingAlias !== alias) return error(logCannotAssignModuleToChunk(module.id, alias, existingAlias));
	manualChunkAliasByEntry.set(module, alias);
}
function isFileNameOutsideOutputDirectory(fileName) {
	const normalized = join$1(fileName).replaceAll("\\", "/");
...
function validateOutputBundleFileNames(bundle) {
	for (const [bundleKey, entry] of Object.entries(bundle)) {
		if (isFileNameOutsideOutputDirectory(bundleKey)) return error(logFileNameOutsideOutputDirectory(bundleKey));
		if (entry.type !== "placeholder") {
			const { fileName } = entry;
			if (fileName !== bundleKey && isFileNameOutsideOutputDirectory(fileName)) return error(logFileNameOutsideOutputDirectory(fileName));
		}
	}
}
function flru(max) {
	...
	return {
		...
		set: function(key, value) {
			if (curr[key] !== void 0) curr[key] = value;
			else keep(key, value);
		}
	};
}
...
function resolveIdViaPlugins(source, importer, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes) {
	...
	if (skip) {
		skipped = new Set();
		for (const skippedCall of skip) if (source === skippedCall.source && importer === skippedCall.importer) skipped.add(skippedCall.plugin);
		replaceContext = (pluginContext, plugin) => ({
			...pluginContext,
			resolve: (source$1, importer$1, { attributes: attributes$1, custom, isEntry: isEntry$1, skipSelf, importerAttributes: importerAttributes$1 } = BLANK) => {
				skipSelf ??= true;
				if (skipSelf && skip.findIndex((skippedCall) => {
					return skippedCall.plugin === plugin && skippedCall.source === source$1 && skippedCall.importer === importer$1;
				}) !== -1) return Promise.resolve(null);
				return moduleLoaderResolveId(source$1, importer$1, custom, isEntry$1, attributes$1 || EMPTY_OBJECT, importerAttributes$1, skipSelf ? [...skip, {
					importer: importer$1,
					plugin,
					source: source$1
				}] : skip);
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes, fs$2) {
	...
	if (pluginResult != null) {
		...
		if (typeof resolveIdResult === "object" && !resolveIdResult.resolvedBy) return {
			...resolveIdResult,
			resolvedBy: plugin.name
		};
		if (typeof resolveIdResult === "string") return {
			id: resolveIdResult,
			resolvedBy: plugin.name
		};
	...
	return addJsExtensionIfNecessary(importer ? resolve$1(dirname$1(importer), source) : resolve$1(source), preserveSymlinks, fs$2);
}
async function addJsExtensionIfNecessary(file, preserveSymlinks, fs$2) {
	return await findFile(file, preserveSymlinks, fs$2) ?? await findFile(file + ".mjs", preserveSymlinks, fs$2) ?? await findFile(file + ".js", preserveSymlinks, fs$2);
}
async function findFile(file, preserveSymlinks, fs$2) {
	try {
		const stats = await fs$2.lstat(file);
		if (!preserveSymlinks && stats.isSymbolicLink()) return await findFile(await fs$2.realpath(file), preserveSymlinks, fs$2);
		if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
			const name = basename$1(file);
			const files = await fs$2.readdir(dirname$1(file));
			if (files.includes(name)) return file;
		}
	} catch {}
}
function stripBom(content) {
	if (content.charCodeAt(0) === 65279) return stripBom(content.slice(1));
	return content;
}
...
const getOnLog = (config, logLevel, printLog = defaultPrintLog) => {
	...
	if (onLog) {
		...
		return (level, log) => onLog(level, addLogToString(log), (level$1, handledLog) => {
			if (level$1 === LOGLEVEL_ERROR) return error(normalizeLog(handledLog));
...
const getDefaultOnLog = (printLog, onwarn) => onwarn ? (level, log) => {
	if (level === LOGLEVEL_WARN) onwarn(addLogToString(log), (warning) => printLog(LOGLEVEL_WARN, normalizeLog(warning)));
...
const defaultPrintLog = (level, { message }) => {
	switch (level) {
		case LOGLEVEL_WARN: return console.warn(message);
		case LOGLEVEL_DEBUG: return console.debug(message);
...
function warnUnknownOptions(passedOptions, validOptions, optionType, log, ignoredKeys = /$./) {
	...
	if (unknownOptions.length > 0) log(LOGLEVEL_WARN, logUnknownOption(optionType, unknownOptions, [...validOptionSet].sort()));
}
...
const getOptionWithPreset = (value, presets, optionName, urlSnippet, additionalValues) => {
	...
	if (presetName) {
		...
		else error(logInvalidOption(`${optionName}.preset`, urlSnippet, `valid values are ${printQuotedStringList(Object.keys(presets))}`, presetName));
	}
...
function uncacheablePluginError(pluginName) {
	if (pluginName.startsWith(ANONYMOUS_PLUGIN_PREFIX) || pluginName.startsWith(ANONYMOUS_OUTPUT_PLUGIN_PREFIX)) return error(logAnonymousPluginCache());
...
async function transform(source, module, pluginDriver, options) {
	...
	function transformReducer(previousCode, result, plugin) {
		let code$1;
		let map;
		if (typeof result === "string") code$1 = result;
		else if (result && typeof result === "object") {
			module.updateOptions(result);
			if (result.code == null) {
				if (result.map || result.ast) options.onLog(LOGLEVEL_WARN, logNoTransformMapOrAstWithoutCode(plugin.name));
				return previousCode;
			}
			if (result.attributes) warnDeprecation("Returning attributes from the \"transform\" hook is forbidden.", URL_TRANSFORM, false, options);
			({code: code$1, map, ast} = result);
		} else return previousCode;
		if (map !== null) sourcemapChain.push(decodedSourcemap(typeof map === "string" ? JSON.parse(map) : map) || {
			missing: true,
			plugin: plugin.name
		});
		currentSource = code$1;
		return code$1;
	}
	...
	try {
		code = await pluginDriver.hookReduceArg0("transform", [
			...
			return {
				...
				info: getLogHandler$1(pluginContext.info),
				setAssetSource() {
					return this.error(logInvalidSetAssetSourceCall());
				},
				warn: getLogHandler$1(pluginContext.warn)
			};
		});
	} catch (error_) {
		return error(logPluginError(error_, pluginName, {
			hook: "transform",
			id
		}));
	}
	if (!customTransformCache && emittedFiles.length > 0) module.transformFiles = emittedFiles;
	return {
		ast,
		code,
...
var ModuleLoader = class {
	...
	async addEntryModules(unresolvedEntryModules, isUserDefined) {
		const newEntryModules = await this.extendLoadModulesPromise(Promise.all(unresolvedEntryModules.map(({ id, importer }) => this.loadEntryModule(id, true, importer, null, void 0, void 0))).then((entryModules) => {
			...
			if (shouldReorder) this.sortedEntryModules.sort((a, b) => a.id > b.id ? 1 : -1);
			return entryModules;
		}));
		await this.awaitLoadModulesPromise();
		return {
	...
	async emitChunk({ fileName, id, importer, name, implicitlyLoadedAfterOneOf, preserveSignature }) {
		...
		if (preserveSignature != null) module.preserveSignature = preserveSignature;
		return module;
	}
		...
		return this.extendLoadModulesPromise(this.loadEntryModule(unresolvedModule.id, false, unresolvedModule.importer, null, void 0, void 0).then(async (entryModule) => {
			...
			if (!entryModule.info.isEntry) {
				...
				if (!entryModule.info.isEntry) {
					...
					for (const module of implicitlyLoadedAfterModules) entryModule.implicitlyLoadedAfter.add(module);
					for (const dependent of entryModule.implicitlyLoadedAfter) dependent.implicitlyLoadedBefore.add(entryModule);
				}
			}
			return entryModule;
		}));
	...
	async addModuleSource(id, importer, module) {
		...
		try {
			source = await this.graph.fileOperationQueue.run(async () => {
				const content = await this.pluginDriver.hookFirst("load", [id, { attributes: module.info.attributes }]);
				if (content !== null) {
					if (typeof content === "object" && content.attributes) warnDeprecation("Returning attributes from the \"load\" hook is forbidden.", URL_LOAD, false, this.options);
					return content;
				}
		...
		if (cachedModule && !cachedModule.customTransformCache && cachedModule.originalCode === sourceDescription.code && !await this.pluginDriver.hookFirst("shouldTransformCachedModule", [{
			...
			if (cachedModule.transformFiles) for (const emittedFile of cachedModule.transformFiles) this.pluginDriver.emitFile(emittedFile);
	...
	async fetchDynamicDependencies(module, resolveDynamicImportPromises) {
		const dependencies = await Promise.all(resolveDynamicImportPromises.map((resolveDynamicImportPromise) => resolveDynamicImportPromise.then(async ([{ argument, node }, resolvedId]) => {
			...
			if (node.phase === "source" && !resolvedId.external) return error(logNonExternalSourcePhaseImport(typeof argument === "string" ? argument : relativeId(resolvedId.id), module.id));
			return node.resolution = await this.fetchResolvedDependency(relativeId(resolvedId.id), module.id, resolvedId);
		})));
		for (const dependency of dependencies) if (dependency) {
	...
	async fetchModule({ attributes, id, meta, moduleSideEffects, syntheticNamedExports }, importer, isEntry, isPreload) {
		...
		if (existingModule instanceof Module) {
			if (importer && doAttributesDiffer(attributes, existingModule.info.attributes)) this.options.onLog(LOGLEVEL_WARN, logInconsistentImportAttributes(existingModule.info.attributes, attributes, id, importer));
			await this.handleExistingModule(existingModule, isEntry, isPreload);
			return existingModule;
		}
		if (existingModule instanceof ExternalModule) return error(logExternalModulesCannotBeTransformedToModules(existingModule.id));
		...
		if (!isPreload) await this.fetchModuleDependencies(module, ...resolveDependencyPromises);
		else if (isPreload === RESOLVE_DEPENDENCIES) await loadAndResolveDependenciesPromise;
		return module;
	}
	async fetchModuleDependencies(module, resolveStaticDependencyPromises, resolveDynamicDependencyPromises, loadAndResolveDependenciesPromise) {
		if (this.modulesWithLoadedDependencies.has(module)) return;
	...
	fetchResolvedDependency(source, importer, resolvedId) {
		if (resolvedId.external) {
			...
			} else if (!(externalModule instanceof ExternalModule)) return error(logInternalIdCannotBeExternal(source, importer));
			else if (doAttributesDiffer(externalModule.info.attributes, attributes)) this.options.onLog(LOGLEVEL_WARN, logInconsistentImportAttributes(externalModule.info.attributes, attributes, source, importer));
	...
	async fetchStaticDependencies(module, resolveStaticDependencyPromises) {
		for (const dependency of await Promise.all(resolveStaticDependencyPromises.map((resolveStaticDependencyPromise) => resolveStaticDependencyPromise.then(([source, resolvedId]) => {
			if (module.sourcePhaseSources.has(source) && !resolvedId.external) return error(logNonExternalSourcePhaseImport(source, module.id));
		...
		if (!this.options.treeshake || module.info.moduleSideEffects === "no-treeshake") {
			for (const dependency of module.dependencies) if (dependency instanceof Module) dependency.importedFromNotTreeshaken = true;
		}
	}
	getNormalizedResolvedIdWithoutDefaults(resolveIdResult, importer, source) {
		...
		if (resolveIdResult) {
			if (typeof resolveIdResult === "object") {
				const external$1 = resolveIdResult.external || this.options.external(resolveIdResult.id, importer, true);
				return {
					...resolveIdResult,
					external: external$1 && (external$1 === "relative" || !isAbsolute(resolveIdResult.id) || external$1 === true && isNotAbsoluteExternal(resolveIdResult.id, source, makeAbsoluteExternalsRelative) || "absolute")
				};
			}
		...
		if (resolveIdResult !== false && !this.options.external(id, importer, true)) return null;
		return {
	...
	getResolveDynamicImportPromises(module) {
		return module.dynamicImports.map(async (dynamicImport) => {
			...
			if (!resolvedId || typeof resolvedId === "string") dynamicImport.node.shouldIncludeDynamicAttributes = true;
			else {
	...
	getResolveStaticDependencyPromises(module) {
		return Array.from(module.sourcesWithAttributes, async ([source, attributes]) => [source, module.resolvedIds[source] = module.resolvedIds[source] || this.handleInvalidResolvedId(await this.resolveId(source, module.id, EMPTY_OBJECT, false, attributes, module.info.attributes), source, module.id, attributes)]);
	}
	...
	async handleExistingModule(module, isEntry, isPreload) {
		...
		if (isPreload) return isPreload === RESOLVE_DEPENDENCIES ? waitForDependencyResolution(loadPromise) : loadPromise;
		if (isEntry) {
			module.info.isEntry = true;
			this.implicitEntryModules.delete(module);
			for (const dependent of module.implicitlyLoadedAfter) dependent.implicitlyLoadedBefore.delete(module);
	...
	handleInvalidResolvedId(resolvedId, source, importer, attributes) {
		if (resolvedId === null) {
			if (isRelative(source)) return error(logUnresolvedImport(source, importer));
		...
		} else if (resolvedId.external && resolvedId.syntheticNamedExports) this.options.onLog(LOGLEVEL_WARN, logExternalSyntheticExports(source, importer));
		return resolvedId;
	}
	async loadEntryModule(unresolvedId, isEntry, importer, implicitlyLoadedBefore, isLoadForManualChunks = false, importerAttributes) {
		...
		if (resolveIdResult == null) return error(implicitlyLoadedBefore === null ? logUnresolvedEntry(unresolvedId) : logUnresolvedImplicitDependant(unresolvedId, implicitlyLoadedBefore));
		const isExternalModules = typeof resolveIdResult === "object" && resolveIdResult.external;
		if (resolveIdResult === false || isExternalModules) return error(implicitlyLoadedBefore === null ? isExternalModules && isLoadForManualChunks ? logExternalModulesCannotBeIncludedInManualChunks(unresolvedId) : logEntryCannotBeExternal(unresolvedId) : logImplicitDependantCannotBeExternal(unresolvedId, implicitlyLoadedBefore));
	...
	async resolveDynamicImport(module, specifier, importer, attributes) {
		const resolution = await this.pluginDriver.hookFirst("resolveDynamicImport", [
			...
			{
				attributes,
				importerAttributes: module.info.attributes
			}
		]);
		...
		if (resolution == null) {
			...
			if (existingResolution) {
				if (doAttributesDiffer(existingResolution.attributes, attributes)) this.options.onLog(LOGLEVEL_WARN, logInconsistentImportAttributes(existingResolution.attributes, attributes, specifier, importer));
...
function addChunkNamesToModule(module, { fileName, name }, isUserDefined) {
	if (fileName !== null) module.chunkFileNames.add(fileName);
	else if (name !== null) {
		module.chunkNames.push({
			isUserDefined,
			name
		});
...
function reserveFileNameInBundle(fileName, { bundle }, log) {
	if (bundle[lowercaseBundleKeys].has(fileName.toLowerCase())) log(LOGLEVEL_WARN, logFileNameConflict(fileName));
	else bundle[fileName] = FILE_PLACEHOLDER;
}
const emittedFileTypes = new Set([
	"chunk",
	"asset",
	"prebuilt-chunk"
]);
...
function getAssetFileName(file, referenceId) {
	if (typeof file.fileName !== "string") return error(logAssetNotFinalisedForFileName(file.name || referenceId));
	return file.fileName;
}
function getChunkFileName(file, facadeChunkByModule) {
	if (file.fileName) return file.fileName;
	if (facadeChunkByModule) return facadeChunkByModule.get(file.module).getFileName();
...
var FileEmitter = class {
	constructor(graph, options, baseFileEmitter) {
		...
		this.emitFile = (emittedFile) => {
			if (!hasValidType(emittedFile)) return error(logFailedValidation(`Emitted files must be of type "asset", "chunk" or "prebuilt-chunk", received "${emittedFile && emittedFile.type}".`));
			if (emittedFile.type === "prebuilt-chunk") return this.emitPrebuiltChunk(emittedFile);
			if (!hasValidName(emittedFile)) return error(logFailedValidation(`The "fileName" or "name" properties of emitted chunks and assets must be strings that are neither absolute nor relative paths, received "${emittedFile.fileName || emittedFile.name}".`));
			if (emittedFile.type === "chunk") return this.emitChunk(emittedFile);
			return this.emitAsset(emittedFile);
		};
		this.finaliseAssets = () => {
			for (const [referenceId, emittedFile] of this.filesByReferenceId) if (emittedFile.type === "asset" && typeof emittedFile.fileName !== "string") return error(logNoAssetSourceSet(emittedFile.name || referenceId));
		};
		this.getFileName = (fileReferenceId) => {
			const emittedFile = this.filesByReferenceId.get(fileReferenceId);
			if (!emittedFile) return error(logFileReferenceIdNotFoundForFilename(fileReferenceId));
			if (emittedFile.type === "chunk") return getChunkFileName(emittedFile, this.facadeChunkByModule);
			if (emittedFile.type === "prebuilt-chunk") return emittedFile.fileName;
			return getAssetFileName(emittedFile, fileReferenceId);
		};
		this.setAssetSource = (referenceId, requestedSource) => {
			const consumedFile = this.filesByReferenceId.get(referenceId);
			if (!consumedFile) return error(logAssetReferenceIdNotFoundForSetSource(referenceId));
			if (consumedFile.type !== "asset") return error(logFailedValidation(`Asset sources can only be set for emitted assets but "${referenceId}" is an emitted chunk.`));
			if (consumedFile.source !== void 0) return error(logAssetSourceAlreadySet(consumedFile.name || referenceId));
			const source = getValidSource(requestedSource, consumedFile, referenceId);
			if (this.output) this.finalizeAdditionalAsset(consumedFile, source, this.output);
			else {
				consumedFile.source = source;
				for (const emitter of this.outputFileEmitters) emitter.finalizeAdditionalAsset(consumedFile, source, emitter.output);
			}
		};
		...
		this.setOutputBundle = (bundle, outputOptions) => {
			...
			for (const emittedFile of this.filesByReferenceId.values()) if (emittedFile.fileName) reserveFileNameInBundle(emittedFile.fileName, output, this.options.onLog);
			const consumedAssetsByHash = new Map();
			for (const consumedFile of this.filesByReferenceId.values()) if (consumedFile.type === "asset" && consumedFile.source !== void 0) if (consumedFile.fileName) this.finalizeAdditionalAsset(consumedFile, consumedFile.source, output);
			else {
				const sourceHash = getHash(consumedFile.source);
				getOrCreate(consumedAssetsByHash, sourceHash, getNewArray).push(consumedFile);
			}
			else if (consumedFile.type === "prebuilt-chunk") this.output.bundle[consumedFile.fileName] = this.createPrebuiltChunk(consumedFile);
			for (const [sourceHash, consumedFiles] of consumedAssetsByHash) this.finalizeAssetsWithSameSource(consumedFiles, sourceHash, output);
		};
	...
	assignReferenceId(file, idBase) {
		...
		for (const { filesByReferenceId } of this.outputFileEmitters) filesByReferenceId.set(referenceId, file);
		return referenceId;
	}
	...
	emitAsset(emittedAsset) {
		...
		if (typeof originalFileName === "string") this.graph.watchFiles[originalFileName] = true;
		const consumedAsset = {
		...
		if (this.output) this.emitAssetWithReferenceId(consumedAsset, this.output);
		else for (const fileEmitter of this.outputFileEmitters) fileEmitter.emitAssetWithReferenceId(consumedAsset, fileEmitter.output);
		return referenceId;
	}
	emitAssetWithReferenceId(consumedAsset, output) {
		const { fileName, source } = consumedAsset;
		if (fileName) reserveFileNameInBundle(fileName, output, this.options.onLog);
		if (source !== void 0) this.finalizeAdditionalAsset(consumedAsset, source, output);
	}
	emitChunk(emittedChunk) {
		if (this.graph.phase > BuildPhase.LOAD_AND_PARSE) return error(logInvalidRollupPhaseForChunkEmission());
		if (typeof emittedChunk.id !== "string") return error(logFailedValidation(`Emitted chunks need to have a valid string id, received "${emittedChunk.id}"`));
		const consumedChunk = {
	...
	emitPrebuiltChunk(emitPrebuiltChunk) {
		if (typeof emitPrebuiltChunk.code !== "string") return error(logFailedValidation(`Emitted prebuilt chunks need to have a valid string code, received "${emitPrebuiltChunk.code}".`));
		if (typeof emitPrebuiltChunk.fileName !== "string" || isPathFragment(emitPrebuiltChunk.fileName)) return error(logFailedValidation(`The "fileName" property of emitted prebuilt chunks must be strings that are neither absolute nor relative paths, received "${emitPrebuiltChunk.fileName}".`));
		...
		if (this.output) this.output.bundle[consumedPrebuiltChunk.fileName] = this.createPrebuiltChunk(consumedPrebuiltChunk);
		return referenceId;
	}
	finalizeAdditionalAsset(consumedFile, source, { bundle, fileNamesBySourceHash, getHash, outputOptions }) {
		...
		if (existingAsset?.type === "asset") {
			...
			if (name) existingAsset.names.push(name);
			if (originalFileName) existingAsset.originalFileNames.push(originalFileName);
		} else {
			const { options } = this;
			bundle[fileName] = {
				fileName,
...
function getNamesFromAssets(consumedFiles) {
	...
	for (const { name, originalFileName } of consumedFiles) {
		...
		if (originalFileName) originalFileNames.push(originalFileName);
	}
...
function getLogHandler(level, code, logger, pluginName, logLevel) {
	if (logLevelPriority[level] < logLevelPriority[logLevel]) return doNothing;
	return (log, pos) => {
		if (pos != null) logger(LOGLEVEL_WARN, logInvalidLogPosition(pluginName));
		log = normalizeLog(log);
...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
	...
	let cacheable = true;
	if (typeof plugin.cacheKey !== "string") if (plugin.name.startsWith(ANONYMOUS_PLUGIN_PREFIX) || plugin.name.startsWith(ANONYMOUS_OUTPUT_PLUGIN_PREFIX) || existingPluginNames.has(plugin.name)) cacheable = false;
	else existingPluginNames.add(plugin.name);
	let cacheInstance;
	if (!pluginCache) cacheInstance = NO_CACHE;
	else if (cacheable) {
		const cacheKey = plugin.cacheKey || plugin.name;
		cacheInstance = createPluginCache(pluginCache[cacheKey] || (pluginCache[cacheKey] = Object.create(null)));
	} else cacheInstance = getCacheForUncacheablePlugin(plugin.name);
	return {
		addWatchFile(id) {
		...
		resolve(source, importer, { attributes, custom, isEntry, skipSelf, importerAttributes } = BLANK) {
			skipSelf ??= true;
			return graph.moduleLoader.resolveId(source, importer, custom, isEntry, attributes || EMPTY_OBJECT, importerAttributes, skipSelf ? [{
				importer,
				plugin,
				source
			}] : null);
...
var PluginDriver = class PluginDriver {
	constructor(graph, options, userPlugins, pluginCache, basePluginDriver) {
		...
		if (basePluginDriver) {
			for (const plugin of userPlugins) for (const hook of inputHooks) if (hook in plugin) options.onLog(LOGLEVEL_WARN, logInputHookInOutputPlugin(plugin.name, hook));
		}
	}
	...
	async hookParallel(hookName, parameters, replaceContext) {
		...
		for (const plugin of this.getSortedPlugins(hookName)) if (plugin[hookName].sequential) {
			await Promise.all(parallelPromises);
			parallelPromises.length = 0;
			await this.runHook(hookName, parameters, plugin, replaceContext);
		} else parallelPromises.push(this.runHook(hookName, parameters, plugin, replaceContext));
		await Promise.all(parallelPromises);
	}
	hookReduceArg0(hookName, [argument0, ...rest], reduce, replaceContext) {
		let promise = Promise.resolve(argument0);
		for (const plugin of this.getSortedPlugins(hookName)) promise = promise.then((argument0$1) => this.runHook(hookName, [argument0$1, ...rest], plugin, replaceContext).then((result) => reduce.call(this.pluginContexts.get(plugin), argument0$1, result, plugin)));
		return promise;
	}
	...
	async hookReduceValue(hookName, initialValue, parameters, reducer) {
		...
		for (const plugin of this.getSortedPlugins(hookName, validateAddonPluginHandler)) if (plugin[hookName].sequential) {
			results.push(...await Promise.all(parallelResults));
			parallelResults.length = 0;
			results.push(await this.runHook(hookName, parameters, plugin));
		} else parallelResults.push(this.runHook(hookName, parameters, plugin));
		results.push(...await Promise.all(parallelResults));
	...
	hookSeq(hookName, parameters, replaceContext) {
		...
		for (const plugin of this.getSortedPlugins(hookName)) promise = promise.then(() => this.runHook(hookName, parameters, plugin, replaceContext));
	...
	runHook(hookName, parameters, plugin, replaceContext) {
		...
		if (typeof hook === "object" && "filter" in hook && hook.filter) {
			if (hookName === "transform") {
				...
				if (compiledFilter && !compiledFilter(hookParameters[1], hookParameters[0])) return Promise.resolve();
				...
				if (compiledFilter && !compiledFilter(hookParameters[0])) return Promise.resolve();
			}
		}
		let context = this.pluginContexts.get(plugin);
		if (replaceContext) context = replaceContext(context, plugin);
		let action = null;
		return Promise.resolve().then(() => {
			if (typeof handler !== "function") return handler;
			const hookResult = handler.apply(context, parameters);
			if (!hookResult?.then) return hookResult;
			action = [
				plugin.name,
				hookName,
				parameters
			...
			if (action !== null) this.unfulfilledActions.delete(action);
...
function getSortedValidatedPlugins(hookName, plugins, validateHandler = validateFunctionPluginHandler) {
	...
	for (const plugin of plugins) {
		...
		if (hook) {
			...
			} else validateHandler(hook, hookName, plugin);
			normal.push(plugin);
		}
	}
	return [
...
function validateAddonPluginHandler(handler, hookName, plugin) {
	if (typeof handler !== "string" && typeof handler !== "function") return error(logInvalidAddonPluginHook(hookName, plugin.name));
}
function noReturn() {}
var Queue = class {
	...
	async work() {
		...
		while (entry = this.queue.shift()) {
			const { reject, resolve: resolve$2, task } = entry;
			try {
...
function normalizeEntryModules(entryModules) {
	if (Array.isArray(entryModules)) return entryModules.map((id) => ({
		fileName: null,
		id,
...
var Graph = class {
	...
	getCache() {
		for (const name in this.pluginCache) {
			const cache = this.pluginCache[name];
			let allDeleted = true;
			for (const [key, value] of Object.entries(cache)) if (value[0] >= this.options.experimentalCacheExpiry) delete cache[key];
	...
	async generateModuleGraph() {
		...
		if (this.entryModules.length === 0) throw new Error("You must supply options.input to rollup");
		for (const module of this.modulesById.values()) {
			module.cacheInfoGetters();
			if (module instanceof Module) this.modules.push(module);
			else this.externalModules.push(module);
		}
	}
	includeStatements() {
		const entryModules = [...this.entryModules, ...this.implicitEntryModules];
		for (const module of entryModules) markModuleAndImpureDependenciesAsExecuted(module);
		if (this.options.treeshake) {
			...
			do {
				...
				this.needsTreeshakingPass = false;
				for (const module of this.modules) if (module.isExecuted) {
					module.hasTreeShakingPassStarted = true;
					if (module.info.moduleSideEffects === "no-treeshake") module.includeAllInBundle();
					else module.include();
				...
				if (treeshakingPass === 1) {
					for (const module of entryModules) if (module.preserveSignature !== false) {
		...
		} else for (const module of this.modules) module.includeAllInBundle();
		for (const externalModule of this.externalModules) externalModule.warnUnusedImports();
		for (const module of this.implicitEntryModules) for (const dependent of module.implicitlyLoadedAfter) if (!(dependent.info.isEntry || dependent.isIncluded())) error(logImplicitDependantIsNotIncluded(dependent));
	}
	sortAndBindModules() {
		const { orderedModules, cyclePaths } = analyseModuleExecution(this.entryModules);
		for (const cyclePath of cyclePaths) this.options.onLog(LOGLEVEL_WARN, logCircularDependency(cyclePath));
		this.modules = orderedModules;
		for (const module of this.modules) module.bindReferences();
		this.warnForMissingExports();
	}
	warnForMissingExports() {
		for (const module of this.modules) for (const importDescription of module.importDescriptions.values()) if (importDescription.name !== "*" && importDescription.phase !== "source") {
			const [variable, options] = importDescription.module.getVariableForExportName(importDescription.name, { importChain: [module.id] });
			if (!variable) module.log(LOGLEVEL_WARN, logMissingExport(importDescription.name, module.id, importDescription.module.id, !!options?.missingButExportExists), importDescription.start);
		}
	}
};
function formatAction([pluginName, hookName, parameters]) {
	...
	switch (hookName) {
		case "resolveId": return `${action} ${s(parameters[0])} ${s(parameters[1])}`;
		case "load": return `${action} ${s(parameters[0])}`;
		case "transform": return `${action} ${s(parameters[1])}`;
		case "shouldTransformCachedModule": return `${action} ${s(parameters[0].id)}`;
...
async function catchUnfinishedHookActions(pluginDriver, callback) {
	const emptyEventLoopPromise = new Promise((_, reject) => {
		...
		if (!handleBeforeExit) {
			handleBeforeExit = () => {
				for (const [pluginDriver$1, reject$1] of rejectByPluginDriver) {
					const unfulfilledActions = pluginDriver$1.getUnfulfilledHookActions();
					reject$1(new Error(`Unexpected early exit. This happens when Promises returned by plugins cannot resolve. Unfinished hook action(s) on exit:\n` + [...unfulfilledActions].map(formatAction).join("\n")));
				}
			};
	...
	const logger = (level, log, skipped = EMPTY_SET) => {
		...
		for (const plugin of plugins) {
			...
			const getLogHandler$1 = (level$1) => {
				if (logLevelPriority[level$1] < minimalPriority) return doNothing;
				return (log$1) => logger(level$1, normalizeLog(log$1), new Set(skipped).add(plugin));
			};
			const handler = "handler" in pluginOnLog ? pluginOnLog.handler : pluginOnLog;
			if (handler.call({
				debug: getLogHandler$1(LOGLEVEL_DEBUG),
				error: (log$1) => error(normalizeLog(log$1)),
				info: getLogHandler$1(LOGLEVEL_INFO),
				meta: {
					rollupVersion: rollupVersion$1,
					watchMode
				},
				warn: getLogHandler$1(LOGLEVEL_WARN)
...
const fs$1 = /* @__PURE__ */ _mergeNamespaces({ __proto__: null }, [promises]);
async function normalizeInputOptions(config, watchMode) {
	...
	warnUnknownOptions(config, [
		...Object.keys(options),
		"onwarn",
		"watch"
	], "input options", onLog, /^(output)$/);
	return {
		options,
		unsetOptions
	};
}
const getCache = (config) => config.cache === true ? void 0 : config.cache?.cache || config.cache;
const getIdMatcher = (option) => {
	if (option === true) return () => true;
	if (typeof option === "function") return (id, ...parameters) => id[0] !== "\0" && option(id, ...parameters) || false;
	if (option) {
		const ids = new Set();
		const matchers = [];
		for (const value of ensureArray(option)) if (value instanceof RegExp) matchers.push(value);
		else ids.add(value);
...
const getJsx = (config) => {
	...
	switch (mode) {
		...
		case "preserve": {
			if (importSource && !(factory || configWithPreset.fragment)) error(logInvalidOption("jsx", URL_JSX, "when preserving JSX and specifying an importSource, you also need to specify a factory or fragment"));
			return {
		...
		default: {
			if (mode && mode !== "classic") error(logInvalidOption("jsx.mode", URL_JSX, "mode must be \"automatic\", \"classic\" or \"preserve\"", mode));
			return {
				factory: factory || "React.createElement",
...
const getModuleContext = (config, context) => {
	...
	if (typeof configModuleContext === "function") return (id) => configModuleContext(id) ?? context;
	if (configModuleContext) {
		const contextByModuleId = Object.create(null);
		for (const [key, moduleContext] of Object.entries(configModuleContext)) contextByModuleId[resolve$1(key)] = moduleContext;
...
const getTreeshake = (config) => {
	...
	if (configTreeshake === false) return false;
...
const getHasModuleSideEffects = (moduleSideEffectsOption) => {
	if (typeof moduleSideEffectsOption === "boolean") return () => moduleSideEffectsOption;
	if (moduleSideEffectsOption === "no-external") return (_id, external) => !external;
	if (typeof moduleSideEffectsOption === "function") return (id, external) => id[0] === "\0" ? true : moduleSideEffectsOption(id, external) !== false;
	if (Array.isArray(moduleSideEffectsOption)) {
		const ids = new Set(moduleSideEffectsOption);
		return (id) => ids.has(id);
	}
	if (moduleSideEffectsOption) error(logInvalidOption("treeshake.moduleSideEffects", URL_TREESHAKE_MODULESIDEEFFECTS, "please use one of false, \"no-external\", a function or an array"));
	return () => true;
};
...
const getFile = (config, preserveModules, inputOptions) => {
	...
	if (typeof file === "string") {
		if (preserveModules) return error(logInvalidOption("output.file", URL_OUTPUT_DIR, "you must set \"output.dir\" instead of \"output.file\" when using the \"output.preserveModules\" option"));
		if (!Array.isArray(inputOptions.input)) return error(logInvalidOption("output.file", URL_OUTPUT_DIR, "you must set \"output.dir\" instead of \"output.file\" when providing named inputs"));
	}
	return file;
};
const getFormat = (config) => {
	...
	switch (configFormat) {
		...
		case "system":
		case "systemjs": return "system";
		case "amd":
		case "iife":
		case "umd": return configFormat;
		default: return error(logInvalidOption("output.format", URL_OUTPUT_FORMAT, `Valid values are "amd", "cjs", "system", "es", "iife" or "umd"`, configFormat));
	}
};
const getInlineDynamicImports = (config, inputOptions) => {
	const inlineDynamicImports = config.inlineDynamicImports || false;
	const { input } = inputOptions;
	if (inlineDynamicImports && (Array.isArray(input) ? input : Object.keys(input)).length > 1) return error(logInvalidOption("output.inlineDynamicImports", URL_OUTPUT_INLINEDYNAMICIMPORTS, "multiple inputs are not supported when \"output.inlineDynamicImports\" is true"));
	return inlineDynamicImports;
};
const getPreserveModules = (config, inlineDynamicImports, inputOptions) => {
	const preserveModules = config.preserveModules || false;
	if (preserveModules) {
		if (inlineDynamicImports) return error(logInvalidOption("output.inlineDynamicImports", URL_OUTPUT_INLINEDYNAMICIMPORTS, `this option is not supported for "output.preserveModules"`));
		if (inputOptions.preserveEntrySignatures === false) return error(logInvalidOption("preserveEntrySignatures", URL_PRESERVEENTRYSIGNATURES, "setting this option to false is not supported for \"output.preserveModules\""));
	}
	return preserveModules;
};
const getPreserveModulesRoot = (config) => {
	const { preserveModulesRoot } = config;
	if (preserveModulesRoot === null || preserveModulesRoot === void 0) return void 0;
...
const getAmd = (config) => {
	...
	if ((mergedOption.autoId || mergedOption.basePath) && mergedOption.id) return error(logInvalidOption("output.amd.id", URL_OUTPUT_AMD_ID, "this option cannot be used together with \"output.amd.autoId\"/\"output.amd.basePath\""));
	if (mergedOption.basePath && !mergedOption.autoId) return error(logInvalidOption("output.amd.basePath", URL_OUTPUT_AMD_BASEPATH, "this option only works with \"output.amd.autoId\""));
...
const getAddon = (config, name) => {
	...
	if (typeof configAddon === "function") return configAddon;
	return () => configAddon || "";
};
const getDir = (config, file) => {
	const { dir } = config;
	if (typeof dir === "string" && typeof file === "string") return error(logInvalidOption("output.dir", URL_OUTPUT_DIR, "you must set either \"output.file\" for a single-file build or \"output.dir\" when generating multiple chunks"));
	return dir;
};
const getEntryFileNames = (config, unsetOptions) => {
	const configEntryFileNames = config.entryFileNames;
	if (configEntryFileNames == null) unsetOptions.add("entryFileNames");
	return configEntryFileNames ?? "[name].js";
};
function getExports(config, unsetOptions) {
	const configExports = config.exports;
	if (configExports == null) unsetOptions.add("exports");
	else if (![
		"default",
		"named",
		"none",
		"auto"
	].includes(configExports)) return error(logInvalidExportOptionValue(configExports));
	return configExports || "auto";
}
const getExternalImportAttributes = (config, inputOptions) => {
	if (config.externalImportAssertions != void 0) warnDeprecation(`The "output.externalImportAssertions" option is deprecated. Use the "output.externalImportAttributes" option instead.`, URL_OUTPUT_EXTERNALIMPORTATTRIBUTES, true, inputOptions);
...
const validateInterop = (interop) => {
	if (!ALLOWED_INTEROP_TYPES.has(interop)) return error(logInvalidOption("output.interop", URL_OUTPUT_INTEROP, `use one of ${Array.from(ALLOWED_INTEROP_TYPES, (value) => JSON.stringify(value)).join(", ")}`, interop));
	return interop;
};
const getManualChunks = (config, inlineDynamicImports, preserveModules) => {
	const configManualChunks = config.manualChunks;
	if (configManualChunks) {
		if (inlineDynamicImports) return error(logInvalidOption("output.manualChunks", URL_OUTPUT_MANUALCHUNKS, "this option is not supported for \"output.inlineDynamicImports\""));
		if (preserveModules) return error(logInvalidOption("output.manualChunks", URL_OUTPUT_MANUALCHUNKS, "this option is not supported for \"output.preserveModules\""));
	}
...
const getSourcemapFileNames = (config, unsetOptions) => {
	...
	if (configSourcemapFileNames == null) unsetOptions.add("sourcemapFileNames");
	return configSourcemapFileNames;
};
const getSourcemapBaseUrl = (config) => {
	const { sourcemapBaseUrl } = config;
	if (sourcemapBaseUrl) {
		if (isValidUrl(sourcemapBaseUrl)) return addTrailingSlashIfMissed(sourcemapBaseUrl);
...
async function rollupInternal(rawInputOptions, watcher) {
	...
	await catchUnfinishedHookActions(graph.pluginDriver, async () => {
		try {
			...
			if (watchFiles.length > 0) error_.watchFiles = watchFiles;
			try {
...
async function getInputOptions(initialInputOptions, watchMode) {
	if (!initialInputOptions) throw new Error("You must supply an options object to rollup");
...
async function getProcessedInputOptions(inputOptions, watchMode) {
	...
	for (const plugin of plugins) {
		...
		if (processedOptions) inputOptions = processedOptions;
	}
	return inputOptions;
}
function normalizePlugins(plugins, anonymousPrefix) {
	for (const [index, plugin] of plugins.entries()) if (!plugin.name) plugin.name = `${anonymousPrefix}${index + 1}`;
}
async function handleGenerateWrite(isWrite, inputOptions, unsetInputOptions, rawOutputOptions, graph) {
	...
	return catchUnfinishedHookActions(outputPluginDriver, async () => {
		...
		if (isWrite) {
			timeStart("WRITE", 1);
			if (!outputOptions.dir && !outputOptions.file) return error(logMissingFileOrDirOption());
...
async function getOutputOptionsAndPluginDriver(rawOutputOptions, inputPluginDriver, inputOptions, unsetInputOptions) {
	if (!rawOutputOptions) throw new Error("You must supply an options object");
...
function createOutput(outputBundle) {
	return { output: Object.values(outputBundle).filter((outputFile) => Object.keys(outputFile).length > 0).sort((outputFileA, outputFileB) => getSortingFileType(outputFileA) - getSortingFileType(outputFileB)) };
}
var SortingFileType;
(function(SortingFileType$1) {
	SortingFileType$1[SortingFileType$1["ENTRY_CHUNK"] = 0] = "ENTRY_CHUNK";
	SortingFileType$1[SortingFileType$1["SECONDARY_CHUNK"] = 1] = "SECONDARY_CHUNK";
	SortingFileType$1[SortingFileType$1["ASSET"] = 2] = "ASSET";
})(SortingFileType || (SortingFileType = {}));
function getSortingFileType(file) {
	if (file.type === "asset") return SortingFileType.ASSET;
	if (file.isEntry) return SortingFileType.ENTRY_CHUNK;
	return SortingFileType.SECONDARY_CHUNK;
}
async function writeOutputFile(outputFile, outputOptions, { fs: { mkdir, writeFile } }) {
	const fileName = resolve$1(outputOptions.dir || dirname$1(outputOptions.file), outputFile.fileName);
...
function handleError(error$1, recover = false) {
	const name = error$1.name || error$1.cause?.name;
	const nameSection = name ? `${name}: ` : "";
	const pluginSection = error$1.plugin ? `(plugin ${error$1.plugin}) ` : "";
	const message = `${pluginSection}${nameSection}${error$1.message}`;
	const outputLines = [bold(red(`[!] ${bold(message.toString())}`))];
	if (error$1.url) outputLines.push(cyan(error$1.url));
	if (error$1.loc) outputLines.push(`${relativeId(error$1.loc.file || error$1.id)} (${error$1.loc.line}:${error$1.loc.column})`);
	else if (error$1.id) outputLines.push(relativeId(error$1.id));
	if (error$1.frame) outputLines.push(dim(error$1.frame));
	if (error$1.stack) outputLines.push(dim(error$1.stack?.replace(`${nameSection}${error$1.message}\n`, "")));
	if (error$1.cause) {
		let cause = error$1.cause;
		const causeErrorLines = [];
		let indent = "";
		while (cause) {
			indent += "  ";
			const message$1 = cause.stack || cause;
			causeErrorLines.push(...`[cause] ${message$1}`.split("\n").map((line) => indent + line));
			cause = cause.cause;
		}
...
async function mergeOptions(config, watchMode, rawCommandOptions = EMPTY_COMMAND_OPTIONS, printLog) {
	...
	if (command.output) Object.assign(command, command.output);
...
function getCommandOptions(rawCommandOptions) {
	...
	return {
		...
		globals: typeof rawCommandOptions.globals === "string" ? rawCommandOptions.globals.split(",").reduce((globals, globalDefinition) => {
			...
			if (!external.includes(id)) external.push(id);
			return globals;
...
const getObjectOption = (config, overrides, name, objectifyValue = objectifyOption) => {
	...
	if (commandOption !== void 0) return commandOption && {
		...configOption,
		...commandOption
	};
	return configOption;
...
const normalizeObjectOptionValue = (optionValue, objectifyValue) => {
	...
	if (Array.isArray(optionValue)) return optionValue.reduce((result, value) => value && result && {
		...result,
...
var WatchEmitter = class {
	...
	off(event, listener) {
		...
		if (listeners) listeners.splice(listeners.indexOf(listener) >>> 0, 1);
		return this;
	}
	on(event, listener) {
...
function watch(configs) {
	...
	watchInternal(configs, emitter).catch((error$1) => {
		handleError(error$1);
	});
	return emitter;
}
function ensureTrailingSlash(path$2) {
	if (path$2[path$2.length - 1] !== "/") return `${path$2}/`;
	return path$2;
}
function checkWatchConfig(config) {
	for (const item of config) {
		if (typeof item.watch !== "boolean" && item.watch?.allowInputInsideOutputPath) break;
		if (item.input && item.output) {
			...
			for (const index in input) {
				...
				if (outputWithInputAsSubPath) error(logInvalidOption("watch", URL_WATCH, `the input "${inputPath}" is a subpath of the output "${outputWithInputAsSubPath.dir}"`));
			}
		}
	}
}
async function watchInternal(configs, emitter) {
	...
	if (watchOptionsList.length === 0) return error(logInvalidOption("watch", URL_WATCH, "there must be at least one config where \"watch\" is not set to \"false\""));
	checkWatchConfig(watchOptionsList);
	await loadFsEvents();
	const { Watcher } = await import("./watch-npfW0z_f.js");
	new Watcher(watchOptionsList, emitter);
}
const VERSION = package_.version;

//#endregion
export { createFilter$1 as createFilter$1$1, fseventsImporter, getAugmentedNamespace, getDefaultExportFromCjs as getDefaultExportFromCjs$1, rollup, rollupInternal, watch as watch$1 };